...
{
    static const char *arr[] = { "Type1 font in PFA format",
...
template< typename Iterator, typename T >
typename std::enable_if_t< is_random_access_iterator< Iterator > &&
                           std::is_signed_v< T > >
advance(Iterator &iter, Iterator last, T dist)
{
    if (dist > 0) {
...
static bool font_identify_byextension(const char *filepath, font_type &result)
{

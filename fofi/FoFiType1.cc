...
void FoFiType1::getFontMatrix (double* mat) {
    ...
    if (!parsed) { parse (); }
    for (i = 0; i < 6; ++i) { mat[i] = fontMatrix[i]; }
}

void FoFiType1::writeEncoded (
    const char** newEncoding, FoFiOutputFunc outputFunc, void* outputStream) {
    char buf[512];
    char *line, *line2, *p;
    int i;
    ...
    if (line) {
        ...
        if (line) {
            (*outputFunc) (
                outputStream, line, (int)(((char*)file + len) - line));
        }
    }
}
...
char* FoFiType1::getNextLine (char* line) {
    ...
    if (line >= (char*)file + len) { return NULL; }
    return line;
}

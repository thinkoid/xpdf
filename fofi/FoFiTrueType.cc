                                 ...
                                 bool allowHeadlessCFF)
{
    ...
    ff = new FoFiTrueType(fileA, lenA, false, fontNum, false, allowHeadlessCFF);
    if (!ff->parsedOk) {
        delete ff;
        return NULL;
    }
...
FoFiTrueType *FoFiTrueType::load(const char *fileName, int fontNum,
                                 bool allowHeadlessCFF)
{
    FoFiTrueType *ff;
    char *        fileA;
    ...
    ff = new FoFiTrueType(fileA, lenA, true, fontNum, isDfontA, allowHeadlessCFF);
    if (!ff->parsedOk) {
        delete ff;
        return NULL;
    }
...
int FoFiTrueType::mapCodeToGID(int i, int c)
{
    ...
    case 4:
        ...
        if (segOffset == 0) {
            ...
            gid = getU16BE(pos + 16 + 6 * segCnt + 2 * b + segOffset +
                               2 * (c - segStart),
                           &ok);
            if (gid != 0) {
...
void FoFiTrueType::convertToType42(const char *psName, char **encoding,
                                   int *codeToGID, FoFiOutputFunc outputFunc,
                                   void *outputStream)
{
    GString *buf;
    int      maxUsedGlyph;
...
void FoFiTrueType::convertToCIDType2(const char *psName, int *cidMap, int nCIDs,
            ...
            for (i = 0; i < nCIDs; i += 32768 - 16) {
                ...
                for (j = 0; j < 32768 - 16 && i + j < nCIDs; j += 16) {
                    ...
                    for (k = 0; k < 16 && i + j + k < nCIDs; ++k) {
                        cid = cidMap[i + j + k];
                        buf = GString::format("{0:02x}{1:02x}", (cid >> 8) & 0xff,
                                              cid & 0xff);
                        (*outputFunc)(outputStream, buf->c_str(),
                                      buf->getLength());
                        delete buf;
                    }
                    (*outputFunc)(outputStream, "\n", 1);
...
void FoFiTrueType::writeTTF(FoFiOutputFunc outputFunc, void *outputStream,
                            char *name, int *codeToGID)
{
    ...
    if (name) {
        ...
        for (i = 0; i < 4; ++i) {
            ...
            if (i + 1 == 2) {
                memcpy(newNameTab + 6 + 4 * 12 + next, "\0R\0e\0g\0u\0l\0a\0r",
                       14);
                next += 14;
            } else {
                for (j = 0; j < n; ++j) {
        ...
        if (tables[i].len > 0 && (tables[i].tag & 0xe0000000) &&
            ...
            if (newTables[j].tag == cmapTag && codeToGID) {
                ...
                newTables[j].checksum = computeTableChecksum(
                    (unsigned char *)cmapTab, sizeof(cmapTab));
    ...
    for (i = 0; i < nNewTables; ++i) {
        if (newTables[i].tag == headTag) {
            if (checkRegion(newTables[i].origOffset, newTables[i].len)) {
                (*outputFunc)(outputStream,
                              (char *)file + newTables[i].origOffset, 8);
            ...
            if (checkRegion(newTables[i].origOffset, newTables[i].len)) {
                (*outputFunc)(outputStream,
                              (char *)file + newTables[i].origOffset,
                              newTables[i].len);
            } else {
                for (j = 0; j < newTables[i].len; ++j) {
                    (*outputFunc)(outputStream, "\0", 1);
...
void FoFiTrueType::cvtEncoding(char **encoding, FoFiOutputFunc outputFunc,
                               void *outputStream)
{
    const char *name;
    GString *   buf;
...
void FoFiTrueType::cvtCharStrings(char **encoding, int *codeToGID,
                                  FoFiOutputFunc outputFunc, void *outputStream)
{
    char *   name;
    GString *buf;
...
void FoFiTrueType::cvtSfnts(FoFiOutputFunc outputFunc, void *outputStream,
                            GString *name, bool needVerticalMetrics,
                            int *maxUsedGlyph)
{
    ...
    *maxUsedGlyph = -1;
    ...
    for (i = 0; i < nNewTables; ++i) {
        if (i == t42HeadTable) {
            ...
            dumpString(reinterpret_cast< unsigned char * >(locaData.data()),
                       locaData.size(), outputFunc, outputStream);
        } else if (i == t42GlyfTable) {
            glyfPos = tables[seekTable("glyf")].offset;
            for (j = 0; j < nGlyphs; ++j) {
                if (locaTable[j].len > 12 &&
                    checkRegion(glyfPos + locaTable[j].origOffset,
                                locaTable[j].len)) {
...
void FoFiTrueType::dumpString(const unsigned char *s, size_t n,
                              FoFiOutputFunc pfun, void *pstream)
{
    GString *buf;
...
unsigned FoFiTrueType::computeTableChecksum(const unsigned char *xs,
                                            size_t               n) const
{
    unsigned x = 0;

    for (size_t i = 0; i + 3 < n; i += 4) {
        unsigned word = ((xs[i] & 0xff) << 24) + ((xs[i + 1] & 0xff) << 16) +
                        ((xs[i + 2] & 0xff) << 8) + (xs[i + 3] & 0xff);

...
static T42Table t42Tables[nT42Tables] = {
    { "cvt ", true }, { "fpgm", true },  { "glyf", true }, { "head", true },
    { "hhea", true }, { "hmtx", true },  { "loca", true }, { "maxp", true },
    { "prep", true }, { "vhea", false }, { "vmtx", false }
};
...
FoFiTrueType* FoFiTrueType::make (
    char* fileA, int lenA, int fontNum, bool allowHeadlessCFF) {
    FoFiTrueType* ff;

    ff = new FoFiTrueType (
        fileA, lenA, false, fontNum, false, allowHeadlessCFF);
    if (!ff->parsedOk) {
        delete ff;
        return NULL;
    }
...
FoFiTrueType*
FoFiTrueType::load (const char* fileName, int fontNum, bool allowHeadlessCFF) {
    FoFiTrueType* ff;
    char* fileA;
    int lenA, n;
    bool isDfontA;
    ...
    ff = new FoFiTrueType (
        fileA, lenA, true, fontNum, isDfontA, allowHeadlessCFF);
    if (!ff->parsedOk) {
        delete ff;
        return NULL;
    }
...
FoFiTrueType::FoFiTrueType (
    char* fileA, int lenA, bool freeFileDataA, int fontNum, bool isDfontA,
...
int FoFiTrueType::getNumCmaps () { return nCmaps; }

int FoFiTrueType::getCmapPlatform (int i) { return cmaps[i].platform; }

int FoFiTrueType::getCmapEncoding (int i) { return cmaps[i].encoding; }
...
int FoFiTrueType::mapCodeToGID (int i, int c) {
    ...
    if (i < 0 || i >= nCmaps) { return 0; }
    ok = true;
    pos = cmaps[i].offset;
    switch (cmaps[i].fmt) {
    case 0:
        if (c < 0 || c >= cmaps[i].len - 6) { return 0; }
    ...
    case 4:
        ...
        if (segOffset == 0) { gid = (c + segDelta) & 0xffff; }
            ...
            gid = getU16BE (
                pos + 16 + 6 * segCnt + 2 * b + segOffset + 2 * (c - segStart),
                &ok);
            if (gid != 0) { gid = (gid + segDelta) & 0xffff; }
        }
        break;
    case 6:
        cmapFirst = getU16BE (pos + 6, &ok);
        cmapLen = getU16BE (pos + 8, &ok);
        if (c < cmapFirst || c >= cmapFirst + cmapLen) { return 0; }
...
int FoFiTrueType::mapNameToGID (char* name) {
    if (!nameToGID) { return 0; }
    return nameToGID->lookupInt (name);
}

bool FoFiTrueType::getCFFBlock (char** start, int* length) {
    int i;

    if (!openTypeCFF) { return false; }
    i = seekTable ("CFF ");
    if (!checkRegion (tables[i].offset, tables[i].len)) { return false; }
...
int* FoFiTrueType::getCIDToGIDMap (int* nCIDs) {
    ...
    *nCIDs = 0;
    if (!getCFFBlock (&start, &length)) { return NULL; }
    if (!(ff = FoFiType1C::make (start, length))) { return NULL; }
...
int FoFiTrueType::getEmbeddingRights () {
    ...
    if ((i = seekTable ("OS/2")) < 0) { return 4; }
    ok = true;
    fsType = getU16BE (tables[i].offset + 8, &ok);
    if (!ok) { return 4; }
    if (fsType & 0x0008) { return 2; }
    if (fsType & 0x0004) { return 1; }
    if (fsType & 0x0002) { return 0; }
    return 3;
}
...
void FoFiTrueType::getFontMatrix (double* mat) {
    ...
    if (!getCFFBlock (&start, &length)) { return; }
    if (!(ff = FoFiType1C::make (start, length))) { return; }
...
void FoFiTrueType::convertToType42 (
    ...
    cvtSfnts (outputFunc, outputStream, NULL, false, &maxUsedGlyph);
...
void FoFiTrueType::convertToCIDType2 (
    ...
    // write the guts of the dictionary
    cvtSfnts (
        outputFunc, outputStream, NULL, needVerticalMetrics, &maxUsedGlyph);
...
void FoFiTrueType::writeTTF (
    FoFiOutputFunc outputFunc, void* outputStream, char* name, int* codeToGID) {
    ...
    bool missingCmap, missingName, missingPost, missingOS2;
    bool unsortedLoca, emptyCmap, badCmapLen, abbrevHMTX;
    ...
    unsigned locaChecksum, glyfChecksum, fileChecksum;
    char* tableDir;
    ...
    for (i = 0; i < nTables; ++i) {
        if (tables[i].len == 0) {
            ++nZeroLengthTables;
            if (tables[i].tag == cmapTag) { missingCmap = true; }
            ...
            !(tables[i].tag & 0xe0000000) || !(tables[i].tag & 0x00e00000) ||
    ...
    emptyCmap = badCmapLen = false;
    cmapLen = 0; // make gcc happy
    if (!missingCmap) {
        if (nCmaps == 0) { emptyCmap = true; }
        else {
            ...
            if (cmapLen > tables[cmapIdx].len) { badCmapLen = true; }
        }
    }
    ...
    if (!missingCmap && !missingName && !missingPost && !missingOS2 &&
        ...
        nZeroLengthTables == 0 && nBogusTables == 0 && !name && !codeToGID &&
        !isDfont) {
    ...
    if (unsortedLoca) {
        ...
        std::sort (
            locaTable, locaTable + nGlyphs + 1, cmpTrueTypeLocaIdxFunctor ());
    ...
    for (i = 0; i < nTables; ++i) {
        if (tables[i].len > 0 && (tables[i].tag & 0xe0000000) &&
            (tables[i].tag & 0x00e00000) && (tables[i].tag & 0x0000e000) &&
            ...
            if (checkRegion (tables[i].offset, tables[i].len)) {
                newTables[j].checksum = computeTableChecksum (
                    file + tables[i].offset, tables[i].len);
                if (tables[i].tag == headTag) {
                    // don't include the file checksum
                    newTables[j].checksum -=
                        getU32BE (tables[i].offset + 8, &ok);
                }
            }
            if (newTables[j].tag == cmapTag && codeToGID) {
                newTables[j].len = newCmapLen;
                newTables[j].checksum =
                    computeTableChecksum ((unsigned char*)newCmapTab, newCmapLen);
            }
            else if (newTables[j].tag == cmapTag && emptyCmap) {
                newTables[j].checksum =
                    computeTableChecksum ((unsigned char*)cmapTab, sizeof (cmapTab));
                ...
                newTables[j].checksum =
                    computeTableChecksum ((unsigned char*)newNameTab, newNameLen);
            }
            else if (newTables[j].tag == hheaTag && abbrevHMTX) {
                newTables[j].len = newHHEALen;
                newTables[j].checksum =
                    computeTableChecksum ((unsigned char*)newHHEATab, newHHEALen);
            }
            else if (newTables[j].tag == hmtxTag && abbrevHMTX) {
                newTables[j].len = newHMTXLen;
                newTables[j].checksum =
                    computeTableChecksum ((unsigned char*)newHMTXTab, newHMTXLen);
            }
            ++j;
        }
    }
    ...
    if (missingName) {
        ...
        if (name) {
            ...
            newTables[j].checksum =
                computeTableChecksum ((unsigned char*)nameTab, sizeof (nameTab));
    ...
    if (missingPost) {
        ...
        newTables[j].checksum =
            computeTableChecksum ((unsigned char*)postTab, sizeof (postTab));
    ...
    if (missingOS2) {
        ...
        newTables[j].checksum =
            computeTableChecksum ((unsigned char*)os2Tab, sizeof (os2Tab));
        newTables[j].len = sizeof (os2Tab);
        ++j;
    }
    std::sort (
        newTables, newTables + nNewTables, cmpTrueTypeTableTagFunctor ());
    ...
    for (i = -1, t = (unsigned)nNewTables; t; ++i, t >>= 1)
        ;
    t = 1 << (4 + i);
    ...
    fileChecksum =
        computeTableChecksum ((unsigned char*)tableDir, 12 + nNewTables * 16);
    for (i = 0; i < nNewTables; ++i) { fileChecksum += newTables[i].checksum; }
    ...
    for (i = 0; i < nNewTables; ++i) {
        if (newTables[i].tag == headTag) {
            if (checkRegion (newTables[i].origOffset, newTables[i].len)) {
                (*outputFunc) (
                    outputStream, (char*)file + newTables[i].origOffset, 8);
                ...
                (*outputFunc) (
                    outputStream, (char*)file + newTables[i].origOffset + 12,
                    newTables[i].len - 12);
            }
            else {
            ...
            if (checkRegion (newTables[i].origOffset, newTables[i].len)) {
                (*outputFunc) (
                    outputStream, (char*)file + newTables[i].origOffset,
                    newTables[i].len);
            }
            else {
                for (j = 0; j < newTables[i].len; ++j) {
...
void FoFiTrueType::cvtCharStrings (
    char** encoding, int* codeToGID, FoFiOutputFunc outputFunc,
    void* outputStream) {
    char* name;
    GString* buf;
    char buf2[16];
...
void FoFiTrueType::cvtSfnts (
    ...
    std::sort (
        locaTable, locaTable + nGlyphs + 1, cmpTrueTypeLocaOffsetFunctor ());
    for (i = 0; i < nGlyphs; ++i) {
        locaTable[i].len =
            locaTable[i + 1].origOffset - locaTable[i].origOffset;
    }
    ...
    *maxUsedGlyph = -1;
    ...
    if (needVerticalMetrics) {
        ...
        if (needVhea || needVmtx) {
            ...
            if (needVhea) { ++nNewTables; }
            if (needVmtx) { ++nNewTables; }
        }
    }
        ...
        if (i == t42HeadTable) {
            ...
            for (j = 0; j < nGlyphs; ++j) {
                if (locaTable[j].len > 12) {
                    ...
                    if (length & 3) { length += 4 - (length & 3); }
                    if (checkRegion (
                            glyfPos + locaTable[j].origOffset,
                            locaTable[j].len)) {
                        checksum += computeTableChecksum (
                            file + glyfPos + locaTable[j].origOffset,
                            locaTable[j].len);
                    }
                }
            }
            ...
            if ((j = seekTable (t42Tables[i].tag)) >= 0) {
                ...
                if (checkRegion (tables[j].offset, length)) {
                    checksum =
                        computeTableChecksum (file + tables[j].offset, length);
                }
            }
    ...
    for (i = 0; i < nNewTables; ++i) { checksum += newTables[i].checksum; }
    ...
    for (i = 0; i < nNewTables; ++i) {
        if (i == t42HeadTable) {
            ...
            for (j = 0; j < nGlyphs; ++j) {
                if (locaTable[j].len > 12 &&
                    checkRegion (
                        glyfPos + locaTable[j].origOffset, locaTable[j].len)) {
                    dumpString (
                        file + glyfPos + locaTable[j].origOffset,
            ...
            if ((length = newTables[i].len) > 0) {
                if ((j = seekTable (t42Tables[i].tag)) >= 0 &&
                    ...
                    dumpString (
                        file + tables[j].offset, tables[j].len, outputFunc,
                        outputStream);
                }
                else if (needVerticalMetrics && i == t42VheaTable) {
...
void FoFiTrueType::parse (int fontNum, bool allowHeadlessCFF) {
    ...
    if (isDfont) { parseDfont (fontNum, &offset, &pos); }
    else {
        offset = 0;
        topTag = getU32BE (0, &parsedOk);
        if (!parsedOk) { return; }
        if (topTag == ttcfTag) { parseTTC (fontNum, &pos); }
        else {
            pos = 0;
        }
    }
    ...
    if (seekTable ("hhea") < 0 || seekTable ("maxp") < 0 ||
        seekTable ("hmtx") < 0 || (!openTypeCFF && seekTable ("loca") < 0) ||
    ...
    if (!openTypeCFF) {
        ...
        for (j = 0; j <= nGlyphs; ++j) {
            ...
            if (pos < 0 || pos > len) { parsedOk = false; }
        }
        if (!parsedOk) { return; }
    }
    ...
    if (postFmt == 0x00010000) {
        ...
        for (i = 0; i < n; ++i) {
            ...
            if (j < 258) {
                ...
                if (!ok || !checkRegion (stringPos + 1, m)) { goto err; }
...
int FoFiTrueType::seekTable (const char* tag) {
    ...
    tagI = ((tag[0] & 0xff) << 24) | ((tag[1] & 0xff) << 16) |
           ((tag[2] & 0xff) << 8) | (tag[3] & 0xff);
    for (i = 0; i < nTables; ++i) {
        if (tables[i].tag == tagI) { return i; }
    }
    return -1;
}

...
void FoFiType1C::getFontMatrix (double* mat) {
    ...
    if (topDict.firstOp == 0x0c1e && privateDicts[0].hasFontMatrix) {
        if (topDict.hasFontMatrix) {
            ...
            for (i = 0; i < 6; ++i) { mat[i] = privateDicts[0].fontMatrix[i]; }
        }
    }
    else {
        for (i = 0; i < 6; ++i) { mat[i] = topDict.fontMatrix[i]; }
    }
}
...
void FoFiType1C::convertToType1 (
    ...
    buf = GString::format (
        "/FontMatrix [{0:.8g} {1:.8g} {2:.8g} {3:.8g} {4:.8g} {5:.8g}] "
        "readonly def\n",
    ...
    if (!newEncoding && encoding == (char**)fofiType1StandardEncoding) {
        ...
        (*outputFunc) (
            outputStream, "0 1 255 {1 index exch /.notdef put} for\n", 40);
    ...
    if (privateDicts[0].nStemSnapH) {
        ...
        for (i = 1; i < privateDicts[0].nStemSnapH; ++i) {
            if (privateDicts[0].stemSnapH[i - 1] >=
    ...
    if (privateDicts[0].nStemSnapV) {
        ...
        for (i = 1; i < privateDicts[0].nStemSnapV; ++i) {
            if (privateDicts[0].stemSnapV[i - 1] >=
    ...
    for (i = 0; i < nGlyphs; ++i) {
        ...
        if (ok) {
            ...
            if (ok) {
                eexecCvtGlyph (
                    &eb, buf2, val.pos, val.len, &subrIdx, &privateDicts[0]);
            }
        }
    }
    eexecWrite (&eb, "end\n");
...
void FoFiType1C::convertToCIDType0 (
    ...
    for (i = 0; i < nCIDs; ++i) {
        ...
        if ((gid = cidMap[i]) >= 0) {
            ...
            if (ok) {
                getIndex (
                    privateDicts[fdSelect ? fdSelect[gid] : 0].subrsOffset,
                    &subrIdx, &ok);
                if (!ok) { subrIdx.pos = -1; }
                cvtGlyph (
                    val.pos, val.len, charStrings, &subrIdx,
                    &privateDicts[fdSelect ? fdSelect[gid] : 0], true);
            }
        }
    }
    ...
    if (topDict.hasFontMatrix) {
        ...
        (*outputFunc) (
            outputStream, "/FontMatrix [0.001 0 0 0.001 0 0] def\n", 38);
    }
    buf = GString::format (
    ...
    for (i = 0; i < nFDs; ++i) {
        ...
        if (privateDicts[i].nStemSnapH) {
            ...
            for (j = 1; j < privateDicts[i].nStemSnapH; ++j) {
                if (privateDicts[i].stemSnapH[j - 1] >=
        ...
        if (privateDicts[i].nStemSnapV) {
            ...
            for (j = 1; j < privateDicts[i].nStemSnapV; ++j) {
                if (privateDicts[i].stemSnapV[j - 1] >=
    ...
    for (i = 0; i < nCIDs; i += 256) {
        ...
        if (privateDicts[fd].nStemSnapH) {
            ...
            for (k = 1; k < privateDicts[fd].nStemSnapH; ++k) {
                if (privateDicts[fd].stemSnapH[k - 1] >=
        ...
        if (privateDicts[fd].nStemSnapV) {
            ...
            for (k = 1; k < privateDicts[fd].nStemSnapV; ++k) {
                if (privateDicts[fd].stemSnapV[k - 1] >=
        ...
        if (ok) {
            eexecCvtGlyph (
                &eb, ".notdef", val.pos, val.len, &subrIdx, &privateDicts[fd]);
        }
...
void FoFiType1C::eexecCvtGlyph (
    ...
    eexecWriteCharstring (
        eb, (unsigned char*)charBuf->c_str (), charBuf->getLength ());
    eexecWrite (eb, " ND\n");

    delete charBuf;
}

void FoFiType1C::cvtGlyph (
        ...
        if (!ops[nOps - 1].isNum) {
            ...
            case 0x000a: // callsubr
                if (nOps >= 1) {
                    ...
                    if (ok) {
                        cvtGlyph (
                            val.pos, val.len, charBuf, subrIdx, pDict, false);
                    }
                }
                else {
            ...
            case 0x0c25: // flex1
                ...
                cvtNum (ops[8].num, ops[8].isFP, charBuf);
                cvtNum (ops[9].num, ops[9].isFP, charBuf);
                dx = ops[0].num + ops[2].num + ops[4].num + ops[6].num +
                     ops[8].num;
                dy = ops[1].num + ops[3].num + ops[5].num + ops[7].num +
                     ops[9].num;
                if (fabs (dx) > fabs (dy)) {
                    cvtNum (ops[10].num, ops[10].isFP, charBuf);
                    cvtNum (
                        -dy,
                        ops[1].isFP | ops[3].isFP | ops[5].isFP | ops[7].isFP |
                            ops[9].isFP,
                        charBuf);
                }
                else {
                    cvtNum (
                        -dx,
                        ops[0].isFP | ops[2].isFP | ops[4].isFP | ops[6].isFP |
                            ops[8].isFP,
                        charBuf);
                    cvtNum (ops[10].num, ops[10].isFP, charBuf);
                }
...
void FoFiType1C::cvtGlyphWidth (
    bool useOp, GString* charBuf, Type1CPrivateDict* pDict) {
    double w;
    bool wFP;
    int i;
    ...
    if (useOp) {
        ...
        for (i = 1; i < nOps; ++i) { ops[i - 1] = ops[i]; }
        --nOps;
    }
    else {
...
void FoFiType1C::eexecWriteCharstring (Type1CEexecBuf* eb, unsigned char* s, int n) {
    unsigned char x;
    int i;
...
void FoFiType1C::writePSString (
    char* s, FoFiOutputFunc outputFunc, void* outputStream) {
    char buf[80];
    char* p;
    int i, c;
...
bool FoFiType1C::parse () {
    ...
    gsubrBias =
        (gsubrIdx.len < 1240) ? 107 : (gsubrIdx.len < 33900) ? 1131 : 32768;
...
void FoFiType1C::readTopDict () {
    ...
    while (pos < topDictPtr.pos + topDictPtr.len) {
        ...
        if (!ops[nOps - 1].isNum) {
            --nOps; // drop the operator
            if (topDict.firstOp < 0) { topDict.firstOp = ops[nOps].op; }
            switch (ops[nOps].op) {
...
void FoFiType1C::readPrivateDict (
    int offset, int length, Type1CPrivateDict* pDict) {
    int pos;
    ...
    while (pos < offset + length) {
        ...
        if (!ops[nOps - 1].isNum) {
            ...
            case 0x0006:
                pDict->nBlueValues =
                    getDeltaIntArray (pDict->blueValues, type1CMaxBlueValues);
                break;
            case 0x0007:
                pDict->nOtherBlues =
                    getDeltaIntArray (pDict->otherBlues, type1CMaxOtherBlues);
                break;
            case 0x0008:
                pDict->nFamilyBlues =
                    getDeltaIntArray (pDict->familyBlues, type1CMaxBlueValues);
                break;
            case 0x0009:
                pDict->nFamilyOtherBlues = getDeltaIntArray (
                    pDict->familyOtherBlues, type1CMaxOtherBlues);
                break;
            case 0x0c09: pDict->blueScale = ops[0].num; break;
            case 0x0c0a: pDict->blueShift = (int)ops[0].num; break;
            case 0x0c0b: pDict->blueFuzz = (int)ops[0].num; break;
            case 0x000a:
            ...
            case 0x0c0c:
                pDict->nStemSnapH =
                    getDeltaFPArray (pDict->stemSnapH, type1CMaxStemSnap);
                break;
            case 0x0c0d:
                pDict->nStemSnapV =
                    getDeltaFPArray (pDict->stemSnapV, type1CMaxStemSnap);
                break;
            case 0x0c0e:
                pDict->forceBold = ops[0].num != 0;
                pDict->hasForceBold = true;
                break;
            case 0x0c0f: pDict->forceBoldThreshold = ops[0].num; break;
            case 0x0c11: pDict->languageGroup = (int)ops[0].num; break;
            case 0x0c12: pDict->expansionFactor = ops[0].num; break;
            case 0x0c13: pDict->initialRandomSeed = (int)ops[0].num; break;
            case 0x0013: pDict->subrsOffset = offset + (int)ops[0].num; break;
            case 0x0014:
    ...
    if (topDict.fdSelectOffset == 0) {
        ...
        if (fdSelectFmt == 0) {
            ...
            for (i = 1; i <= nRanges; ++i) {
                ...
                for (j = gid0; j < gid1; ++j) { fdSelect[j] = fd; }
                gid0 = gid1;
            }
        }
        else {
...
bool FoFiType1C::readCharset () {
    ...
    if (topDict.charsetOffset == 0) { charset = fofiType1CISOAdobeCharset; }
...
void FoFiType1C::getIndex (int pos, Type1CIndex* idx, bool* ok) {
    ...
    if (idx->len == 0) {
        ...
        if (idx->offSize < 1 || idx->offSize > 4) { *ok = false; }
        idx->startPos = pos + 3 + (idx->len + 1) * idx->offSize - 1;
        if (idx->startPos < 0 || idx->startPos >= len) { *ok = false; }
        idx->endPos =
            idx->startPos +
            getUVarBE (pos + 3 + idx->len * idx->offSize, idx->offSize, ok);
        if (idx->endPos < idx->startPos || idx->endPos > len) { *ok = false; }
    }
}

void FoFiType1C::getIndexVal (
    Type1CIndex* idx, int i, Type1CIndexVal* val, bool* ok) {
    int pos0, pos1;
    ...
    pos0 = idx->startPos +
           getUVarBE (idx->pos + 3 + i * idx->offSize, idx->offSize, ok);
    pos1 = idx->startPos +
           getUVarBE (idx->pos + 3 + (i + 1) * idx->offSize, idx->offSize, ok);
    if (pos0 < idx->startPos || pos0 > idx->endPos || pos1 <= idx->startPos ||
        pos1 > idx->endPos || pos1 < pos0) {
        *ok = false;
    }
    val->pos = pos0;
    val->len = pos1 - pos0;

...
void FoFiType1C::convertToType1(const char *psName, const char **newEncoding,
                                ...
                                void *outputStream)
{
    ...
    if (privateDicts[0].nStemSnapH) {
        ...
        if (i == privateDicts[0].nStemSnapH) {
            ...
            for (i = 0; i < privateDicts[0].nStemSnapH; ++i) {
                buf = GString::format("{0:s}{1:.4g}", i > 0 ? " " : "",
                                      privateDicts[0].stemSnapH[i]);
                eexecWrite(&eb, buf->c_str());
                delete buf;
            }
    ...
    for (i = 0; i < nGlyphs; ++i) {
        ...
        if (ok) {
            ...
            if (ok) {
                eexecCvtGlyph(&eb, buf2, val.pos, val.len, &subrIdx,
                              &privateDicts[0]);
            }
        }
    }
                                   ...
                                   FoFiOutputFunc outputFunc, void *outputStream)
{
        ...
        if (privateDicts[i].nStemSnapH) {
            ...
            if (j == privateDicts[0].nStemSnapH) {
                ...
                for (j = 0; j < privateDicts[i].nStemSnapH; ++j) {
                    buf = GString::format("{0:s}{1:.4g}", j > 0 ? " " : "",
                                          privateDicts[i].stemSnapH[j]);
                    (*outputFunc)(outputStream, buf->c_str(), buf->getLength());
                    delete buf;
                }
    ...
    for (i = 0; i < n; i += 32) {
        for (j = 0; j < 32 && i + j < n; ++j) {
            buf = GString::format("{0:02x}", (*charStrings)[i + j] & 0xff);
            (*outputFunc)(outputStream, buf->c_str(), buf->getLength());
            delete buf;
        }
        if (i + 32 >= n) {
                                ...
                                FoFiOutputFunc outputFunc, void *outputStream)
{
    ...
    for (i = 0; i < nCIDs; i += 256) {
        ...
        if (privateDicts[fd].nStemSnapH) {
            ...
            if (k == privateDicts[0].nStemSnapH) {
                ...
                for (k = 0; k < privateDicts[fd].nStemSnapH; ++k) {
                    buf = GString::format("{0:s}{1:.4g}", k > 0 ? " " : "",
                                          privateDicts[fd].stemSnapH[k]);
                    eexecWrite(&eb, buf->c_str());
                    delete buf;
                }
        ...
        if (ok) {
            eexecCvtGlyph(&eb, ".notdef", val.pos, val.len, &subrIdx,
                          &privateDicts[fd]);
        }
        ...
        for (j = 0; j < 256 && i + j < nCIDs; ++j) {
            if (cidMap[i + j] >= 0) {
                ...
                if (ok) {
                    ...
                    eexecCvtGlyph(&eb, buf->c_str(), val.pos, val.len, &subrIdx,
                                  &privateDicts[fd]);
                    delete buf;
                }
            }
...
void FoFiType1C::eexecCvtGlyph(Type1CEexecBuf *eb, const char *glyphName,
                               int offset, int nBytes, Type1CIndex *subrIdx,
        ...
        if (!ops[nOps - 1].isNum) {
            ...
            case 0x000a: // callsubr
                if (nOps >= 1) {
                    ...
                    if (ok) {
                        cvtGlyph(val.pos, val.len, charBuf, subrIdx, pDict,
                                 false);
                    }
                } else {
...
void FoFiType1C::cvtGlyphWidth(bool useOp, GString *charBuf,
                               Type1CPrivateDict *pDict)
{
    double w;
    bool   wFP;
    ...
    if (topDict.encodingOffset == 0) {
        ...
        if ((encFormat & 0x7f) == 0) {
            ...
            for (i = 1; i < nCodes; ++i) {
                ...
                encoding[c] = strdup(getString(charset[i], buf, &parsedOk));
            }
            ...
            for (i = 0; i < nRanges; ++i) {
                ...
                for (j = 0; j <= nLeft && nCodes < nGlyphs; ++j) {
                    if (c < 256) {
                        ...
                        encoding[c] =
                            strdup(getString(charset[nCodes], buf, &parsedOk));
                    }
                    ++nCodes;
                    ++c;
                }
...
void FoFiType1C::getIndexVal(Type1CIndex *idx, int i, Type1CIndexVal *val,
                             bool *ok)
{
    int pos0, pos1;

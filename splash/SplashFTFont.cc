...
static int
glyphPathConicTo (const FT_Vector* ctrl, const FT_Vector* pt, void* path);
static int glyphPathCubicTo (
    const FT_Vector* ctrl1, const FT_Vector* ctrl2, const FT_Vector* pt,
    void* path);
...
SplashFTFont::SplashFTFont (
    ...
    face = fontFileA->face;
    if (FT_New_Size (face, &sizeObj)) { return; }
    face->size = sizeObj;
...
GBool SplashFTFont::makeGlyph (
    ...
    if (ff->engine->flags & splashFTNoHinting) { flags |= FT_LOAD_NO_HINTING; }
...
SplashPath* SplashFTFont::getGlyphPath (int c) {
    static FT_Outline_Funcs outlineFuncs = {
        ...
        (int (*) (FT_Vector*, FT_Vector*, FT_Vector*, void*)) &
            glyphPathCubicTo,
#else
        &glyphPathMoveTo,
        &glyphPathLineTo,
...
glyphPathConicTo (const FT_Vector* ctrl, const FT_Vector* pt, void* path) {
    ...
    if (!p->path->getCurPt (&x0, &y0)) { return 0; }
...
static int glyphPathCubicTo (
    ...
    p->path->curveTo (
        (SplashCoord)ctrl1->x * p->textScale / 64.0,

...
SplashClip::SplashClip(SplashClip *clip)
{
    ...
    scanners = (SplashXPathScanner **)calloc(size, sizeof(SplashXPathScanner *));
    for (i = 0; i < length; ++i) {
...
void SplashClip::resetToRect(SplashCoord x0, SplashCoord y0, SplashCoord x1,
                             SplashCoord y1)
{
    int w, i;
...
SplashError SplashClip::clipToRect(SplashCoord x0, SplashCoord y0, SplashCoord x1,
                                   SplashCoord y1)
{
    if (x0 < x1) {
        if (x0 > xMin) {
                                   ...
                                   SplashCoord flatness, bool eoA)
{
    ...
    if (xPath->length == 4 && xPath->segs[0].y0 == xPath->segs[0].y1 &&
        ...
        clipToRect(xPath->segs[1].x0, xPath->segs[0].y0, xPath->segs[2].x0,
                   xPath->segs[3].y0);
        delete xPath;
        return splashOk;
    }
    if (xPath->length == 4 && xPath->segs[0].x0 == xPath->segs[0].x1 &&
        ...
        xPath->segs[2].x0 == xPath->segs[2].x1 &&
        xPath->segs[3].y0 == xPath->segs[3].y1) {
        clipToRect(xPath->segs[0].x0, xPath->segs[1].y0, xPath->segs[2].x0,
                   xPath->segs[3].y0);
        delete xPath;
        return splashOk;
    }
    if (xPath->length == 4 && xPath->segs[0].x0 == xPath->segs[0].x1 &&
        ...
        xPath->segs[3].y0 == xPath->segs[3].y1) {
        clipToRect(xPath->segs[0].x0, xPath->segs[2].y0, xPath->segs[1].x0,
                   xPath->segs[3].y0);
        delete xPath;
        return splashOk;
    }
...
SplashClipResult SplashClip::testRect(int rectXMin, int rectYMin, int rectXMax,
                                      int rectYMax, bool strokeAdjust)
{
...
void SplashClip::clipSpan(unsigned char *line, int y, int x0, int x1,
                          bool strokeAdjust)
{
    SplashCoord d;
    int         x0a, x1a, x, i;
...
bool SplashClip::clipSpanBinary(unsigned char *line, int y, int x0, int x1,
                                bool strokeAdjust)
{

...
SplashFTFontEngine::SplashFTFontEngine (
    GBool aaA, Guint flagsA, FT_Library libA) {
    ...
    useCIDs =
        major > 2 || (major == 2 && (minor > 1 || (minor == 1 && patch > 7)));
}

SplashFTFontEngine* SplashFTFontEngine::init (GBool aaA, Guint flagsA) {
    FT_Library libA;

    if (FT_Init_FreeType (&libA)) { return NULL; }
    return new SplashFTFontEngine (aaA, flagsA, libA);
}

SplashFTFontEngine::~SplashFTFontEngine () { FT_Done_FreeType (lib); }

SplashFontFile* SplashFTFontEngine::loadType1Font (
...
#endif
    const char** enc) {
    return SplashFTFontFile::loadType1Font (
        this, idA,
#if LOAD_FONTS_FROM_MEM
        fontBuf,
#else
...
SplashFontFile* SplashFTFontEngine::loadType1CFont (
    SplashFontFileID* idA,
#if LOAD_FONTS_FROM_MEM
    GString* fontBuf,
#else
...
SplashFontFile* SplashFTFontEngine::loadOpenTypeT1CFont (
    ...
    if (!(ff = FoFiTrueType::make (
              fontBuf->c_str (), fontBuf->getLength (), 0, gTrue))) {
#else
    ...
    if (ff->isHeadlessCFF ()) {
        ...
        delete ff;
        ret =
            SplashFTFontFile::loadType1Font (this, idA, fontBuf2, enc, gFalse);
        if (ret) { delete fontBuf; }
        else {
            delete fontBuf2;
        }
#else
        ...
        ret = SplashFTFontFile::loadType1Font (
            this, idA, tmpFileName->c_str (), gTrue, enc, gFalse);
        if (ret) {
            if (deleteFile) { unlink (fileName); }
        }
        else {
...
SplashFontFile* SplashFTFontEngine::loadCIDFont (
    SplashFontFileID* idA,
#if LOAD_FONTS_FROM_MEM
    GString* fontBuf
#else
...
SplashFontFile* SplashFTFontEngine::loadOpenTypeCFFFont (
    SplashFontFileID* idA,
#if LOAD_FONTS_FROM_MEM
    GString* fontBuf,
#else
    ...
    nCIDs = 0;
    if (ff->isHeadlessCFF ()) {
        if (!ff->getCFFBlock (&cffStart, &cffLength)) { return NULL; }
#if LOAD_FONTS_FROM_MEM
        fontBuf2 = new GString (cffStart, cffLength);
        if (!useCIDs) { cidToGIDMap = ff->getCIDToGIDMap (&nCIDs); }
        ret = SplashFTFontFile::loadCIDFont (
            this, idA, fontBuf2, cidToGIDMap, nCIDs);
        if (ret) { delete fontBuf; }
        else {
            delete fontBuf2;
        }
        ...
        if (!useCIDs) { cidToGIDMap = ff->getCIDToGIDMap (&nCIDs); }
        ret = SplashFTFontFile::loadCIDFont (
            this, idA, tmpFileName->c_str (), gTrue, cidToGIDMap, nCIDs);
        if (ret) {
            if (deleteFile) { unlink (fileName); }
        }
        else {
...
SplashFontFile* SplashFTFontEngine::loadTrueTypeFont (
    SplashFontFileID* idA,
#if LOAD_FONTS_FROM_MEM
    GString* fontBuf,
#else
...
#endif
    delete ff;
    ret = SplashFTFontFile::loadTrueTypeFont (
        this, idA,
#if LOAD_FONTS_FROM_MEM
        fontBuf2,
#else
        tmpFileName->c_str (), gTrue,
#endif

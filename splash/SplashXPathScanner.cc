...
SplashXPathScanner::SplashXPathScanner(SplashXPath *xPathA, bool eoA, int yMinA,
                                       int yMaxA)
{
    xPath = xPathA;
        ...
        if (intersect) {
            for (; y0 < y1; y0 += minVertStep) {
                ...
                for (auto p : activeSegs) {
                    ...
                    if (!state0 && state1) {
                        ...
                        drawRectangle(line, x0, x1, y0, y1p, seg0->xCur0,
                                      p->xCur0);
                    }
            ...
            for (auto p : activeSegs) {
                ...
                if (!state0 && state1) {
                    ...
                    drawTrapezoid(line, x0, x1, y0, y1, seg0->xCur0, seg0->xCur1,
...
void SplashXPathScanner::drawTrapezoid(unsigned char *line, int xMin, int xMax,
                                       SplashCoord y0, SplashCoord y1,
                                       SplashCoord xa0, SplashCoord xa1,
                                       SplashCoord dydxa, SplashCoord xb0,
...
SplashCoord SplashXPathScanner::areaLeft(int xp, SplashCoord x0, SplashCoord y0,
...
// Compute area within a pixel slice ((xp,y0)-(xp+1,y1)) to the left
// of a trapezoid edge ((x0,y0)-(x1,y1)).
SplashCoord SplashXPathScanner::areaRight(int xp, SplashCoord x0, SplashCoord y0,
                                          SplashCoord x1, SplashCoord y1,
                                          SplashCoord dydx)
{
    SplashCoord a, ya, yb;
...
void SplashXPathScanner::drawRectangle(unsigned char *line, int xMin, int xMax,
                                       SplashCoord y0, SplashCoord y1,

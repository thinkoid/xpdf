...
GBool parseArgs (ArgDesc* args, int* argc, char* argv[]) {
    ...
    while (i < *argc) {
        if (!strcmp (argv[i], "--")) {
            ...
            if (!grabArg (arg, i, argc, argv)) ok = gFalse;
        }
        else {
            ++i;
        }
...
void printUsage (const char* program, const char* otherArgs, ArgDesc* args) {
    ...
    for (arg = args; arg->arg; ++arg) {
        if ((w1 = (int)strlen (arg->arg)) > w) w = w1;
    }

    fprintf (stderr, "Usage: %s [options]", program);
    if (otherArgs) fprintf (stderr, " %s", otherArgs);
    fprintf (stderr, "\n");
    ...
    for (arg = args; arg->arg; ++arg) {
        ...
        switch (arg->kind) {
        case argInt:
        case argIntDummy: typ = " <int>"; break;
        case argFP:
        case argFPDummy: typ = " <fp>"; break;
        case argString:
        case argStringDummy: typ = " <string>"; break;
        case argFlag:
        case argFlagDummy:
        default: typ = ""; break;
        }
...
static ArgDesc* findArg (ArgDesc* args, char* arg) {
    ...
    for (p = args; p->arg; ++p) {
        if (p->kind < argFlagDummy && !strcmp (p->arg, arg)) return p;
    }
    return NULL;
}
...
static GBool grabArg (ArgDesc* arg, int i, int* argc, char* argv[]) {
    ...
    if (n > 0) {
        *argc -= n;
        for (j = i; j < *argc; ++j) argv[j] = argv[j + n];
    }
    return ok;
}
...
GBool isFP (char* s) {
    ...
    if (n > 0 && (*s == 'e' || *s == 'E')) {
        ...
        if (*s == '-' || *s == '+') ++s;
        n = 0;
        if (!isdigit (*s & 0xff)) return gFalse;
        do { ++s; } while (isdigit (*s & 0xff));
    }
    if (*s) return gFalse;
    return gTrue;
}

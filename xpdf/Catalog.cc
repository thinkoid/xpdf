...
LinkDest *Catalog::findDest(GString *name)
{
    ...
    dest = NULL;
    if (obj1.is_array()) {
        dest = new LinkDest(obj1.as_array());
...
void Catalog::loadPage2(int pg, int relPg, PageTreeNode *node)
{
    ...
    if (!node->kids) {
        ...
        // merge the PageAttrs
        attrs =
            new PageAttrs(node->parent ? node->parent->attrs : (PageAttrs *)NULL,
                          &pageObj.as_dict());
            ...
            for (i = 0; i < kidsObj.as_array().size(); ++i) {
                ...
                if (kidRefObj.is_ref()) {
                    if ((kidObj = resolve(kidRefObj)).is_dict()) {
                        if ((countObj = resolve(kidObj.as_dict()["Count"]))
                                .is_int()) {
                        ...
                        node->kids->append(
                            new PageTreeNode(kidRefObj.as_ref(), count, node));
                    } else {
                        error(errSyntaxError, -1,
...
void Catalog::readFileAttachmentAnnots(const Object &pageNodeRef,
                                       char *        touchedObjs)
{
                    ...
                    if ((annot = resolve(annots[i])).is_dict()) {
                        ...
                        if (subtype.is_name("FileAttachment")) {
                            ...
                            if (!fileSpec.is_null()) {
                                readEmbeddedFile(
                                    fileSpec,
                                    resolve(annot.as_dict()["Contents"]));
                            }
                        }
                    }
                }

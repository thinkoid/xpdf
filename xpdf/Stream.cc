...
char* Stream::getLine (char* buf, int size) {
    ...
    if (lookChar () == EOF || size < 0) return NULL;
    for (i = 0; i < size - 1; ++i) {
        c = getChar ();
        if (c == EOF || c == '\n') break;
        if (c == '\r') {
...
Guint Stream::discardChars (Guint n) {
    ...
    count = 0;
    while (count < n) {
        if ((i = n - count) > sizeof (buf)) { i = (Guint)sizeof (buf); }
...
void FilterStream::close () { str->close (); }
...
ImageStream::ImageStream (Stream* strA, int widthA, int nCompsA, int nBitsA) {
    ...
    if (nBits == 8) { imgLine = (Guchar*)inputLine; }
    else {
...
ImageStream::~ImageStream () {
    if (imgLine != (Guchar*)inputLine) { gfree (imgLine); }
    gfree (inputLine);
}

void ImageStream::reset () { str->reset (); }

void ImageStream::close () { str->close (); }

GBool ImageStream::getPixel (Guchar* pix) {
    int i;

    if (imgIdx >= nVals) {
        if (!getLine ()) { return gFalse; }
        imgIdx = 0;
    }
    for (i = 0; i < nComps; ++i) { pix[i] = imgLine[imgIdx++]; }
    return gTrue;
}
...
Guchar* ImageStream::getLine () {
    ...
    if (nBits == 1) {
        ...
        for (i = 0; i < nVals; ++i) { imgLine[i] = (Guchar)inputLine[2 * i]; }
    }
    else {
        bitMask = (1 << nBits) - 1;
        buf = 0;
...
void ImageStream::skipLine () { str->getBlock (inputLine, inputLineSize); }
...
GBool StreamPredictor::getNextLine () {
    ...
    if (predictor >= 10) {
        if ((curPred = str->getRawChar ()) == EOF) { return gFalse; }
        curPred += 10;
    }
    else {
        curPred = predictor;
    ...
    for (i = pixBytes; i < rowBytes; ++i) {
        ...
        case 13: // PNG average
            predLine[i] =
                ((predLine[i - pixBytes] + predLine[i]) >> 1) + (Guchar)c;
            break;
        case 14: // PNG Paeth
...
FileStream::FileStream (
    FILE* fA, GFileOffset startA, GBool limitedA, GFileOffset lengthA,
    Object* dictA)
    : BaseStream (dictA) {
    f = fA;
    start = startA;
...
Stream* FileStream::makeSubStream (
    GFileOffset startA, GBool limitedA, GFileOffset lengthA, Object* dictA) {
...
GBool FileStream::fillBuf () {
    ...
    bufPtr = bufEnd = buf;
    if (limited && bufPos >= start + length) { return gFalse; }
    ...
    bufEnd = buf + n;
    if (bufPtr >= bufEnd) { return gFalse; }
    return gTrue;
}
...
Stream* MemStream::makeSubStream (
    ...
    if (startA < start) { newStart = start; }
...
void MemStream::reset () { bufPtr = buf + start; }
...
int MemStream::getBlock (char* blk, int size) {
    ...
    if (size <= 0) { return 0; }
    if (bufEnd - bufPtr < size) { n = (int)(bufEnd - bufPtr); }
    else {
        n = size;
    }
    memcpy (blk, bufPtr, n);
...
EmbedStream::EmbedStream (
    Stream* strA, Object* dictA, GBool limitedA, GFileOffset lengthA)
    : BaseStream (dictA) {
    str = strA;
    limited = limitedA;
    length = lengthA;
...
Stream* EmbedStream::makeSubStream (
    GFileOffset start, GBool limitedA, GFileOffset lengthA, Object* dictA) {
...
int EmbedStream::getBlock (char* blk, int size) {
    if (size <= 0) { return 0; }
    if (limited && length < (Guint)size) { size = (int)length; }
...
int ASCIIHexStream::lookChar () {
    ...
    if (buf != EOF) return buf;
    if (eof) {
        buf = EOF;
        return EOF;
    }
    do { c1 = str->getChar (); } while (isspace (c1));
    if (c1 == '>') {
        eof = gTrue;
        buf = EOF;
        return buf;
    }
    do { c2 = str->getChar (); } while (isspace (c2));
    if (c2 == '>') {
        eof = gTrue;
        c2 = '0';
    }
...
GBool ASCIIHexStream::isBinary (GBool last) { return str->isBinary (gFalse); }
...
int ASCII85Stream::lookChar () {
    ...
    if (index >= n) {
        ...
        if (c[0] == '~' || c[0] == EOF) {
            ...
            n = k - 1;
            if (k < 5 && (c[k] == '~' || c[k] == EOF)) {
                for (++k; k < 5; ++k) c[k] = 0x21 + 84;
                eof = gTrue;
            }
            t = 0;
            for (k = 0; k < 5; ++k) t = t * 85 + (c[k] - 0x21);
            for (k = 3; k >= 0; --k) {
                b[k] = (int)(t & 0xff);
                t >>= 8;
            }
...
GBool ASCII85Stream::isBinary (GBool last) { return str->isBinary (gFalse); }
...
int LZWStream::getChar () {
    if (pred) { return pred->getChar (); }
    if (eof) { return EOF; }
    if (seqIndex >= seqLength) {
        if (!processNextCode ()) { return EOF; }
    }
    return seqBuf[seqIndex++];
}

int LZWStream::lookChar () {
    if (pred) { return pred->lookChar (); }
    if (eof) { return EOF; }
    if (seqIndex >= seqLength) {
        if (!processNextCode ()) { return EOF; }
    }
...
int LZWStream::getBlock (char* blk, int size) {
    ...
    if (pred) { return pred->getBlock (blk, size); }
    if (eof) { return 0; }
    n = 0;
    while (n < size) {
        if (seqIndex >= seqLength) {
            if (!processNextCode ()) { break; }
        }
...
GBool LZWStream::isBinary (GBool last) { return str->isBinary (gTrue); }
...
GBool RunLengthStream::isBinary (GBool last) { return str->isBinary (gTrue); }
...
GBool RunLengthStream::fillBuf () {
    ...
    if (c < 0x80) {
        n = c + 1;
        for (i = 0; i < n; ++i) buf[i] = (char)str->getChar ();
    }
    else {
        n = 0x101 - c;
        c = str->getChar ();
        for (i = 0; i < n; ++i) buf[i] = (char)c;
    }
    bufPtr = buf;
    bufEnd = buf + n;
    return gTrue;
...
CCITTFaxStream::CCITTFaxStream (
    Stream* strA, int encodingA, GBool endOfLineA, GBool byteAlignA,
    int columnsA, int rowsA, GBool endOfBlockA, GBool blackA)
    : FilterStream (strA) {
    encoding = encodingA;
...
void CCITTFaxStream::reset () {
    ...
    // encoding tag
    while ((code1 = lookBits (12)) == 0) { eatBits (1); }
    if (code1 == 0x001) {
        eatBits (12);
        endOfLine = gTrue;
    }
...
int CCITTFaxStream::lookChar () {
    ...
    if (outputBits == 0) {
        ...
        if (nextLine2D) {
            ...
            while (codingLine[a0i] < columns) {
                ...
                case twoDimPass:
                    ...
                    if (refLine[b1i + 1] < columns) { b1i += 2; }
                    break;
                case twoDimHoriz:
                    code1 = code2 = 0;
                    if (blackPixels) {
        ...
        // case)
        gotEOL = gFalse;
        if (!endOfBlock && row == rows - 1) { eof = gTrue; }
        ...
        // separate bytes.)
        if (byteAlign && !gotEOL) { inputBits &= ~7; }

        // check for end of stream
        if (lookBits (1) == EOF) { eof = gTrue; }
        ...
        // set up for output
        if (codingLine[0] > 0) { outputBits = codingLine[a0i = 0]; }
        else {
    ...
    if (outputBits >= 8) {
        ...
        do {
            if (outputBits > bits) {
                buf <<= bits;
                if (!(a0i & 1)) { buf |= 0xff >> (8 - bits); }
                outputBits -= bits;
                bits = 0;
            }
            else {
                buf <<= outputBits;
                if (!(a0i & 1)) { buf |= 0xff >> (8 - outputBits); }
                bits -= outputBits;
                outputBits = 0;
                if (codingLine[a0i] < columns) {
                    ++a0i;
...
GString* CCITTFaxStream::getPSFilter (int psLevel, const char* indent) {
    ...
    if (endOfLine) { s->append ("/EndOfLine true "); }
    if (byteAlign) { s->append ("/EncodedByteAlign true "); }
    ...
    if (black) { s->append ("/BlackIs1 true "); }
    s->append (">> /CCITTFaxDecode filter\n");
    return s;
}

GBool CCITTFaxStream::isBinary (GBool last) { return str->isBinary (gTrue); }
...
static inline void dctClipInit () {
    ...
    int i;
    if (!initDone) {
        for (i = -384; i < 0; ++i) { dctClipData[dctClipOffset + i] = 0; }
        for (i = 0; i < 256; ++i) { dctClipData[dctClipOffset + i] = i; }
        for (i = 256; i < 639; ++i) { dctClipData[dctClipOffset + i] = 255; }
        initDone = 1;
    }
}
...
void DCTStream::reset () {
    ...
    // compute MCU size
    if (numComps == 1) { compInfo[0].hSample = compInfo[0].vSample = 1; }
    mcuWidth = compInfo[0].hSample;
    mcuHeight = compInfo[0].vSample;
    for (i = 1; i < numComps; ++i) {
        if (compInfo[i].hSample > mcuWidth) { mcuWidth = compInfo[i].hSample; }
    ...
    if (colorXform == -1) {
        if (numComps == 3) {
            if (gotJFIFMarker) { colorXform = 1; }
            else if (
                compInfo[0].id == 82 && compInfo[1].id == 71 &&
    ...
    if (progressive || !interleaved) {
        ...
        if (bufWidth <= 0 || bufHeight <= 0 ||
            ...
            error (
                errSyntaxError, getPos (), "Invalid image size in DCT stream");
            y = height;
            return;
        }
...
int DCTStream::getChar () {
    ...
    if (progressive || !interleaved) {
        ...
        if (rowBufPtr == rowBufEnd) {
            if (y + mcuHeight >= height) { return EOF; }
            y += mcuHeight;
            if (!readMCURow ()) {
                y = height;
                return EOF;
...
void DCTStream::restart () {
    ...
    for (i = 0; i < numComps; ++i) { compInfo[i].prevDC = 0; }
    eobRun = 0;
}
...
GBool DCTStream::readMCURow () {
    ...
    for (x1 = 0; x1 < width; x1 += mcuWidth) {
        ...
        if (restartInterval > 0 && restartCtr == 0) {
            ...
            if (++restartMarker == 0xd8) restartMarker = 0xd0;
            restart ();
        }
        ...
        for (cc = 0; cc < numComps; ++cc) {
            ...
            for (y2 = 0; y2 < mcuHeight; y2 += vert) {
                for (x2 = 0; x2 < mcuWidth; x2 += horiz) {
                    ...
                    transformDataUnit (
                        quantTables[compInfo[cc].quantTable], data1, data2);
    ...
    if (colorXform) {
        ...
        if (numComps == 3) {
            for (i = 0, p1 = rowBuf; i < width * mcuHeight; ++i, p1 += 3) {
                ...
                p1[0] = dctClip (pR);
                pG = ((pY << 16) + dctCbToG * pCb + dctCrToG * pCr + 32768) >>
                     16;
                p1[1] = dctClip (pG);
...
GBool DCTStream::readDataUnit (
    ...
    if ((size = readHuffSym (dcHuffTable)) == 9999) { return gFalse; }
    if (size > 0) {
        if ((amp = readAmp (size)) == 9999) { return gFalse; }
    }
    else {
        amp = 0;
    }
    data[0] = *prevDC += amp;
    for (i = 1; i < 64; ++i) { data[i] = 0; }
    i = 1;
    while (i < 64) {
        run = 0;
        ...
        if (c == 9999) { return gFalse; }
        if (c == 0x00) { break; }
        else {
            run += (c >> 4) & 0x0f;
            size = c & 0x0f;
            amp = readAmp (size);
            if (amp == 9999) { return gFalse; }
            i += run;
            if (i < 64) {
                j = dctZigZag[i++];
                data[j] = amp;
...
GBool DCTStream::readProgressiveDataUnit (
    DCTHuffTable* dcHuffTable, DCTHuffTable* acHuffTable, int* prevDC,
    int data[64]) {
    ...
    if (i == 0) {
        if (scanInfo.ah == 0) {
            ...
            if ((bit = readBit ()) == 9999) { return gFalse; }
            if (bit) {
                if (data[0] >= 0) { data[0] += 1 << scanInfo.al; }
                else {
    ...
    if (scanInfo.lastCoeff == 0) { return gTrue; }
    ...
    if (eobRun > 0) {
        while (i <= scanInfo.lastCoeff) {
            j = dctZigZag[i++];
            if (data[j] != 0) {
                if ((bit = readBit ()) == EOF) { return gFalse; }
                if (bit) {
                    if (data[j] >= 0) { data[j] += 1 << scanInfo.al; }
                    else {
    ...
    while (i <= scanInfo.lastCoeff) {
        if ((c = readHuffSym (acHuffTable)) == 9999) { return gFalse; }
...
// paper.
void DCTStream::transformDataUnit (
    Gushort* quantTable, int dataIn[64], Guchar dataOut[64]) {
    ...
    for (i = 0; i < 64; ++i) { dataOut[i] = dctClip (128 + (dataIn[i] >> 3)); }
}
...
int DCTStream::readHuffSym (DCTHuffTable* table) {
    ...
    do {
        ...
        if ((bit = readBit ()) == EOF) { return 9999; }
        code = (code << 1) + bit;
        ++codeBits;

        // look up code
        if (code < table->firstCode[codeBits]) { break; }
...
int DCTStream::readBit () {
    ...
    if (inputBits == 0) {
        if ((c = str->getChar ()) == EOF) return EOF;
        if (c == 0xff) {
            do { c2 = str->getChar (); } while (c2 == 0xff);
            if (c2 != 0x00) {
                error (
                    errSyntaxError, getPos (),
...
GBool DCTStream::readHeader () {
    ...
    while (!doScan) {
        ...
        case 0xc1: // SOF1 (extended sequential)
            if (!readBaselineSOF ()) { return gFalse; }
            break;
        case 0xc2: // SOF2 (progressive)
            if (!readProgressiveSOF ()) { return gFalse; }
            break;
        case 0xc4: // DHT
            if (!readHuffmanTables ()) { return gFalse; }
            break;
        case 0xd8: // SOI
            break;
        case 0xd9: // EOI
            return gFalse;
        case 0xda: // SOS
            if (!readScanInfo ()) { return gFalse; }
            doScan = gTrue;
            break;
        case 0xdb: // DQT
            if (!readQuantTables ()) { return gFalse; }
            break;
        case 0xdd: // DRI
            if (!readRestartInterval ()) { return gFalse; }
            break;
        case 0xe0: // APP0
            if (!readJFIFMarker ()) { return gFalse; }
            break;
        case 0xee: // APP14
            if (!readAdobeMarker ()) { return gFalse; }
            break;
        case EOF:
        ...
        default:
            ...
            if (c >= 0xe0) {
                ...
                error (
                    errSyntaxError, getPos (), "Unknown DCT marker <{0:02x}>",
                    c);
                return gFalse;
            }
            break;
...
GBool DCTStream::readBaselineSOF () {
    ...
    if (numComps <= 0 || numComps > 4) {
        error (
            errSyntaxError, getPos (),
            "Bad number of components in DCT stream");
        numComps = 0;
        return gFalse;
    }
    if (prec != 8) {
...
GBool DCTStream::readScanInfo () {
    ...
    for (j = 0; j < numComps; ++j) { scanInfo.comp[j] = gFalse; }
    for (i = 0; i < scanInfo.numComps; ++i) {
        ...
        if (id == compInfo[i].id) { j = i; }
        else {
            for (j = 0; j < numComps; ++j) {
                if (id == compInfo[j].id) { break; }
            }
            if (j == numComps) {
                error (
                    errSyntaxError, getPos (),
...
GBool DCTStream::readQuantTables () {
    ...
    while (length > 0) {
        ...
        if (index == numQuantTables) { numQuantTables = index + 1; }
        for (i = 0; i < 64; ++i) {
            if (prec) { quantTables[index][dctZigZag[i]] = read16 (); }
            else {
...
GBool DCTStream::readHuffmanTables () {
    ...
    while (length > 0) {
        ...
        if (index & 0x10) {
            index &= 0x0f;
            if (index >= numACHuffTables) numACHuffTables = index + 1;
            tbl = &acHuffTables[index];
        }
        else {
            index &= 0x0f;
            if (index >= numDCHuffTables) numDCHuffTables = index + 1;
        ...
        length -= 16;
        for (i = 0; i < sym; ++i) tbl->sym[i] = str->getChar ();
        length -= sym;
    }
    return gTrue;
}
...
GBool DCTStream::readJFIFMarker () {
    ...
    if (length >= 5) {
        ...
        length -= 5;
        if (!memcmp (buf, "JFIF\0", 5)) { gotJFIFMarker = gTrue; }
    }
    while (length > 0) {
        if (str->getChar () == EOF) {
            error (errSyntaxError, getPos (), "Bad DCT APP0 marker");
...
GBool DCTStream::readAdobeMarker () {
    ...
    length = read16 ();
    if (length < 14) { goto err; }
    for (i = 0; i < 12; ++i) {
        if ((c = str->getChar ()) == EOF) { goto err; }
        buf[i] = c;
    }
    if (strncmp (buf, "Adobe", 5)) { goto err; }
    colorXform = buf[11];
    gotAdobeMarker = gTrue;
    for (i = 14; i < length; ++i) {
        if (str->getChar () == EOF) { goto err; }
    }
    return gTrue;
...
int DCTStream::readMarker () {
    ...
    do {
        do { c = str->getChar (); } while (c != 0xff && c != EOF);
        do { c = str->getChar (); } while (c == 0xff);
    } while (c == 0x00);
    return c;
}
...
int DCTStream::read16 () {
    ...
    if ((c1 = str->getChar ()) == EOF) return EOF;
    if ((c2 = str->getChar ()) == EOF) return EOF;
    return (c1 << 8) + c2;
}
...
GBool DCTStream::isBinary (GBool last) { return str->isBinary (gTrue); }
...
void FlateStream::readSome () {
    ...
    if (compressedBlock) {
        if ((code1 = getHuffmanCodeWord (&litCodeTab)) == EOF) goto err;
        if (code1 < 256) {
            buf[index] = code1;
            remain = 1;
        }
            ...
            if (code2 > 0 && (code2 = getCodeWord (code2)) == EOF) goto err;
            len = lengthDecode[code1].first + code2;
            if ((code1 = getHuffmanCodeWord (&distCodeTab)) == EOF) goto err;
        ...
        remain = i;
        blockLen -= len;
        if (blockLen == 0) endOfBlock = gTrue;
    }
...
GBool FlateStream::readDynamicCodes () {
    ...
    // read lengths
    if ((numLitCodes = getCodeWord (5)) == EOF) { goto err; }
    numLitCodes += 257;
    if ((numDistCodes = getCodeWord (5)) == EOF) { goto err; }
    numDistCodes += 1;
    if ((numCodeLenCodes = getCodeWord (4)) == EOF) { goto err; }
    numCodeLenCodes += 4;
    ...
    for (i = 0; i < flateMaxCodeLenCodes; ++i) { codeLenCodeLengths[i] = 0; }
    ...
    compHuffmanCodes (
        codeLenCodeLengths, flateMaxCodeLenCodes, &codeLenCodeTab);
    ...
    while (i < numLitCodes + numDistCodes) {
        if ((code = getHuffmanCodeWord (&codeLenCodeTab)) == EOF) { goto err; }
        if (code == 16) {
            if ((repeat = getCodeWord (2)) == EOF) { goto err; }
            repeat += 3;
            if (i + repeat > numLitCodes + numDistCodes) { goto err; }
            for (; repeat > 0; --repeat) { codeLengths[i++] = len; }
        }
        else if (code == 17) {
            if ((repeat = getCodeWord (3)) == EOF) { goto err; }
            repeat += 3;
            if (i + repeat > numLitCodes + numDistCodes) { goto err; }
            len = 0;
            for (; repeat > 0; --repeat) { codeLengths[i++] = 0; }
        }
        else if (code == 18) {
            if ((repeat = getCodeWord (7)) == EOF) { goto err; }
            repeat += 11;
            if (i + repeat > numLitCodes + numDistCodes) { goto err; }
            len = 0;
            for (; repeat > 0; --repeat) { codeLengths[i++] = 0; }
...
void FlateStream::compHuffmanCodes (int* lengths, int n, FlateHuffmanTab* tab) {
    ...
    tab->maxLen = 0;
    for (val = 0; val < n; ++val) {
        if (lengths[val] > tab->maxLen) { tab->maxLen = lengths[val]; }
    }
...
int FlateStream::getHuffmanCodeWord (FlateHuffmanTab* tab) {
    ...
    while (codeSize < tab->maxLen) {
        if ((c = str->getChar ()) == EOF) { break; }
        codeBuf |= (c & 0xff) << codeSize;
        codeSize += 8;
    }
    code = &tab->codes[codeBuf & ((1 << tab->maxLen) - 1)];
    if (codeSize == 0 || codeSize < code->len || code->len == 0) { return EOF; }
    codeBuf >>= code->len;
    codeSize -= code->len;
    return (int)code->val;
}
...
void BufStream::reset () {
    ...
    str->reset ();
    for (i = 0; i < bufSize; ++i) { buf[i] = str->getChar (); }
}
...
int BufStream::getChar () {
    ...
    c = buf[0];
    for (i = 1; i < bufSize; ++i) { buf[i - 1] = buf[i]; }
    buf[bufSize - 1] = str->getChar ();
    return c;
}

int BufStream::lookChar () { return buf[0]; }

int BufStream::lookChar (int idx) { return buf[idx]; }

GBool BufStream::isBinary (GBool last) { return str->isBinary (gTrue); }
...
int FixedLengthEncoder::getChar () {
    if (length >= 0 && count >= length) return EOF;
    ++count;
    return str->getChar ();
}
...
GBool RunLengthEncoder::fillBuf () {
    ...
    if (c1 == c2) {
        n = 2;
        while (n < 128 && (c = str->getChar ()) == c1) ++n;
        buf[0] = (char)(257 - n);
        buf[1] = c1;
        bufEnd = &buf[2];
        if (c == EOF) { eof = gTrue; }
...
int LZWEncoder::getChar () {
    ...
    if (inBufLen == 0 && !needEOD && outBufLen == 0) { return EOF; }
    if (outBufLen < 8 && (inBufLen > 0 || needEOD)) { fillBuf (); }
    if (outBufLen >= 8) {
...
int LZWEncoder::lookChar () {
    if (inBufLen == 0 && !needEOD && outBufLen == 0) { return EOF; }
    if (outBufLen < 8 && (inBufLen > 0 || needEOD)) { fillBuf (); }
    if (outBufLen >= 8) { return (outBuf >> (outBufLen - 8)) & 0xff; }
    else {
...
void LZWEncoder::fillBuf () {
    ...
    if (table[code].children) { table[nextSeq].next = table[code].children; }
    else {
    ...
    inBufLen -= seqLen;
    inBufLen +=
        str->getBlock ((char*)inBuf + inBufLen, sizeof (inBuf) - inBufLen);

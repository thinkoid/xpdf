...
AnnotBorderStyle::AnnotBorderStyle(AnnotBorderType typeA, double widthA,
                                   double *dashA, int dashLengthA, double *colorA,
                                   int nColorCompsA)
{
    type = typeA;
    width = widthA;
...
Annot::Annot(PDFDoc *docA, Dict *dict, Ref *refA)
{
    ...
    borderStyle = new AnnotBorderStyle(borderType, borderWidth, borderDash,
                                       borderDashLength, borderColor,
                                       nBorderColorComps);
...
void Annot::adjustLineEndpoint(AnnotLineEndType lineEnd, double x, double y,
                               double dx, double dy, double w, double *tx,
                               double *ty)
{
    switch (lineEnd) {
...
void Annot::drawLineArrow(AnnotLineEndType lineEnd, double x, double y, double dx,
                          double dy, double w, bool fill)
{
    switch (lineEnd) {
    case annotLineEndNone:
        break;
            ...
            if (obj1.is_dict()) {
                if (drawWidgetAnnots ||
                    ...
                    if (annot->isOk()) {
                        if (nAnnots >= size) {
                            size += 16;
                            annots = (Annot **)reallocarray(annots, size,
                                                            sizeof(Annot *));
                        }

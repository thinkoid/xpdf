...
struct function_t::impl_t
{
    ...
    virtual void operator()(const double *, const double *const,
                            double *) const = 0;
...
{
    return std::vector< std::tuple< double, double > >{ function_t::max_arity,
                                                        { 0., 1. } };
}
...
identity_function_t::identity_function_t()
    : domain(make_default_domain())
    , range(make_default_range())
{
}

void identity_function_t::operator()(const double *src, const double *const end,
                                     double *dst) const
{
    copy(src, end, dst);
}
...
std::vector< off_t > make_offsets(const std::vector< size_t > &zs, size_t n)
{
...
std::vector< char > block_from(Object &obj, size_t n)
{
...
inline std::vector< double > samples_from(const std::vector< char > &xs, size_t n,
                                          size_t bps)
{
...
std::vector< std::tuple< double, double > >
decode_array_from(Dict &                                             dict,
                  const std::vector< std::tuple< double, double > > &default_)
{
...
std::vector< double >
multipliers_from(const std::vector< std::tuple< double, double > > &d,
                 const std::vector< std::tuple< double, double > > &e)
{
    std::vector< double > xs;

    transform(views::zip(d, e), ranges::back_inserter(xs), [](auto arg) {
        const auto &[d, e] = arg;
...
sampled_function_t::sampled_function_t(Object &obj, Dict &dict)
    : domain(domain_from(dict))
    , range(range_from(dict))
{
    ...
    const size_t nsamples =
        accumulate(sizes, range.size(), std::multiplies< size_t >{});
...
void sampled_function_t::operator()(const double *src, const double *const end,
                                    double *dst) const
{
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        for (j = 0, t = (1UL << m); j < m; ++j, t >>= 1) {
            for (k = 0; k < t; k += 2) {
                scratch[k >> 1] =
                    efrac0[j] * scratch[k] + efrac1[j] * scratch[k + 1];
            }
        }
...
inline std::vector< double > exponential_array_from(Dict &dict, const char *s,
                                                    int default_)
{
...
exponential_function_t::exponential_function_t(Object &obj, Dict &dict)
    : domain(domain_from(dict))
    , range(optional_range_from(dict))
...
void exponential_function_t::operator()(const double *      src,
...
std::shared_ptr< function_t::impl_t > make_function(Object &, size_t = 0);
...
auto bounds_array_from(Dict &dict, double lhs, double rhs)
{
...
inline auto encode_array_from(Dict &dict)
{
    return as_array< std::tuple< double, double > >(dict, "Encode");
}

stitching_function_t::stitching_function_t(Object &, Dict &dict, int recursion)
    : domain(domain_from(dict))
    , range(optional_range_from(dict))
{
    ASSERT(1UL == domain.size());
    ...
    auto is_like = [](int n) {
        return [=](auto &f) { return f->coarity() == n; };
    };
    ASSERT(::all_of(fs, is_like(fs[0]->coarity())));
    ASSERT(range.empty() || range.size() == fs[0]->coarity());

    bounds = bounds_array_from(dict, std::get< 0 >(domain[0]),
                               std::get< 1 >(domain[0]));
...
void stitching_function_t::operator()(const double *src, const double *const end,
                                      double *dst) const
{
    ASSERT(1UL == domain.size());
    const auto &[d_0, d_1] = domain[0];
...
std::vector< postscript_function_t::code_t > parse(Iterator &iter, Iterator last)
{
    ...
    for (; iter != last;) {
        ...
        if (isdigit(tok[0]) || tok[0] == '.' || tok[0] == '-') {
            ...
            if (iter2 == ns.end()) {
                throw std::runtime_error(format("invalid PostScript: {}", tok));
            }
...
void postscript_function_t::operator()(const double *src, const double *const end,
                                       double *dst) const
{
...
static inline Dict *dictionary_from(Object &obj)
{
    return obj.is_stream() ? obj.streamGetDict() :
                             obj.is_dict() ? &obj.as_dict() : 0;
}

std::shared_ptr< function_t::impl_t > make_function(Object &obj,
                                                    size_t  recursion /* = 0 */)
{
    ...
    if (obj.is_name("Identity")) {
        ...
        case 0:
            return std::make_shared< sampled_function_t >(obj, *p);
        case 2:
            return std::make_shared< exponential_function_t >(obj, *p);
        case 3:
            return std::make_shared< stitching_function_t >(obj, *p,
                                                            recursion + 1);
        case 4:
            return std::make_shared< postscript_function_t >(obj, *p);
        default:
            throw std::runtime_error(format("invalid function type: {}", type));
        }
    }
}
...
void function_t::operator()(const double *src, const double *const end,
                            double *dst) const
{
    return p_->operator()(src, end, dst);
}

size_t function_t::arity() const
{
    return p_->arity();
}

size_t function_t::coarity() const
{
    return p_->coarity();
}

std::string function_t::to_ps() const
{
    return p_->to_ps();
}

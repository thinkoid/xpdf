...
static void initialize (
    Widget requestWidget, Widget newWidget, ArgList args, Cardinal* numArgs);
static void destroy (Widget widget);
static void destroySubtree (XPDFTreeEntry* e);
static void resize (Widget widget);
static void redisplay (Widget widget, XEvent* event, Region region);
static void redisplaySubtree (
    XPDFTreeWidget w, XPDFTreeEntry* e, XEvent* event, Region region);
...
static Boolean setValues (
    Widget oldWidget, Widget requestWidget, Widget newWidget, ArgList args,
    Cardinal* numArgs);
static void setValuesAlmost (
    Widget oldWidget, Widget newWidget, XtWidgetGeometry* request,
    XtWidgetGeometry* reply);
static XtGeometryResult queryGeometry (
    Widget widget, XtWidgetGeometry* request, XtWidgetGeometry* reply);
...
static int layoutSubtree (
    XPDFTreeWidget w, Widget instigator, XPDFTreeEntry* e, Position x,
    Position y, Boolean visible);
static void calcSize (
    Widget widget, Widget instigator, Dimension* totalWidth,
    Dimension* totalHeight);
static void calcSubtreeSize (
    XPDFTreeWidget w, Widget instigator, XPDFTreeEntry* e, Dimension* width,
    Dimension* height);
static Boolean needRelayout (Widget oldWidget, Widget newWidget);
static void
click (Widget widget, XEvent* event, String* params, Cardinal* numParams);
static Boolean findPosition (
    XPDFTreeWidget w, int x, int y, XPDFTreeEntry** e, Boolean* onExpandIcon);
static Boolean findPositionInSubtree (
    XPDFTreeWidget w, int x, int y, XPDFTreeEntry** e, Boolean* onExpandIcon);
...
static XtResource resources[] = {
    ...
    { XmNmarginHeight, XmCMarginHeight, XmRVerticalDimension,
      ...
      XmRImmediate, (XtPointer)0 },
    { XPDFNselectionCallback, XmCCallback, XmRCallback, sizeof (XtCallbackList),
      XtOffsetOf (XPDFTreeRec, tree.selectCallback), XmRImmediate,
      (XtPointer)NULL }
};
...
static XtResource constraints[] = {
    { XPDFNentryParent, XPDFCentryParent, XmRWidget, sizeof (Widget),
      XtOffsetOf (XPDFTreeConstraintRec, tree.entryParent), XmRImmediate,
      (XtPointer)NULL },
    { XPDFNentryExpanded, XPDFCentryExpanded, XmRBoolean, sizeof (Boolean),
      XtOffsetOf (XPDFTreeConstraintRec, tree.entryExpanded), XmRImmediate,
      (XtPointer)False },
    { XPDFNentryPosition, XPDFCentryPosition, XmRInt, sizeof (int),
      XtOffsetOf (XPDFTreeConstraintRec, tree.entryPosition), XmRImmediate,
      (XtPointer)0 }
};

static char defaultTranslations[] = "<Btn1Down>: XPDFTreeClick()";

static XtActionsRec actions[] = { { "XPDFTreeClick", click } };
...
externaldef (xpdftreewidgetclass)
    WidgetClass xpdfTreeWidgetClass = (WidgetClass)&xpdfTreeClassRec;
...
static void initialize (
    ...
    nw->tree.root = NULL;
    nw->tree.redrawY = -1;
    if (cls->treeClass.createGC) { (*cls->treeClass.createGC) (newWidget); }
    else {
        createGC (newWidget);
    }
}
...
static void destroy (Widget widget) {
    ...
    if (cls->treeClass.destroyGC) { (*cls->treeClass.destroyGC) (widget); }
    else {
        destroyGC (widget);
    }
}
...
static void destroySubtree (XPDFTreeEntry* e) {
    if (e->children) { destroySubtree (e->children); }
    if (e->next) { destroySubtree (e->next); }
}

static void resize (Widget widget) {
    XPDFTreeWidgetClass cls = (XPDFTreeWidgetClass)XtClass (widget);

    if (cls->treeClass.layout) { (*cls->treeClass.layout) (widget, NULL); }
    else {
        layout (widget, NULL);
    }
}
...
static void redisplay (Widget widget, XEvent* event, Region region) {
    ...
    if (w->tree.redrawY >= 0) {
        XClearArea (
            XtDisplay ((Widget)w), XtWindow ((Widget)w), 0, w->tree.redrawY,
        ...
        if (c->entryExpanded) {
            ...
            for (child = e->children; child; child = child->next) {
                ...
                XDrawLine (
                    ...
                    w->tree.dottedGC, x - 8, y2, x + 6, y2);
                redisplaySubtree (w, child, event, region);
            }
            XDrawLine (
                XtDisplay ((Widget)w), XtWindow ((Widget)w), w->tree.dottedGC,
                x - 8, y + 2, x - 8, y2);
        }
        else {
...
static void drawExpandedIcon (XPDFTreeWidget w, Position x, Position y) {
    ...
    pts[3].y = y - 2;
    XDrawLines (
        XtDisplay ((Widget)w), XtWindow ((Widget)w), w->tree.plainGC, pts, 4,
        CoordModeOrigin);
}
...
static Boolean setValues (
    ...
    if (relayout) {
        ...
        if (nw->core.width == ow->core.width) { nw->core.width = 0; }
        if (nw->core.height == ow->core.height) { nw->core.height = 0; }
        if (cls->treeClass.calcSize) {
            (*cls->treeClass.calcSize) (
                (Widget)nw, NULL, &nw->core.width, &nw->core.height);
        }
        else {
...
static XtGeometryResult queryGeometry (
    ...
    if (cls->treeClass.calcSize) {
        (*cls->treeClass.calcSize) (
            widget, NULL, &reply->width, &reply->height);
    }
    else {
    ...
    if ((request->request_mode & CWWidth) &&
        (request->request_mode & CWHeight) && request->width == reply->width &&
...
static XtGeometryResult geometryManager (
    ...
    if (cls->treeClass.calcSize) {
        (*cls->treeClass.calcSize) (
            (Widget)w, widget, &parentReq.width, &reply->height);
    }
    else {
...
static void destroyConstraint (Widget widget) { deleteSubtree (widget); }
...
static void deleteSubtree (Widget widget) {
    ...
    if (!c->e) { return; }
    while (c->e->children) { deleteSubtree (c->e->children->widget); }
    if (c->entryParent) {
        deleteChildFromList (
            c->e, &XPDFTreeCPart (c->entryParent)->e->children);
    }
    else {
...
static Boolean constraintSetValues (
    ...
    if (!XtIsManaged (newWidget)) { return False; }
    ...
    if (nc->entryParent != oc->entryParent ||
        ...
        if (oc->entryParent) {
            deleteChildFromList (
                oc->e, &XPDFTreeCPart (oc->entryParent)->e->children);
        }
        else {
        ...
        if (nc->entryParent) {
            insertChildOnList (
                nc->e, &XPDFTreeCPart (nc->entryParent)->e->children);
        }
        else {
...
static void insertChildOnList (XPDFTreeEntry* e, XPDFTreeEntry** listHead) {
    ...
    if (!*listHead ||
        pos < XPDFTreeCPart ((*listHead)->widget)->entryPosition) {
        e->next = *listHead;
        *listHead = e;
    }
    else {
...
static void createGC (Widget widget) {
    ...
    w->tree.plainGC =
        XtGetGC (widget, GCForeground | GCLineWidth | GCLineStyle, &gcValues);

    gcValues.line_style = LineOnOffDash;
    gcValues.dashes = 1;
    gcValues.dash_offset = 0;
    w->tree.dottedGC = XtGetGC (
        widget,
        GCForeground | GCLineWidth | GCLineStyle | GCDashList | GCDashOffset,
        &gcValues);
}
...
static int layoutSubtree (
...
#if XmVERSION > 1
                XmeConfigureObject (
                    ew, x, y, ew->core.width, ew->core.height,
    ...
    for (child = e->children; child; child = child->next) {
        y = layoutSubtree (
            w, instigator, child, x, y, visible && (!c || c->entryExpanded));
    }
...
static void calcSize (
    ...
    if (!*totalWidth) { *totalWidth = w1; }
    if (!*totalHeight) { *totalHeight = (Dimension)h1; }
}
...
static void calcSubtreeSize (
    ...
    if (ew) {
        ...
        if (ew == instigator) {
            ...
            h1 =
                (int)((geom.request_mode & CWHeight) ? geom.height : ew->core.height);
        }
...
click (Widget widget, XEvent* event, String* params, Cardinal* numParams) {
    ...
    if (findPosition (w, bpe->x, bpe->y, &e, &onExpandIcon)) {
        if (onExpandIcon) {
            ...
            XtVaSetValues (
                e->widget, XPDFNentryExpanded, !c->entryExpanded, NULL);
        }
        else {
...
static Boolean findPosition (
    ...
    for (e2 = w->tree.root; e2; e2 = e2->next) {
        *e = e2;
        if (findPositionInSubtree (w, x, y, e, onExpandIcon)) { return True; }
    }
    return False;
}
...
static Boolean findPositionInSubtree (
    ...
    if (x >= child->core.x && x < child->core.x + child->core.width &&
        ...
        *onExpandIcon = False;
        ...
        x >= child->core.x - 16 && x < child->core.x - 4 && y >= y1 - 6 &&
        y < y1 + 6 && (*e)->children) {
        *onExpandIcon = True;
        return True;
    }
...
Widget
XPDFCreateTree (Widget parent, char* name, ArgList argList, Cardinal numArgs) {

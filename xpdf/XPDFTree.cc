...
static void    initialize(Widget requestWidget, Widget newWidget, ArgList args,
                          Cardinal *numArgs);
static void    destroy(Widget widget);
static void    destroySubtree(XPDFTreeEntry *e);
static void    resize(Widget widget);
static void    redisplay(Widget widget, XEvent *event, Region region);
static void    redisplaySubtree(XPDFTreeWidget w, XPDFTreeEntry *e, XEvent *event,
                                Region region);
...
static Boolean setValues(Widget oldWidget, Widget requestWidget, Widget newWidget,
                         ArgList args, Cardinal *numArgs);
static void    setValuesAlmost(Widget oldWidget, Widget newWidget,
                               XtWidgetGeometry *request, XtWidgetGeometry *reply);
static XtGeometryResult queryGeometry(Widget widget, XtWidgetGeometry *request,
                                      XtWidgetGeometry *reply);
static XtGeometryResult geometryManager(Widget widget, XtWidgetGeometry *request,
                                        XtWidgetGeometry *reply);
static void             changeManaged(Widget widget);
static void initConstraint(Widget requestWidget, Widget newWidget, ArgList args,
                           Cardinal *numArgs);
...
static int  layoutSubtree(XPDFTreeWidget w, Widget instigator, XPDFTreeEntry *e,
                          Position x, Position y, Boolean visible);
static void calcSize(Widget widget, Widget instigator, Dimension *totalWidth,
                     Dimension *totalHeight);
static void calcSubtreeSize(XPDFTreeWidget w, Widget instigator, XPDFTreeEntry *e,
                            Dimension *width, Dimension *height);
static Boolean needRelayout(Widget oldWidget, Widget newWidget);
static void    click(Widget widget, XEvent *event, String *params,
                     Cardinal *numParams);
static Boolean findPosition(XPDFTreeWidget w, int x, int y, XPDFTreeEntry **e,
...
static XtResource resources[] = {
    { XmNmarginWidth, XmCMarginWidth, XmRHorizontalDimension, sizeof(Dimension),
      XtOffsetOf(XPDFTreeRec, tree.marginWidth), XmRImmediate, (XtPointer)0 },
    { XmNmarginHeight, XmCMarginHeight, XmRVerticalDimension, sizeof(Dimension),
      XtOffsetOf(XPDFTreeRec, tree.marginHeight), XmRImmediate, (XtPointer)0 },
...
static void redisplay(Widget widget, XEvent *event, Region region)
{
    ...
    if (w->tree.redrawY >= 0) {
        XClearArea(XtDisplay((Widget)w), XtWindow((Widget)w), 0, w->tree.redrawY,
...
static void drawExpandedIcon(XPDFTreeWidget w, Position x, Position y)
{
    ...
    pts[3].x = x - 4;
    pts[3].y = y - 2;
    XDrawLines(XtDisplay((Widget)w), XtWindow((Widget)w), w->tree.plainGC, pts, 4,
               CoordModeOrigin);
}
                         ...
                         ArgList args, Cardinal *numArgs)
{
    ...
    if (relayout) {
        ...
        if (cls->treeClass.calcSize) {
            (*cls->treeClass.calcSize)((Widget)nw, NULL, &nw->core.width,
                                       &nw->core.height);
        } else {
                                      ...
                                      XtWidgetGeometry *reply)
{
    ...
    if (reply->width == XtWidth(widget) && reply->height == XtHeight(widget)) {
        return XtGeometryNo;
    }
                                        ...
                                        XtWidgetGeometry *reply)
{
    ...
    parentReq.height = 0;
    if (cls->treeClass.calcSize) {
        (*cls->treeClass.calcSize)((Widget)w, widget, &parentReq.width,
                                   &reply->height);
    } else {
                                   ...
                                   Cardinal *numArgs)
{
    ...
    if (nc->entryParent != oc->entryParent ||
        ...
        if (oc->entryParent) {
            deleteChildFromList(oc->e,
                                &XPDFTreeCPart(oc->entryParent)->e->children);
        } else {
            deleteChildFromList(oc->e, &w->tree.root);
        }
        if (nc->entryParent) {
            insertChildOnList(nc->e,
                              &XPDFTreeCPart(nc->entryParent)->e->children);
        } else {
                                     ...
                                     XPDFTreeEntry **e, Boolean *onExpandIcon)
{
    ...
    if (x >= child->core.x && x < child->core.x + child->core.width &&
        ...
        *onExpandIcon = False;
        return True;
    } else if (x >= child->core.x - 16 && x < child->core.x - 4 && y >= y1 - 6 &&
               y < y1 + 6 && (*e)->children) {
        *onExpandIcon = True;
        return True;
    }
...
Widget XPDFCreateTree(Widget parent, char *name, ArgList argList,
                      Cardinal numArgs)
{

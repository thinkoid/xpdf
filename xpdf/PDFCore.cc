...
void PDFCore::segment()
{
    ...
    BOOST_SCOPE_EXIT(this_) { this_->setBusyCursor(false); }
                 ...
                 bool incrementalUpdate)
{
    ...
    out = new CoreOutputDev(colorModeA, bitmapRowPadA, reverseVideoA, paperColorA,
                          ...
                          bool scrollToTop, bool addToHist)
{
    ...
    update(topPageA, scrollXA, scrollYA, zoomA, rotateA, true, addToHist, true);
}
                          ...
                          bool addToHist)
{
    ...
    case destFitBV:
        ...
        break;
    case destFitR:
        zoomToRect(topPageA, dest->getLeft(), dest->getTop(), dest->getRight(),
                   dest->getBottom());
        break;
    }
}

void PDFCore::update(int topPageA, int scrollXA, int scrollYA, double zoomA,
                     int rotateA, bool force, bool addToHist, bool adjustScrollX)
{
    ...
    if (force || pages->getLength() == 0 ||
        (!continuousMode && topPageA != topPage) || fabs(zoomA - zoom) > 1e-8 ||
        fabs(dpiA - dpi) > 1e-8 || rotateA != rotate) {
        needUpdate = true;
...
void PDFCore::addPage(int pg, int rot)
{
    ...
    for (i = 0;
         i < pages->getLength() && pg > ((PDFCorePage *)pages->get(i))->page; ++i)
        ;
    pages->insert(i, page);
}
...
void PDFCore::needTile(PDFCorePage *page, int x, int y)
{
    ...
    doc->displayPageSlice(out, page->page, dpi, dpi, rotate, false, true, false,
                          x, y, sliceW, sliceH);
    ...
    if (!page->text) {
        ...
        doc->displayPage(&textOut, page->page, dpi, dpi, rotate, false, true,
                         false);
...
bool PDFCore::goForward()
{
    ...
    update(pg, scrollX, continuousMode ? -1 : scrollY, zoom, rotate, false, false,
           true);
    return true;
}
...
void PDFCore::scrollToRightEdge()
{
    ...
    page = (PDFCorePage *)pages->get(0);
    update(topPage, page->w - drawAreaWidth, scrollY, zoom, rotate, false, false,
           false);
}
...
void PDFCore::zoomToRect(int pg, double ulx, double uly, double lrx, double lry)
{
...
void PDFCore::zoomCentered(double zoomA)
{
    ...
    if (continuousMode) {
        ...
        sy = sy +
             (int)((scrollY - pageY[topPage - 1] + drawAreaHeight / 2) *
                   (dpi1 / dpi)) +
...
void PDFCore::setSelection(int newSelectPage, int newSelectULX, int newSelectULY,
                           int newSelectLRX, int newSelectLRY)
{
    ...
    if (newHaveSel) {
        xorRectangle(newSelectPage, newSelectULX, newSelectULY, newSelectLRX,
    ...
    if (needRedraw) {
        if (!haveSel) {
            ...
            x1 = newSelectLRX;
            y1 = newSelectLRY;
            redrawWindow(page->xDest + x0, page->yDest + y0, x1 - x0 + 1,
                         y1 - y0 + 1, false);
        } else if (!newHaveSel) {
...
void PDFCore::moveSelection(int pg, int x, int y)
{
    ...
    // redraw the selection
    setSelection(selectPage, newSelectULX, newSelectULY, newSelectLRX,
                 newSelectLRY);
}
...
bool PDFCore::getSelection(int *pg, double *ulx, double *uly, double *lrx,
                           double *lry)
{
    if (selectULX == selectLRX || selectULY == selectLRY) {
        return false;
...
GString *PDFCore::extractText(int pg, double xMin, double yMin, double xMax,
                              double yMax)
{
    PDFCorePage *page;
    int          x0, y0, x1, y1, t;
    ...
    if ((page = findPage(pg))) {
        ...
        s = page->text->getText(
            xpdf::bbox_t{ double(x0), double(y0), double(x1), double(y1) });
    } else {
...
bool PDFCore::find(char *s, bool caseSensitive, bool next, bool backward,
...
bool PDFCore::findU(Unicode *u, int len, bool caseSensitive, bool next,
                    bool backward, bool wholeWord, bool onePageOnly)
{
    ...
    if (page->text->findText(u, len, startAtTop, true, startAtLast, false,
                             caseSensitive, backward, wholeWord, box)) {
        goto found;
    }
    ...
    if (!onePageOnly) {
        ...
        for (pg = backward ? pg - 1 : pg + 1;
            ...
            if (textOut.findText(u, len, true, true, false, false, caseSensitive,
...
foundPage:
    update(pg, scrollX, continuousMode ? -1 : 0, zoom, rotate, false, true, true);

    page = findPage(pg);

    if (!page->text->findText(u, len, true, true, false, false, caseSensitive,
                              backward, wholeWord, box)) {
...
void PDFCore::cvtUserToDev(int pg, double xu, double yu, int *xd, int *yd)
{
    ...
    if (tile) {
        *xd = (int)(tile->xMin + tile->ctm[0] * xu + tile->ctm[2] * yu +
                    tile->ctm[4] + 0.5);
        *yd = (int)(tile->yMin + tile->ctm[1] * xu + tile->ctm[3] * yu +
                    tile->ctm[5] + 0.5);
    } else {
                        ...
                        bool composited)
{
    ...
    core->clippedRedrawRect(core->curTile, x0, y0, core->curTile->xDest + x0,
                            core->curTile->yDest + y0, x1 - x0 + 1, y1 - y0 + 1,
                            0, 0, core->drawAreaWidth, core->drawAreaHeight, true,
                            composited);
}

void PDFCore::redrawWindow(int x, int y, int width, int height, bool needUpdate)
{
    PDFCorePage *page;
    PDFCoreTile *tile;
    ...
    for (i = 0; i < pages->getLength(); ++i) {
        ...
        for (j = 0; j < page->tiles->getLength(); ++j) {
            ...
            if (tile->edges & pdfCoreTileTopEdge) {
                ...
                clippedRedrawRect(NULL, 0, 0, xDest, 0, w, tile->yDest, x, y,
                                  width, height, false);
            }
            if (tile->edges & pdfCoreTileBottomEdge) {
                ...
                clippedRedrawRect(NULL, 0, 0, xDest, yDest, w,
                                  drawAreaHeight - yDest, x, y, width, height,
                                  false);
                ...
                yDest = tile->yDest + (tile->yMax - tile->yMin);
                clippedRedrawRect(NULL, 0, 0, xDest, yDest, w,
                                  ((PDFCorePage *)pages->get(i + 1))->yDest -
                                      yDest,
                                  x, y, width, height, false);
            }
            if (tile->edges & pdfCoreTileLeftEdge) {
                clippedRedrawRect(NULL, 0, 0, 0, tile->yDest, tile->xDest,
                                  tile->yMax - tile->yMin, x, y, width, height,
                                  false);
            }
            if (tile->edges & pdfCoreTileRightEdge) {
                xDest = tile->xDest + (tile->xMax - tile->xMin);
                clippedRedrawRect(NULL, 0, 0, xDest, tile->yDest,
...
void PDFCore::updateTileData(PDFCoreTile *tileA, int xSrc, int ySrc, int width,
                             int height, bool composited)
{
}

void PDFCore::clippedRedrawRect(PDFCoreTile *tile, int xSrc, int ySrc, int xDest,
                                int yDest, int width, int height, int xClip,
                                int yClip, int wClip, int hClip, bool needUpdate,
                                bool composited)
{
    if (tile && needUpdate) {

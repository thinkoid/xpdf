...
static SubstFontInfo substFonts[16] = { { 0.833 }, { 0.833 }, { 0.889 },
                                        { 0.889 }, { 0.788 }, { 0.722 },
                                        { 0.833 }, { 0.778 }, { 0.600 },
                                        { 0.600 }, { 0.600 }, { 0.600 } };
...
static inline int
pr (int (*writeFunc) (void* stream, const char* data, int size), void* stream,
    const char* data) {
    return writeFunc (stream, data, (int)strlen (data));
}

static int
pf (int (*writeFunc) (void* stream, const char* data, int size), void* stream,
    const char* fmt, ...) {
    va_list args;
    GString* s;
    int ret;
...
int HTMLGen::convertPage (
    ...
    pf (writeHTML, htmlStream,
        "<img id=\"background\" style=\"position:absolute; left:0px; "
        "top:0px;\" width=\"{0:d}\" height=\"{1:d}\" src=\"{2:s}\">\n",
            ...
            for (lineIdx = 0; lineIdx < lines->getLength (); ++lineIdx) {
                ...
                for (wordIdx = 0; wordIdx < words->getLength (); ++wordIdx) {
                    ...
                    if (!word0 ||
                        ...
                        subSuper1 != subSuper0 || r1 != r0 || g1 != g0 ||
                        b1 != b0) {
                        if (word0) { s->append ("</span>"); }
                        for (i = 0; i < fonts->getLength (); ++i) {
                            if (word1->getFontInfo () ==
                                (TextFontInfo*)fonts->get (i)) {
                                break;
                            }
                        }
                        s->appendf (
                            "<span id=\"f{0:d}\" "
                            "style=\"font-size:{1:d}px;vertical-align:{2:s};"
                            "color:#{3:02x}{4:02x}{5:02x};\">",
                ...
                pf (writeHTML, htmlStream,
                    ...
                    "left:{0:d}px; top:{1:d}px;\">{2:t}</div>\n",
...
GString* HTMLGen::getFontDefn (TextFontInfo* font, double* scale) {
    ...
    *scale = 1;
    if ((s = font->getMWidth ())) {
        i = (fixedWidth ? 8 : serif ? 4 : 0) + (bold ? 2 : 0) +
            (italic ? 1 : 0);
        if (s < substFonts[i].mWidth) { *scale = s / substFonts[i].mWidth; }
    }

    // generate the CSS markup
    return GString::format (
        "font-family:{0:s}; font-weight:{1:s}; font-style:{2:s};",
        fixedWidth ? "monospace" : serif ? "serif" : "sans-serif",
        bold ? "bold" : "normal", italic ? "italic" : "normal");
}

...
static void setupPNG (
    png_structp* png, png_infop* pngInfo, FILE* f, int bitDepth, int colorType,
    SplashBitmap* bitmap);
...
int main (int argc, char* argv[]) {
    ...
    for (pg = firstPage; pg <= lastPage; ++pg) {
        ...
        if (mono) {
            ...
            setupPNG (
                &png, &pngInfo, f, 1, PNG_COLOR_TYPE_GRAY,
                splashOut->getBitmap ());
            ...
            setupPNG (
                &png, &pngInfo, f, 8, PNG_COLOR_TYPE_GRAY,
                splashOut->getBitmap ());
            writePNGData (png, splashOut->getBitmap ());
            finishPNG (&png, &pngInfo);
            fclose (f);
            ...
            setupPNG (
                &png, &pngInfo, f, 8, PNG_COLOR_TYPE_RGB,
                splashOut->getBitmap ());
            writePNGData (png, splashOut->getBitmap ());
            finishPNG (&png, &pngInfo);
            fclose (f);
...
static void setupPNG (
    ...
    png_init_io (*png, f);
    png_set_IHDR (
        *png, *pngInfo, bitmap->getWidth (), bitmap->getHeight (), bitDepth,

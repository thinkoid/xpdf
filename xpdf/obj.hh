...
template< typename T, typename U, typename Enable = void > struct cast_t
{
...
using is_convertible_t =
    typename std::enable_if< std::is_convertible< T, U >::value >::type;

template< typename T, typename U > struct cast_t< T, U, is_convertible_t< T, U > >
{
...
struct obj_t
{
    ...
    obj_t(eof_t) noexcept
        : var_(eof_t{})
    {
    }
    obj_t(err_t) noexcept
        : var_(error_t{})
    {
    }

    obj_t(bool arg) noexcept
        : var_(arg)
    {
    }
    obj_t(int arg) noexcept
        : var_(arg)
    {
    }
    obj_t(double arg) noexcept
        : var_(arg)
    {
    }
    ...
    obj_t(const name_t &arg)
        : var_(arg)
    {
    }
    obj_t(const cmd_t &arg)
        : var_(arg)
    {
    }

    obj_t(const ref_t &arg) noexcept
        : var_(arg)
    {
    }
    ...
    bool is_name(const std::string &s) const { return is_name(s.c_str()); }
    ...
    // Accessors:
    //
    template< typename T > T &as() { return std::get< T >(var_); }

    template< typename T > const T &as() const { return std::get< T >(var_); }
...
#define CAST(T) detail::cast_t< T, U >{}(as< T >());
        ...
        case 5:
            return CAST(double);
        case 6:
            return CAST(std::shared_ptr< GString >);
        case 7:
            return CAST(name_t);
        case 8:
            return CAST(cmd_t);
        case 9:
            return CAST(ref_t);
        case 10:
            return CAST(std::shared_ptr< Array >);
        case 11:
...
#undef CAST
    ...
    const char *as_name() const { return as< name_t >().c_str(); }

    const char *as_cmd() const { return as< cmd_t >().c_str(); }

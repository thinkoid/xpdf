...
PDFDoc::PDFDoc(GString *fileNameA, GString *ownerPassword, GString *userPassword,
               PDFCore *coreA)
{
    Object   obj;
    GString *fileName1;
...
bool PDFDoc::setup2(GString *ownerPassword, GString *userPassword,
                    bool repairXRef)
{
    // read xref table
    ...
    if ((encrypted = encrypt.is_dict())) {
        if ((secHdlr = SecurityHandler::make(this, &encrypt))) {
            if (secHdlr->isUnencrypted()) {
                ...
                xref->setEncryption(
                    secHdlr->getPermissionFlags(), secHdlr->getOwnerPasswordOk(),
...
void PDFDoc::displayPageSlice(OutputDev *out, int page, double hDPI, double vDPI,
                              int rotate, bool useMediaBox, bool crop,
                              bool printing, int sliceX, int sliceY, int sliceW,
                              int   sliceH, bool (*abortCheckCbk)(void *data),
                              void *abortCheckCbkData)
{
    catalog->getPage(page)->displaySlice(out, hDPI, vDPI, rotate, useMediaBox,
                                         crop, sliceX, sliceY, sliceW, sliceH,
                                         printing, abortCheckCbk,
                                         abortCheckCbkData);
}
...
bool PDFDoc::isLinearized()
{
    ...
    parser = new Parser(
        xref, new Lexer(str->makeSubStream(str->getStart(), false, 0, &obj1)),
        true);
    ...
    parser->getObj(&obj3);
    parser->getObj(&obj4);
    if (obj1.is_int() && obj2.is_int() && obj3.is_cmd("obj") && obj4.is_dict()) {
        obj5 = resolve(obj4.as_dict()["Linearized"]);
        if (obj5.is_num() && obj5.as_num() > 0) {
            lin = true;
        }
    }
    delete parser;

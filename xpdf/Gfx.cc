...
Operator Gfx::opTab[] = {
    ...
    { "SCN",
      -33,
      { tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN,
        tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN,
        tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN,
        tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN,
        tchkSCN, tchkSCN, tchkSCN, tchkSCN, tchkSCN },
    ...
    { "Tm",
      6,
      { tchkNum, tchkNum, tchkNum, tchkNum, tchkNum, tchkNum },
      &Gfx::opSetTextMatrix },
...
Gfx::Gfx (
    ...
    PDFRectangle* cropBox, GBool (*abortCheckCbkA) (void* data),
...
void Gfx::opConcat (Object args[], int numArgs) {
    ...
    out->updateCTM (
        state, args[0].getNum (), args[1].getNum (), args[2].getNum (),
        args[3].getNum (), args[4].getNum (), args[5].getNum ());
    fontChanged = gTrue;
}
...
void Gfx::opSetExtGState (Object args[], int numArgs) {
    ...
    if (!res->lookupGState (args[0].getName (), &obj1)) { return; }
    if (!obj1.isDict ()) {
        error (
    ...
    if (obj1.dictLookup ("LW", &obj2)->isNum ()) { opSetLineWidth (&obj2, 1); }
    obj2.free ();
    ...
    if (!obj1.dictLookup ("SMask", &obj2)->isNull ()) {
        if (obj2.isName ("None")) { out->clearSoftMask (state); }
        else if (obj2.isDict ()) {
            ...
            if ((haveBackdropColor =
                     obj2.dictLookup ("BC", &obj3)->isArray ())) {
...
void Gfx::doSoftMask (
    Object* str, Object* strRef, GBool alpha, GfxColorSpace* blendingColorSpace,
    GBool isolated, GBool knockout, Function* transferFunc,
    GfxColor* backdropColor) {
    Dict *dict, *resDict;
    double m[6], bbox[4];
    Object obj1, obj2;
    ...
    ++formDepth;
    drawForm (
        strRef, resDict, m, bbox, gTrue, gTrue, blendingColorSpace, isolated,
...
void Gfx::opSetFillColorSpace (Object args[], int numArgs) {
    ...
    if (obj.isNull ()) { colorSpace = GfxColorSpace::parse (&args[0]); }
    else {
...
void Gfx::doPatternFill (GBool eoFill) {
    ...
    case 1:
        doTilingPatternFill (
            (GfxTilingPattern*)pattern, gFalse, eoFill, gFalse);
        break;
    case 2:
        doShadingPatternFill (
            (GfxShadingPattern*)pattern, gFalse, eoFill, gFalse);
        break;
    default:
        error (
...
void Gfx::doPatternStroke () {
    ...
    case 2:
        doShadingPatternFill (
            (GfxShadingPattern*)pattern, gTrue, gFalse, gFalse);
        break;
    default:
        error (
...
void Gfx::doPatternText () {
    ...
    case 2:
        doShadingPatternFill (
            (GfxShadingPattern*)pattern, gFalse, gFalse, gTrue);
        break;
    default:
        error (
            errSyntaxError, getPos (), "Unknown pattern type ({0:d}) in fill",
...
void Gfx::doPatternImageMask (
    Object* ref, Stream* str, int width, int height, GBool invert,
...
void Gfx::doTilingPatternFill (
    GfxTilingPattern* tPat, GBool stroke, GBool eoFill, GBool text) {
...
void Gfx::doShadingPatternFill (
    GfxShadingPattern* sPat, GBool stroke, GBool eoFill, GBool text) {
    GfxShading* shading;
    GfxState* savedState;
    double *ctm, *btm, *ptm;
...
void Gfx::opShFill (Object args[], int numArgs) {
    ...
    if (!(shading = res->lookupShading (args[0].getName ()))) { return; }
    ...
    if (ocState) {
        ...
        for (i = 0; i < a->getLength (); ++i) {
            ...
            if (obj.isNum ()) {
                if (wMode) {
                    state->textShift (
                        0, -obj.getNum () * 0.001 * state->getFontSize ());
                }
                else {
                    state->textShift (
                        -obj.getNum () * 0.001 * state->getFontSize () *
    ...
    if (font->getType () == fontType3 && out->interpretType3Chars ()) {
        ...
        while (len > 0) {
            ...
            if (!out->beginType3Char (
                ...
                if (charProc.isStream ()) { display (&charProcRef, gFalse); }
                else {
                    error (
                        errSyntaxError, getPos (),
...
void Gfx::opXObject (Object args[], int numArgs) {
    ...
    if (opiDict.isDict ()) { out->opiBegin (state, opiDict.getDict ()); }
#endif
    ...
    obj2.free ();
#if OPI_SUPPORT
    if (opiDict.isDict ()) { out->opiEnd (state, opiDict.getDict ()); }
    opiDict.free ();
#endif
    obj1.free ();
}
...
void Gfx::doImage (Object* ref, Stream* str, GBool inlineImg) {
    ...
    if (mask) {
        ...
        if (!obj1.isNull ()) { colorSpace = GfxColorSpace::parse (&obj1); }
        ...
        if (smaskObj.isStream ()) {
            ...
            if (!obj1.isBool () || !obj1.getBool ()) { goto err2; }
            obj1.free ();
            maskInvert = gFalse;
            maskDict->lookup ("Decode", &obj1);
            if (obj1.isNull ()) {
...
void Gfx::doForm (Object* strRef, Object* str) {
    ...
    if (doc->getOptionalContent ()->evalOCObject (&obj1, &oc) && !oc) {
        obj1.free ();
        if (out->needCharCount ()) { ocState = gFalse; }
        else {
            return;
        }
    }
...
void Gfx::drawForm (
    ...
    GBool transpGroup, GBool softMask, GfxColorSpace* blendingColorSpace,
    GBool isolated, GBool knockout, GBool alpha, Function* transferFunc,
    GfxColor* backdropColor) {
    Parser* oldParser;
    GfxState* savedState;

...
private:
    void readSegments();
    bool readSymbolDictSeg(unsigned segNum, unsigned length, unsigned *refSegs,
                           unsigned nRefSegs);
    void readTextRegionSeg(unsigned segNum, bool imm, bool lossless,
                           unsigned length, unsigned *refSegs, unsigned nRefSegs);
    JBIG2Bitmap *readTextRegion(
        ...
        unsigned symCodeLen, JBIG2Bitmap **syms, unsigned defPixel,
        unsigned combOp, unsigned transposed, unsigned refCorner, int sOffset,
    ...
    void readHalftoneRegionSeg(unsigned segNum, bool imm, bool lossless,
                               unsigned length, unsigned *refSegs,
                               unsigned nRefSegs);
    void readGenericRegionSeg(unsigned segNum, bool imm, bool lossless,
                              unsigned length);
    void mmrAddPixels(int a1, int blackPixels, int *codingLine, int *a0i, int w);
    void mmrAddPixelsNeg(int a1, int blackPixels, int *codingLine, int *a0i,
                         int w);
    JBIG2Bitmap *readGenericBitmap(bool mmr, int w, int h, int templ, bool tpgdOn,
                                   bool useSkip, JBIG2Bitmap *skip, int *atx,
                                   int *aty, int mmrDataLength);
    void readGenericRefinementRegionSeg(unsigned segNum, bool imm, bool lossless,
                                        unsigned length, unsigned *refSegs,
                                        unsigned nRefSegs);
    JBIG2Bitmap *readGenericRefinementRegion(int w, int h, int templ, bool tpgrOn,
                                             JBIG2Bitmap *refBitmap, int refDX,

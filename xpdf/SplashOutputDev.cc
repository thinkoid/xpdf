...
static void splashOutBlendMultiply (
...
static void splashOutBlendScreen (
    SplashColorPtr src, SplashColorPtr dest, SplashColorPtr blend,
    SplashColorMode cm) {
    int i;
...
static void splashOutBlendOverlay (
    SplashColorPtr src, SplashColorPtr dest, SplashColorPtr blend,
    SplashColorMode cm) {
    int i;
...
static void splashOutBlendDarken (
    SplashColorPtr src, SplashColorPtr dest, SplashColorPtr blend,
    SplashColorMode cm) {
    int i;
...
static void splashOutBlendLighten (
    SplashColorPtr src, SplashColorPtr dest, SplashColorPtr blend,
    SplashColorMode cm) {
    int i;
...
static void splashOutBlendColorDodge (
    SplashColorPtr src, SplashColorPtr dest, SplashColorPtr blend,
    SplashColorMode cm) {
    int i, x;

    for (i = 0; i < splashColorModeNComps[cm]; ++i) {
        if (dest[i] == 0) { blend[i] = 0; }
        else if (src[i] == 255) {
            blend[i] = 255;
        }
        else {
...
static void splashOutBlendColorBurn (
    SplashColorPtr src, SplashColorPtr dest, SplashColorPtr blend,
    SplashColorMode cm) {
    int i, x;

    for (i = 0; i < splashColorModeNComps[cm]; ++i) {
        if (dest[i] == 255) { blend[i] = 255; }
        else if (src[i] == 0) {
            blend[i] = 0;
        }
        else {
...
static void splashOutBlendHardLight (
    SplashColorPtr src, SplashColorPtr dest, SplashColorPtr blend,
    SplashColorMode cm) {
    int i;
...
static void splashOutBlendSoftLight (
    SplashColorPtr src, SplashColorPtr dest, SplashColorPtr blend,
    SplashColorMode cm) {
    int i, x;
    ...
    for (i = 0; i < splashColorModeNComps[cm]; ++i) {
        if (src[i] < 0x80) {
            blend[i] = dest[i] - (255 - 2 * src[i]) * dest[i] *
                                     (255 - dest[i]) / (255 * 255);
        }
        else {
            if (dest[i] < 0x40) {
...
static void splashOutBlendDifference (
    SplashColorPtr src, SplashColorPtr dest, SplashColorPtr blend,
    SplashColorMode cm) {
    int i;
...
static void clipColor (
    int rIn, int gIn, int bIn, Guchar* rOut, Guchar* gOut, Guchar* bOut) {
    int lum, rgbMin, rgbMax;

    lum = getLum (rIn, gIn, bIn);
    rgbMin = rgbMax = rIn;
    if (gIn < rgbMin) { rgbMin = gIn; }
    else if (gIn > rgbMax) {
        rgbMax = gIn;
    }
    if (bIn < rgbMin) { rgbMin = bIn; }
    else if (bIn > rgbMax) {
        rgbMax = bIn;
    }
    if (rgbMin < 0) {
...
static void setLum (
    Guchar rIn, Guchar gIn, Guchar bIn, int lum, Guchar* rOut, Guchar* gOut,
    Guchar* bOut) {
    int d;
...
static void setSat (
    Guchar rIn, Guchar gIn, Guchar bIn, int sat, Guchar* rOut, Guchar* gOut,
    Guchar* bOut) {
...
static void splashOutBlendHue (
    ...
    case splashModeMono8: blend[0] = dest[0]; break;
    case splashModeRGB8:
    case splashModeBGR8:
        setSat (
            src[0], src[1], src[2], getSat (dest[0], dest[1], dest[2]), &r0,
            &g0, &b0);
        setLum (
            r0, g0, b0, getLum (dest[0], dest[1], dest[2]), &blend[0],
            &blend[1], &blend[2]);
        break;
#if SPLASH_CMYK
...
static void splashOutBlendSaturation (
    SplashColorPtr src, SplashColorPtr dest, SplashColorPtr blend,
    SplashColorMode cm) {
    Guchar r0, g0, b0;
#if SPLASH_CMYK
    ...
    case splashModeBGR8:
        setSat (
            dest[0], dest[1], dest[2], getSat (src[0], src[1], src[2]), &r0,
            &g0, &b0);
        setLum (
            r0, g0, b0, getLum (dest[0], dest[1], dest[2]), &blend[0],
            &blend[1], &blend[2]);
...
static void splashOutBlendColor (
    SplashColorPtr src, SplashColorPtr dest, SplashColorPtr blend,
    SplashColorMode cm) {
#if SPLASH_CMYK
    Guchar r, g, b;
    ...
    case splashModeBGR8:
        setLum (
            src[0], src[1], src[2], getLum (dest[0], dest[1], dest[2]),
    ...
    case splashModeCMYK8:
        // NB: inputs have already been converted to additive mode
        setLum (
            src[0], src[1], src[2], getLum (dest[0], dest[1], dest[2]), &r, &g,
            &b);
        blend[0] = r;
        blend[1] = g;
        blend[2] = b;
...
static void splashOutBlendLuminosity (
    SplashColorPtr src, SplashColorPtr dest, SplashColorPtr blend,
    SplashColorMode cm) {
#if SPLASH_CMYK
    Guchar r, g, b;
    ...
    case splashModeBGR8:
        setLum (
            dest[0], dest[1], dest[2], getLum (src[0], src[1], src[2]),
            &blend[0], &blend[1], &blend[2]);
        break;
#if SPLASH_CMYK
    case splashModeCMYK8:
        // NB: inputs have already been converted to additive mode
        setLum (
            dest[0], dest[1], dest[2], getLum (src[0], src[1], src[2]), &r, &g,
            &b);
        blend[0] = r;
        blend[1] = g;
        blend[2] = b;
...
public:
    T3FontCache (
        Ref* fontID, double m11A, double m12A, double m21A, double m22A,
        int glyphXA, int glyphYA, int glyphWA, int glyphHA, GBool aa,
        GBool validBBoxA);
    ~T3FontCache ();
    GBool
    matches (Ref* idA, double m11A, double m12A, double m21A, double m22A) {
...
T3FontCache::T3FontCache (
    Ref* fontIDA, double m11A, double m12A, double m21A, double m22A,
    int glyphXA, int glyphYA, int glyphWA, int glyphHA, GBool validBBoxA,
    GBool aa) {
    int i;
...
SplashOutputDev::SplashOutputDev (
    ...
    SplashColorPtr paperColorA, GBool bitmapTopDownA, GBool allowAntialiasA) {
    ...
    bitmap = new SplashBitmap (
        1, 1, bitmapRowPad, colorMode, colorMode != splashModeMono1,
        bitmapTopDown);
...
SplashOutputDev::~SplashOutputDev () {
    ...
    for (i = 0; i < nT3Fonts; ++i) { delete t3FontCache[i]; }
...
void SplashOutputDev::startPage (int pageNum, GfxState* state) {
    ...
    if (!bitmap || w != bitmap->getWidth () || h != bitmap->getHeight ()) {
        ...
        bitmap = new SplashBitmap (
            w, h, bitmapRowPad, colorMode, colorMode != splashModeMono1,
            bitmapTopDown);
    }
    splash = new Splash (bitmap, vectorAntialias, &screenParams);
    splash->setMinLineWidth (globalParams->getMinLineWidth ());
    ...
    case splashModeRGB8:
    case splashModeBGR8: color[0] = color[1] = color[2] = 0; break;
#if SPLASH_CMYK
    case splashModeCMYK8: color[0] = color[1] = color[2] = color[3] = 0; break;
#endif
    }
...
void SplashOutputDev::updateCTM (
...
void SplashOutputDev::setOverprintMask (
    GfxColorSpace* colorSpace, GBool overprintFlag, int overprintMode,
...
void SplashOutputDev::updateFont (GfxState* state) { needFontUpdate = gTrue; }
...
void SplashOutputDev::doUpdateFont (GfxState* state) {
    ...
    state->textTransformDelta (
        state->getFontSize (), state->getFontSize (), &fsx, &fsy);
    ...
    if ((fontFile = fontEngine->getFontFile (id))) { delete id; }
    else {
        fontNum = 0;
...
void SplashOutputDev::stroke (GfxState* state) {
    ...
    if (state->getStrokeColorSpace ()->isNonMarking ()) { return; }
    setOverprintMask (
        state->getStrokeColorSpace (), state->getStrokeOverprint (),
...
void SplashOutputDev::fill (GfxState* state) {
    ...
    setOverprintMask (
        state->getFillColorSpace (), state->getFillOverprint (),
...
void SplashOutputDev::tilingPatternFill (
    ...
    state->transform (
        bbox[0] * mat[0] + bbox[1] * mat[2] + mat[4],
        bbox[0] * mat[1] + bbox[1] * mat[3] + mat[5], &tx, &ty);
    tileXMin = tileXMax = tx;
    tileYMin = tileYMax = ty;
    state->transform (
        bbox[2] * mat[0] + bbox[1] * mat[2] + mat[4],
        bbox[2] * mat[1] + bbox[1] * mat[3] + mat[5], &tx, &ty);
    if (tx < tileXMin) { tileXMin = tx; }
    else if (tx > tileXMax) {
        tileXMax = tx;
    }
    if (ty < tileYMin) { tileYMin = ty; }
    else if (ty > tileYMax) {
        tileYMax = ty;
    }
    state->transform (
        bbox[2] * mat[0] + bbox[3] * mat[2] + mat[4],
    ...
    state->transform (
        bbox[0] * mat[0] + bbox[3] * mat[2] + mat[4],
    ...
    if (tileXMin == tileXMax || tileYMin == tileYMax) { return; }
    ...
    origBitmap = bitmap;
    origSplash = splash;
    bitmap = tileBitmap = new SplashBitmap (
        tileW, tileH, bitmapRowPad, colorMode, gTrue, bitmapTopDown);
    splash = new Splash (bitmap, vectorAntialias, origSplash->getScreen ());
    splash->setMinLineWidth (globalParams->getMinLineWidth ());
    splash->setStrokeAdjust (globalParams->getStrokeAdjust ());
    for (i = 0; i < splashMaxColorComps; ++i) { color[i] = 0; }
    splash->clear (color);
    ++nestCount;
    ...
    for (y = y0; y < y1; ++y) {
        for (x = x0; x < x1; ++x) {
            ...
            splash->composite (
                tileBitmap, 0, 0, xx, yy, tileW, tileH, gFalse, gFalse);
        }
    }
...
SplashPath* SplashOutputDev::convertPath (
    GfxState* state, GfxPath* path, GBool dropEmptySubpaths) {
    SplashPath* sPath;
    GfxSubpath* subpath;
    int n, i, j;
    ...
    for (i = 0; i < path->getNumSubpaths (); ++i) {
        ...
        if (subpath->getNumPoints () > n) {
            sPath->moveTo (
                (SplashCoord)subpath->getX (0), (SplashCoord)subpath->getY (0));
            j = 1;
            while (j < subpath->getNumPoints ()) {
                if (subpath->getCurve (j)) {
                    sPath->curveTo (
                        (SplashCoord)subpath->getX (j),
            ...
            if (subpath->isClosed ()) { sPath->close (); }
        }
    }
    return sPath;
}
...
void SplashOutputDev::drawChar (
    ...
    if (skipHorizText || skipRotatedText) {
        ...
        horiz =
            m[0] > 0 && fabs (m[1]) < 0.001 && fabs (m[2]) < 0.001 && m[3] < 0;
        if ((skipHorizText && horiz) || (skipRotatedText && !horiz)) { return; }
    }
    ...
    if (doClip) {
        if (path) {
            if (textClipPath) { textClipPath->append (path); }
            else {
                textClipPath = path;
                path = NULL;
            }
    ...
    if (doStroke) { splash->setStrokeAdjust (strokeAdjust); }

    if (path) { delete path; }
}

GBool SplashOutputDev::beginType3Char (
    ...
    if (!(gfxFont = state->getFont ())) { return gFalse; }
    ...
    if (!(nT3Fonts > 0 &&
        ...
        for (i = 1; i < nT3Fonts; ++i) {
            if (t3FontCache[i]->matches (
                    fontID, ctm[0], ctm[1], ctm[2], ctm[3])) {
                t3Font = t3FontCache[i];
                for (j = i; j > 0; --j) { t3FontCache[j] = t3FontCache[j - 1]; }
        ...
        if (i >= nT3Fonts) {
            ...
            t3FontCache[0] = new T3FontCache (
                fontID, ctm[0], ctm[1], ctm[2], ctm[3],
                (int)floor (xMin - xt) - 2, (int)floor (yMin - yt) - 2,
    ...
    for (j = 0; j < t3Font->cacheAssoc; ++j) {
        if ((t3Font->cacheTags[i + j].mru & 0x8000) &&
            ...
            drawType3Glyph (
                state, t3Font, &t3Font->cacheTags[i + j],
...
void SplashOutputDev::endType3Char (GfxState* state) {
    ...
    if (t3GlyphStack->cacheTag) {
        --nestCount;
        memcpy (
            t3GlyphStack->cacheData, bitmap->getDataPtr (),
        ...
        ctm = state->getCTM ();
        state->setCTM (
            ctm[0], ctm[1], ctm[2], ctm[3], t3GlyphStack->origCTM4,
            t3GlyphStack->origCTM5);
        updateCTM (state, 0, 0, 0, 0, 0, 0);
        drawType3Glyph (
            state, t3GlyphStack->cache, t3GlyphStack->cacheTag,
...
void SplashOutputDev::type3D1 (
    ...
    if (t3GlyphStack->haveDx) { return; }
    t3GlyphStack->haveDx = gTrue;
    // don't cache if we got a gsave/grestore before the d1
    if (t3GlyphStack->doNotCache) { return; }
    ...
    if (colorMode == splashModeMono1) {
        bitmap = new SplashBitmap (
            t3Font->glyphW, t3Font->glyphH, 1, splashModeMono1, gFalse);
        splash =
            new Splash (bitmap, gFalse, t3GlyphStack->origSplash->getScreen ());
        color[0] = 0;
        splash->clear (color);
        color[0] = 0xff;
    }
        ...
        bitmap = new SplashBitmap (
            t3Font->glyphW, t3Font->glyphH, 1, splashModeMono8, gFalse);
        splash = new Splash (
            bitmap, vectorAntialias, t3GlyphStack->origSplash->getScreen ());
        color[0] = 0x00;
        splash->clear (color);
        color[0] = 0xff;
    }
    ...
    state->setCTM (
        ctm[0], ctm[1], ctm[2], ctm[3], -t3Font->glyphX, -t3Font->glyphY);
    updateCTM (state, 0, 0, 0, 0, 0, 0);
    ++nestCount;
}

void SplashOutputDev::drawType3Glyph (
    GfxState* state, T3FontCache* t3Font, T3FontCacheTag* tag, Guchar* data) {
...
void SplashOutputDev::drawImageMask (
    ...
    splash->fillImageMask (
        &imageMaskSrc, &imgMaskData, width, height, mat, t3GlyphStack != NULL,
        interpolate);
    if (inlineImg) {
...
void SplashOutputDev::setSoftMaskFromImageMask (
    ...
    maskBitmap = new SplashBitmap (
        bitmap->getWidth (), bitmap->getHeight (), 1, splashModeMono8, gFalse);
    ...
    maskSplash->fillImageMask (
        &imageMaskSrc, &imgMaskData, width, height, mat, gFalse, interpolate);
    delete imgMaskData.imgStr;
    str->close ();
    delete maskSplash;
...
GBool SplashOutputDev::imageSrc (
    void* data, SplashColorPtr colorLine, Guchar* alphaLine) {
...
GBool SplashOutputDev::alphaImageSrc (
    void* data, SplashColorPtr colorLine, Guchar* alphaLine) {
    SplashOutImageData* imgData = (SplashOutImageData*)data;
    Guchar *p, *aq;
    SplashColorPtr q, col;
    ...
    for (x = 0, q = colorLine, aq = alphaLine; x < imgData->width;
        ...
        if (imgData->lookup) {
            switch (imgData->colorMode) {
            case splashModeMono1:
            case splashModeMono8: *q++ = imgData->lookup[*p]; break;
            case splashModeRGB8:
            case splashModeBGR8:
                col = &imgData->lookup[3 * *p];
                *q++ = col[0];
...
void SplashOutputDev::drawImage (
    GfxState* state, Object* ref, Stream* str, int width, int height,
    ...
    setOverprintMask (
        colorMap->getColorSpace (), state->getFillOverprint (),
    ...
    imgData.imgStr = new ImageStream (
    ...
    if (colorMode == splashModeMono1) { srcMode = splashModeMono8; }
    ...
    splash->drawImage (
        src, &imgData, srcMode, maskColors ? gTrue : gFalse, width, height, mat,
        interpolate);
    if (inlineImg) {
...
GBool SplashOutputDev::maskedImageSrc (
    void* data, SplashColorPtr colorLine, Guchar* alphaLine) {
...
void SplashOutputDev::drawMaskedImage (
    ...
    if (maskWidth > width || maskHeight > height) {
        ...
        imgMaskData.y = 0;
        maskBitmap =
            new SplashBitmap (width, height, 1, splashModeMono1, gFalse);
        ...
        maskSplash->fillImageMask (
            &imageMaskSrc, &imgMaskData, maskWidth, maskHeight, mat, gTrue,
            interpolate);
        delete imgMaskData.imgStr;
        maskStr->close ();
        delete maskSplash;
        ...
        splash->drawImage (
            &maskedImageSrc, &imgData, srcMode, gTrue, width, height, mat,
            interpolate);
...
void SplashOutputDev::drawSoftMaskedImage (
    ...
    imgMaskData.imgStr = new ImageStream (
        maskStr, maskWidth, maskColorMap->getNumPixelComps (),
    ...
    maskSplash->setStrokeAdjust (globalParams->getStrokeAdjust ());
    clearMaskRegion (state, maskSplash, 0, 0, 1, 1);
    maskSplash->drawImage (
        &imageSrc, &imgMaskData, splashModeMono8, gFalse, maskWidth, maskHeight,
        mat, interpolate);
    delete imgMaskData.imgStr;
    maskStr->close ();
    ...
    splash->drawImage (
        &imageSrc, &imgData, srcMode, gFalse, width, height, mat, interpolate);
...
void SplashOutputDev::reduceImageResolution (
    Stream* str, double* ctm, int* width, int* height) {
    double sw, sh;
    int reduction;
...
void SplashOutputDev::clearMaskRegion (
    GfxState* state, Splash* maskSplash, double xMin, double yMin, double xMax,
    double yMax) {
    SplashBitmap* maskBitmap;
    ...
    if (xxMinI < 0) { xxMinI = 0; }
    xxMaxI = (int)ceil (xxMax);
    if (xxMaxI > maskBitmap->getWidth ()) { xxMaxI = maskBitmap->getWidth (); }
    yyMinI = (int)floor (yyMin);
    if (yyMinI < 0) { yyMinI = 0; }
...
void SplashOutputDev::beginTransparencyGroup (
    ...
    if (tx + w > bitmap->getWidth ()) { w = bitmap->getWidth () - tx; }
    if (w < 1) { w = 1; }
    h = (int)ceil (yMax) - ty + 1;
    if (ty + h > bitmap->getHeight ()) { h = bitmap->getHeight () - ty; }
    if (h < 1) { h = 1; }
    ...
    if (forSoftMask && isolated && !knockout && blendingColorSpace) {
        if (blendingColorSpace->getMode () == csDeviceGray ||
            ...
            blendingColorSpace->getMode () == csDeviceRGB ||
            ...
            blendingColorSpace->getMode () == csDeviceCMYK ||
            (blendingColorSpace->getMode () == csICCBased &&
    ...
    // create the temporary bitmap
    bitmap =
        new SplashBitmap (w, h, bitmapRowPad, colorMode, gTrue, bitmapTopDown);
    splash = new Splash (
        bitmap, vectorAntialias, transpGroup->origSplash->getScreen ());
    splash->setMinLineWidth (globalParams->getMinLineWidth ());
    splash->setStrokeAdjust (globalParams->getStrokeAdjust ());
    ...
    splash->setFillPattern (
        transpGroup->origSplash->getFillPattern ()->copy ());
    ...
    splash->setInTransparencyGroup (
        transpGroup->origBitmap, tx, ty, !isolated, knockout);
...
void SplashOutputDev::paintTransparencyGroup (GfxState* state, double* bbox) {
    ...
    if (tx < bitmap->getWidth () && ty < bitmap->getHeight ()) {
        splash->setOverprintMask (0xffffffff);
        splash->composite (
            tBitmap, 0, 0, tx, ty, tBitmap->getWidth (), tBitmap->getHeight (),
            gFalse, !isolated);
    }
...
void SplashOutputDev::setSoftMask (
    ...
    softMask = new SplashBitmap (
        bitmap->getWidth (), bitmap->getHeight (), 1, splashModeMono8, gFalse);
    memset (
        softMask->getDataPtr (), (int)(backdrop2 * 255.0 + 0.5),
        softMask->getRowSize () * softMask->getHeight ());
    if (tx < softMask->getWidth () && ty < softMask->getHeight ()) {
        p = softMask->getDataPtr () + ty * softMask->getRowSize () + tx;
        for (y = 0; y < tBitmap->getHeight (); ++y) {
            for (x = 0; x < tBitmap->getWidth (); ++x) {
                if (alpha) { lum = tBitmap->getAlpha (x, y) / 255.0; }
                else {
                    tBitmap->getPixel (x, y, color);
                    // convert to luminosity
                    switch (tBitmap->getMode ()) {
                    case splashModeMono1:
                    case splashModeMono8: lum = color[0] / 255.0; break;
                    case splashModeRGB8:
                    case splashModeBGR8:
...
void SplashOutputDev::getModRegion (
...
SplashFont* SplashOutputDev::getFont (GString* name, SplashCoord* textMatA) {
    ...
    for (i = 0; i < nBuiltinFonts; ++i) {
        if (!name->cmp (builtinFonts[i].name)) { break; }
    }
    if (i == nBuiltinFonts) { return NULL; }
    ref.num = i;
    ref.gen = -1;
    ...
    if ((fontFile = fontEngine->getFontFile (id))) {
        ...
        if (!(fontLoc = GfxFont::locateBase14Font (name))) { return NULL; }
#if LOAD_FONTS_FROM_MEM
        fontBuf = NULL;
...
#endif
        if (fontLoc->fontType == fontType1) {
            fontFile = fontEngine->loadType1Font (
                id,
#if LOAD_FONTS_FROM_MEM
                fontBuf,
#else
...
void SplashOutputDev::setInShading (GBool sh) { splash->setInShading (sh); }
#endif

...
static ZoomMenuInfo zoomMenuInfo[nZoomMenuItems] = { { "400%", 400 },
                                                     { "200%", 200 },
                                                     { "150%", 150 },
                                                     { "125%", 125 },
                                                     { "100%", 100 },
...
XPDFViewer::~XPDFViewer () {
    delete core;
    if (aboutBigFont) { XmFontListFree (aboutBigFont); }
...
void XPDFViewer::open (GString* fileName, int pageA, GString* destName) {
    ...
    if (!core->getDoc () || !core->getDoc ()->getFileName () ||
        ...
        if (!loadFile (fileName, NULL, NULL)) { return; }
    }
    getPageAndDest (pageA, destName, &pg, &dest);
    z = core->getZoom ();
    if (dest) {
...
void XPDFViewer::clear () {
    ...
    // set up title
    title =
        app->getTitle () ? app->getTitle ()->getCString () : (char*)xpdfAppName;
...
void XPDFViewer::reloadFile () {
    ...
    if (!core->getDoc () || !core->getDoc ()->getFileName ()) { return; }
    pg = core->getPageNum ();
...
void XPDFViewer::getPageAndDest (
...
void XPDFViewer::actionCbk (void* data, char* action) {
    ...
    if (!strcmp (action, "Quit")) { viewer->app->quit (); }
}
...
void XPDFViewer::keyPressCbk (
    ...
    if (key >= 0x20 && key <= 0xfe) { keyCode = (int)key; }
    ...
    if ((cmds = globalParams->getKeyBinding (
             keyCode, viewer->getModifiers (modifiers),
...
int XPDFViewer::getModifiers (Guint modifiers) {
    ...
    mods = 0;
    if (modifiers & ShiftMask) { mods |= xpdfKeyModShift; }
    if (modifiers & ControlMask) { mods |= xpdfKeyModCtrl; }
    if (modifiers & Mod1Mask) { mods |= xpdfKeyModAlt; }
    return mods;
}
...
void XPDFViewer::execCmd (GString* cmd, XEvent* event) {
    ...
    name = NULL;
    nArgs = 0;
    for (i = 0; i < cmdMaxArgs; ++i) { args[i] = NULL; }
    p0 = cmd->getCString ();
...
static int mouseX (XEvent* event) {
    switch (event->type) {
    case ButtonPress:
    case ButtonRelease: return event->xbutton.x;
    case KeyPress: return event->xkey.x;
    }
    return 0;
}
...
static int mouseY (XEvent* event) {
    switch (event->type) {
    case ButtonPress:
    case ButtonRelease: return event->xbutton.y;
    case KeyPress: return event->xkey.y;
    }
    return 0;
}

void XPDFViewer::cmdAbout (GString* args[], int nArgs, XEvent* event) {
    XtManageChild (aboutDialog);
}

void XPDFViewer::cmdCloseOutline (GString* args[], int nArgs, XEvent* event) {
    Dimension w;

    if (outlineScroll == None) { return; }
    ...
    if (w > 1) {
        ...
        XtVaSetValues (
            outlineScroll, XmNpaneMinimum, 1, XmNpaneMaximum, 1, NULL);
        XtVaSetValues (
            outlineScroll, XmNpaneMinimum, 1, XmNpaneMaximum, 10000, NULL);
    }
}
...
void XPDFViewer::cmdEndPan (GString* args[], int nArgs, XEvent* event) {
    core->endPan (mouseX (event), mouseY (event));
}

void XPDFViewer::cmdEndSelection (GString* args[], int nArgs, XEvent* event) {
    core->endSelection (mouseX (event), mouseY (event));
}

void XPDFViewer::cmdFind (GString* args[], int nArgs, XEvent* event) {
    mapFindDialog ();
}
...
void XPDFViewer::cmdFocusToPageNum (GString* args[], int nArgs, XEvent* event) {
    if (toolBar != None) {
        XmTextFieldSetSelection (
            pageNumText, 0, strlen (XmTextFieldGetString (pageNumText)),
...
void XPDFViewer::cmdGoBackward (GString* args[], int nArgs, XEvent* event) {
    core->goBackward ();
}

void XPDFViewer::cmdGoForward (GString* args[], int nArgs, XEvent* event) {
    core->goForward ();
}

void XPDFViewer::cmdGotoDest (GString* args[], int nArgs, XEvent* event) {
    int pg;
    LinkDest* dest;
...
void XPDFViewer::cmdOpenFileAtDest (GString* args[], int nArgs, XEvent* event) {
    open (args[0], 1, args[1]);
}

void XPDFViewer::cmdOpenFileAtDestInNewWin (
    GString* args[], int nArgs, XEvent* event) {
    app->openAtDest (args[0], args[1]);
}

void XPDFViewer::cmdOpenFileAtPage (GString* args[], int nArgs, XEvent* event) {
    open (args[0], atoi (args[1]->getCString ()), NULL);
}

void XPDFViewer::cmdOpenFileAtPageInNewWin (
    GString* args[], int nArgs, XEvent* event) {
    app->open (args[0], atoi (args[1]->getCString ()));
}

void XPDFViewer::cmdOpenFileInNewWin (
    GString* args[], int nArgs, XEvent* event) {
    app->open (args[0]);
}
...
void XPDFViewer::cmdOpenOutline (GString* args[], int nArgs, XEvent* event) {
    ...
    if (w == 1) {
        ...
        // within an XmPanedWindow
        XtVaSetValues (
            outlineScroll, XmNpaneMinimum, outlinePaneWidth, XmNpaneMaximum,
...
void XPDFViewer::cmdPageDown (GString* args[], int nArgs, XEvent* event) {
    core->scrollPageDown ();
}

void XPDFViewer::cmdPageUp (GString* args[], int nArgs, XEvent* event) {
    core->scrollPageUp ();
}

void XPDFViewer::cmdPostPopupMenu (GString* args[], int nArgs, XEvent* event) {
    XmMenuPosition (
        popupMenu,
        event->type == ButtonPress ? &event->xbutton : (XButtonEvent*)NULL);
...
void XPDFViewer::cmdQuit (GString* args[], int nArgs, XEvent* event) {
    app->quit ();
}

void XPDFViewer::cmdRaise (GString* args[], int nArgs, XEvent* event) {
...
void XPDFViewer::cmdRotateCCW (GString* args[], int nArgs, XEvent* event) {
    int r;
            ...
            case 'Y':
                if (!gotSel) {
                    if (!core->getSelection (
                            &selPage, &selURX, &selURY, &selLRX, &selLRY)) {
                        selPage = 0;
            ...
            case 'k':
                if (!gotMouse) {
                    if (event->type == ButtonPress ||
                        ...
                        core->cvtWindowToUser (
                            event->xbutton.x, event->xbutton.y, &mousePage,
                            &mouseX, &mouseY);
                    }
                    else if (event->type == KeyPress) {
                        core->cvtWindowToUser (
                            event->xkey.x, event->xkey.y, &mousePage, &mouseX,
                            &mouseY);
                    }
                    else {
                        mousePage = 0;
...
void XPDFViewer::cmdScrollDown (GString* args[], int nArgs, XEvent* event) {
    core->scrollDown (atoi (args[0]->getCString ()));
}

void XPDFViewer::cmdScrollDownNextPage (
    GString* args[], int nArgs, XEvent* event) {
    core->scrollDownNextPage (atoi (args[0]->getCString ()));
}

void XPDFViewer::cmdScrollLeft (GString* args[], int nArgs, XEvent* event) {
    core->scrollLeft (atoi (args[0]->getCString ()));
}

void XPDFViewer::cmdScrollOutlineDown (
    GString* args[], int nArgs, XEvent* event) {
    Widget sb;
    ...
    if ((sb = XtNameToWidget (outlineScroll, "VertScrollBar"))) {
        XtVaGetValues (
            sb, XmNvalue, &val, XmNincrement, &inc, XmNpageIncrement, &pageInc,
            XmNmaximum, &m, XmNsliderSize, &slider, NULL);
...
void XPDFViewer::cmdScrollOutlineUp (
    GString* args[], int nArgs, XEvent* event) {
    Widget sb;
    int val, inc, pageInc, m, slider;
    ...
    if ((sb = XtNameToWidget (outlineScroll, "VertScrollBar"))) {
        XtVaGetValues (
            sb, XmNvalue, &val, XmNincrement, &inc, XmNpageIncrement, &pageInc,
            XmNminimum, &m, XmNsliderSize, &slider, NULL);
        if ((val -= inc * atoi (args[0]->getCString ())) < m) { val = m; }
        XmScrollBarSetValues (sb, val, slider, inc, pageInc, True);
    }
}

void XPDFViewer::cmdScrollRight (GString* args[], int nArgs, XEvent* event) {
    core->scrollRight (atoi (args[0]->getCString ()));
}

void XPDFViewer::cmdScrollToBottomEdge (
    GString* args[], int nArgs, XEvent* event) {
    core->scrollToBottomEdge ();
}

void XPDFViewer::cmdScrollToBottomRight (
    GString* args[], int nArgs, XEvent* event) {
    core->scrollToBottomRight ();
}

void XPDFViewer::cmdScrollToLeftEdge (
    GString* args[], int nArgs, XEvent* event) {
    core->scrollToLeftEdge ();
}

void XPDFViewer::cmdScrollToRightEdge (
    GString* args[], int nArgs, XEvent* event) {
    core->scrollToRightEdge ();
}

void XPDFViewer::cmdScrollToTopEdge (
    GString* args[], int nArgs, XEvent* event) {
    core->scrollToTopEdge ();
}

void XPDFViewer::cmdScrollToTopLeft (
    GString* args[], int nArgs, XEvent* event) {
    core->scrollToTopLeft ();
}

void XPDFViewer::cmdScrollUp (GString* args[], int nArgs, XEvent* event) {
    core->scrollUp (atoi (args[0]->getCString ()));
}

void XPDFViewer::cmdScrollUpPrevPage (
    GString* args[], int nArgs, XEvent* event) {
...
void XPDFViewer::cmdSetSelection (GString* args[], int nArgs, XEvent* event) {
    ...
    pg = atoi (args[0]->getCString ());
    core->cvtUserToDev (
        core->getPageNum (), atof (args[1]->getCString ()),
        atof (args[2]->getCString ()), &ulx, &uly);
    core->cvtUserToDev (
        core->getPageNum (), atof (args[3]->getCString ()),
        atof (args[4]->getCString ()), &lrx, &lry);
    core->setSelection (pg, ulx, uly, lrx, lry);
}

void XPDFViewer::cmdSinglePageMode (GString* args[], int nArgs, XEvent* event) {
    Widget btn;
...
void XPDFViewer::cmdStartPan (GString* args[], int nArgs, XEvent* event) {
    core->startPan (mouseX (event), mouseY (event));
}

void XPDFViewer::cmdStartSelection (GString* args[], int nArgs, XEvent* event) {
    core->startSelection (mouseX (event), mouseY (event));
}

void XPDFViewer::cmdToggleContinuousMode (
    GString* args[], int nArgs, XEvent* event) {
    if (core->getContinuousMode ()) { cmdSinglePageMode (NULL, 0, event); }
    else {
        cmdContinuousMode (NULL, 0, event);
    }
}

void XPDFViewer::cmdToggleFullScreenMode (
    GString* args[], int nArgs, XEvent* event) {
    if (core->getFullScreen ()) { cmdWindowMode (NULL, 0, event); }
    else {
        cmdFullScreenMode (NULL, 0, event);
    }
}

void XPDFViewer::cmdToggleOutline (GString* args[], int nArgs, XEvent* event) {
    Dimension w;

    if (outlineScroll == None) { return; }
    XtVaGetValues (outlineScroll, XmNwidth, &w, NULL);
    if (w > 1) { cmdCloseOutline (NULL, 0, event); }
    else {
        cmdOpenOutline (NULL, 0, event);
    }
}

void XPDFViewer::cmdWindowMode (GString* args[], int nArgs, XEvent* event) {
    PDFDoc* doc;
    XPDFViewer* viewer;
    int pg;
    Widget btn;
...
void XPDFViewer::cmdZoomFitPage (GString* args[], int nArgs, XEvent* event) {
...
void XPDFViewer::cmdZoomFitWidth (GString* args[], int nArgs, XEvent* event) {
...
void XPDFViewer::cmdZoomIn (GString* args[], int nArgs, XEvent* event) {
    int z;
...
void XPDFViewer::cmdZoomOut (GString* args[], int nArgs, XEvent* event) {
    int z;
...
void XPDFViewer::cmdZoomPercent (GString* args[], int nArgs, XEvent* event) {
    double z;
...
void XPDFViewer::initWindow (GBool fullScreen) {
    ...
    ++n;
    win = XtCreatePopupShell (
        "win", topLevelShellWidgetClass, app->getAppShell (), args, n);
    if (app->getInstallCmap ()) {
        XtVaSetValues (win, XmNcolormap, colormap, NULL);
    }
    XmAddWMProtocolCallback (
        win, XInternAtom (display, "WM_DELETE_WINDOW", False), &closeMsgCbk,
        this);
    ...
    if (fullScreen) {
        ...
        if (app->getGeometry ()) {
            n = 0;
            XtSetArg (args[n], XmNgeometry, app->getGeometry ()->getCString ());
            ++n;
    ...
    // set traversal order
    XtVaSetValues (
        core->getDrawAreaWidget (), XmNnavigationType, XmEXCLUSIVE_TAB_GROUP,
        NULL);
    if (toolBar != None) {
        XtVaSetValues (backBtn, XmNnavigationType, XmEXCLUSIVE_TAB_GROUP, NULL);
        XtVaSetValues (
            prevTenPageBtn, XmNnavigationType, XmEXCLUSIVE_TAB_GROUP, NULL);
        XtVaSetValues (
            prevPageBtn, XmNnavigationType, XmEXCLUSIVE_TAB_GROUP, NULL);
        XtVaSetValues (
            nextPageBtn, XmNnavigationType, XmEXCLUSIVE_TAB_GROUP, NULL);
        XtVaSetValues (
            nextTenPageBtn, XmNnavigationType, XmEXCLUSIVE_TAB_GROUP, NULL);
        XtVaSetValues (
            forwardBtn, XmNnavigationType, XmEXCLUSIVE_TAB_GROUP, NULL);
        XtVaSetValues (
            pageNumText, XmNnavigationType, XmEXCLUSIVE_TAB_GROUP, NULL);
        XtVaSetValues (
            zoomWidget, XmNnavigationType, XmEXCLUSIVE_TAB_GROUP, NULL);
        XtVaSetValues (findBtn, XmNnavigationType, XmEXCLUSIVE_TAB_GROUP, NULL);
        XtVaSetValues (
            printBtn, XmNnavigationType, XmEXCLUSIVE_TAB_GROUP, NULL);
        XtVaSetValues (
            aboutBtn, XmNnavigationType, XmEXCLUSIVE_TAB_GROUP, NULL);
...
void XPDFViewer::initToolbar (Widget parent) {
    ...
    XtAddCallback (
        prevTenPageBtn, XmNactivateCallback, &prevTenPageCbk, (XtPointer)this);
    n = 0;
    XtSetArg (args[n], XmNleftAttachment, XmATTACH_WIDGET);
    ++n;
    ...
    XtAddCallback (
        prevPageBtn, XmNactivateCallback, &prevPageCbk, (XtPointer)this);
    n = 0;
    XtSetArg (args[n], XmNleftAttachment, XmATTACH_WIDGET);
    ++n;
    ...
    XtAddCallback (
        nextPageBtn, XmNactivateCallback, &nextPageCbk, (XtPointer)this);
    n = 0;
    XtSetArg (args[n], XmNleftAttachment, XmATTACH_WIDGET);
    ++n;
    ...
    XtAddCallback (
        nextTenPageBtn, XmNactivateCallback, &nextTenPageCbk, (XtPointer)this);
    n = 0;
    XtSetArg (args[n], XmNleftAttachment, XmATTACH_WIDGET);
    ++n;
    ...
    XtAddCallback (
        forwardBtn, XmNactivateCallback, &forwardCbk, (XtPointer)this);
    ...
    XtAddCallback (
        pageNumText, XmNactivateCallback, &pageNumCbk, (XtPointer)this);
    n = 0;
    XtSetArg (args[n], XmNleftAttachment, XmATTACH_WIDGET);
    ++n;
    ...
    for (i = 0; i < nZoomMenuItems; ++i) { XmStringFree (st[i]); }
    addToolTip (zoomComboBox, "Zoom");
    XtAddCallback (
        zoomComboBox, XmNselectionCallback, &zoomComboBoxCbk, (XtPointer)this);
...
void XPDFViewer::initPanedWin (Widget parent) {
    ...
    XtAddCallback (
        outlineTree, XPDFNselectionCallback, &outlineSelectCbk,
        (XtPointer)this);
}
...
void XPDFViewer::initPopupMenu () {
    ...
    XtManageChild (btn);
    XtAddCallback (
        btn, XmNactivateCallback, &openInNewWindowCbk, (XtPointer)this);
    n = 0;
    ...
    XtManageChild (btn);
    XtAddCallback (
        btn, XmNvalueChangedCallback, &continuousModeToggleCbk,
        (XtPointer)this);
    n = 0;
    ...
    XtManageChild (btn);
    XtAddCallback (
        btn, XmNvalueChangedCallback, &fullScreenToggleCbk, (XtPointer)this);
    n = 0;
    ...
    XtManageChild (btn);
    XtAddCallback (
        btn, XmNactivateCallback, &zoomToSelectionCbk, (XtPointer)this);
    n = 0;
...
void XPDFViewer::addToolTip (Widget widget, char* text) {
    ...
    if (XtIsComposite (widget)) {
        XtVaGetValues (
            widget, XmNnumChildren, &n, XmNchildren, &children, NULL);
        for (i = 0; i < n; ++i) { addToolTip (children[i], text); }
    }
    else {
...
void XPDFViewer::mapWindow () {
    ...
    if (XpmCreatePixmapFromData (
            display, XtWindow (win), xpdfIcon, &iconPixmap, NULL, NULL) ==
        XpmSuccess) {
    ...
    if (toolBar != None) {
        XtVaGetValues (
            backBtn, XmNdepth, &depth, XmNforeground, &fg, XmNbackground, &bg,
            XmNarmColor, &arm, NULL);
        XtVaSetValues (
            backBtn, XmNlabelType, XmPIXMAP, XmNlabelPixmap,
            XCreatePixmapFromBitmapData (
                display, XtWindow (toolBar), (char*)backArrow_bits,
                backArrow_width, backArrow_height, fg, bg, depth),
            XmNarmPixmap,
            XCreatePixmapFromBitmapData (
                display, XtWindow (toolBar), (char*)backArrow_bits,
                backArrow_width, backArrow_height, fg, arm, depth),
        ...
        XtVaSetValues (
            ...
            XCreatePixmapFromBitmapData (
                display, XtWindow (toolBar), (char*)leftArrow_bits,
                leftArrow_width, leftArrow_height, fg, bg, depth),
            XmNarmPixmap,
            XCreatePixmapFromBitmapData (
                display, XtWindow (toolBar), (char*)leftArrow_bits,
                leftArrow_width, leftArrow_height, fg, arm, depth),
            XmNlabelInsensitivePixmap,
            XCreatePixmapFromBitmapData (
        ...
        XtVaSetValues (
            ...
            XCreatePixmapFromBitmapData (
                display, XtWindow (toolBar), (char*)rightArrow_bits,
                rightArrow_width, rightArrow_height, fg, bg, depth),
            XmNarmPixmap,
            XCreatePixmapFromBitmapData (
                display, XtWindow (toolBar), (char*)rightArrow_bits,
                rightArrow_width, rightArrow_height, fg, arm, depth),
            XmNlabelInsensitivePixmap,
            XCreatePixmapFromBitmapData (
        ...
        XtVaSetValues (
            ...
            NULL);
        XtVaSetValues (
            findBtn, XmNlabelType, XmPIXMAP, XmNlabelPixmap,
            XCreatePixmapFromBitmapData (
                display, XtWindow (toolBar), (char*)find_bits, find_width,
            ...
            XmNlabelInsensitivePixmap,
            XCreatePixmapFromBitmapData (
                display, XtWindow (toolBar), (char*)findDis_bits, findDis_width,
                findDis_height, fg, bg, depth),
            NULL);
        XtVaSetValues (
            printBtn, XmNlabelType, XmPIXMAP, XmNlabelPixmap,
            XCreatePixmapFromBitmapData (
                display, XtWindow (toolBar), (char*)print_bits, print_width,
            ...
            NULL);
        XtVaSetValues (
            aboutBtn, XmNlabelType, XmPIXMAP, XmNlabelPixmap,
            XCreatePixmapFromBitmapData (
                display, XtWindow (toolBar), (char*)about_bits, about_width,
...
int XPDFViewer::getZoomIdx () {
    ...
    for (i = 0; i < nZoomMenuItems; ++i) {
        if (core->getZoom () == zoomMenuInfo[i].zoom) { return i; }
    }
    return -1;
}
...
void XPDFViewer::setZoomVal (double z) {
    ...
    for (i = maxZoomIdx; i < minZoomIdx; ++i) {
        if (z > zoomMenuInfo[i].zoom) { break; }
    }
    XtVaSetValues (zoomMenu, XmNmenuHistory, zoomMenuBtns[i], NULL);
#endif
}

void XPDFViewer::prevPageCbk (
...
void XPDFViewer::prevTenPageCbk (
    Widget widget, XtPointer ptr, XtPointer callData) {
    XPDFViewer* viewer = (XPDFViewer*)ptr;
...
void XPDFViewer::nextTenPageCbk (
    Widget widget, XtPointer ptr, XtPointer callData) {
    XPDFViewer* viewer = (XPDFViewer*)ptr;
...
void XPDFViewer::backCbk (Widget widget, XtPointer ptr, XtPointer callData) {
    XPDFViewer* viewer = (XPDFViewer*)ptr;
...
void XPDFViewer::forwardCbk (Widget widget, XtPointer ptr, XtPointer callData) {
    XPDFViewer* viewer = (XPDFViewer*)ptr;
...
void XPDFViewer::zoomComboBoxCbk (
    Widget widget, XtPointer ptr, XtPointer callData) {
    XPDFViewer* viewer = (XPDFViewer*)ptr;
...
void XPDFViewer::findCbk (Widget widget, XtPointer ptr, XtPointer callData) {
    XPDFViewer* viewer = (XPDFViewer*)ptr;

    if (!viewer->core->getDoc ()) { return; }
    viewer->mapFindDialog ();
}

void XPDFViewer::printCbk (Widget widget, XtPointer ptr, XtPointer callData) {
    XPDFViewer* viewer = (XPDFViewer*)ptr;
...
void XPDFViewer::aboutCbk (Widget widget, XtPointer ptr, XtPointer callData) {
    XPDFViewer* viewer = (XPDFViewer*)ptr;

    XtManageChild (viewer->aboutDialog);
}

void XPDFViewer::quitCbk (Widget widget, XtPointer ptr, XtPointer callData) {
    XPDFViewer* viewer = (XPDFViewer*)ptr;
...
void XPDFViewer::saveAsCbk (Widget widget, XtPointer ptr, XtPointer callData) {
    XPDFViewer* viewer = (XPDFViewer*)ptr;
...
void XPDFViewer::continuousModeToggleCbk (
    Widget widget, XtPointer ptr, XtPointer callData) {
    XPDFViewer* viewer = (XPDFViewer*)ptr;
...
void XPDFViewer::fullScreenToggleCbk (
    Widget widget, XtPointer ptr, XtPointer callData) {
    XPDFViewer* viewer = (XPDFViewer*)ptr;
    XmToggleButtonCallbackStruct* data =
        (XmToggleButtonCallbackStruct*)callData;

    if (data->set == XmSET) { viewer->cmdFullScreenMode (NULL, 0, NULL); }
    else {
        viewer->cmdWindowMode (NULL, 0, NULL);
    }
}

void XPDFViewer::rotateCCWCbk (
    Widget widget, XtPointer ptr, XtPointer callData) {
    XPDFViewer* viewer = (XPDFViewer*)ptr;
...
void XPDFViewer::zoomToSelectionCbk (
    Widget widget, XtPointer ptr, XtPointer callData) {
    XPDFViewer* viewer = (XPDFViewer*)ptr;
    int pg;
    double ulx, uly, lrx, lry;
...
void XPDFViewer::pageNumCbk (Widget widget, XtPointer ptr, XtPointer callData) {
    ...
    if (!viewer->core->getDoc ()) { goto err; }
    s = XmTextFieldGetString (viewer->pageNumText);
    for (p = s; *p; ++p) {
        if (!isdigit (*p)) { goto err; }
    }
    pg = atoi (s);
...
void XPDFViewer::updateCbk (
    ...
    if (fileName) {
        ...
        XtVaSetValues (
            viewer->win, XmNtitle, title->getCString (), XmNiconName,
            title->getCString (), NULL);
        if (!viewer->app->getTitle ()) { delete title; }
        viewer->setupOutline ();
    ...
    if (viewer->toolBar != None) {
        if (pageNum >= 0) {
            ...
            XmTextFieldSetString (viewer->pageNumText, buf);
            XtVaSetValues (
                viewer->prevTenPageBtn, XmNsensitive, pageNum > 1, NULL);
            XtVaSetValues (
                viewer->prevPageBtn, XmNsensitive, pageNum > 1, NULL);
            XtVaSetValues (
                viewer->nextTenPageBtn, XmNsensitive,
                pageNum < viewer->core->getDoc ()->getNumPages (), NULL);
            XtVaSetValues (
                viewer->nextPageBtn, XmNsensitive,
                pageNum < viewer->core->getDoc ()->getNumPages (), NULL);
            XtVaSetValues (
                viewer->backBtn, XmNsensitive, viewer->core->canGoBack (),
                NULL);
            XtVaSetValues (
                viewer->forwardBtn, XmNsensitive, viewer->core->canGoForward (),
                NULL);
        }
...
void XPDFViewer::setupOutlineItems (
    GList* items, Widget parent, UnicodeMap* uMap) {
    OutlineItem* item;
    GList* kids;
    Widget label;
    Arg args[20];
...
void XPDFViewer::outlineSelectCbk (
    Widget widget, XtPointer ptr, XtPointer callData) {
    XPDFViewer* viewer = (XPDFViewer*)ptr;
    ...
    if (item) {
        if (item->getAction ()) { viewer->core->doAction (item->getAction ()); }
    }
    viewer->core->takeFocus ();
}
...
void XPDFViewer::initOpenDialog () {
    ...
    XmStringFree (s3);
    XtUnmanageChild (
        XmFileSelectionBoxGetChild (openDialog, XmDIALOG_HELP_BUTTON));
    XtAddCallback (openDialog, XmNokCallback, &openOkCbk, (XtPointer)this);

    if (core->getDoc () && core->getDoc ()->getFileName ()) {
        dir = makePathAbsolute (
            grabPath (core->getDoc ()->getFileName ()->getCString ()));
...
void XPDFViewer::openOkCbk (Widget widget, XtPointer ptr, XtPointer callData) {
    XPDFViewer* viewer = (XPDFViewer*)ptr;
    ...
    if (XmStringGetNextSegment (context, &fileName, &charSet, &dir, &sep)) {
        ...
        if (viewer->openInNewWindow) { viewer->app->open (fileNameStr); }
        else {
...
void XPDFViewer::initFindDialog () {
    ...
    XtManageChild (closeBtn);
    XtAddCallback (
        closeBtn, XmNactivateCallback, &findCloseCbk, (XtPointer)this);
    ...
    ++n;
    findWholeWordToggle =
        XmCreateToggleButton (findDialog, "wholeWord", args, n);
    XmStringFree (s);
...
#ifdef LESSTIF_VERSION
    XtAddCallback (
        findText, XmNactivateCallback, &findFindCbk, (XtPointer)this);
#endif
...
void XPDFViewer::mapFindDialog () {
    XmTextFieldSetSelection (
        findText, 0, XmTextFieldGetLastPosition (findText),
        XtLastTimestampProcessed (display));
...
void XPDFViewer::initPrintDialog () {
    ...
    ++n;
    printWithCmdBtn =
        XmCreateToggleButton (printDialog, "printWithCmd", args, n);
    XmStringFree (s);
    XtManageChild (printWithCmdBtn);
    XtAddCallback (
        printWithCmdBtn, XmNvalueChangedCallback, &printWithCmdBtnCbk,
        (XtPointer)this);
    n = 0;
    ...
    XtAddCallback (
        printToFileBtn, XmNvalueChangedCallback, &printToFileBtnCbk,
        (XtPointer)this);
    n = 0;
    XtSetArg (args[n], XmNtopAttachment, XmATTACH_WIDGET);
    ++n;
    ...
    if ((psFileName = globalParams->getPSFile ())) {
        if (psFileName->getChar (0) == '|') {
            XmTextFieldSetString (printCmdText, psFileName->getCString () + 1);
        }
        else {
...
void XPDFViewer::setupPrintDialog () {
    ...
    if (!psFileName || psFileName->getChar (0) == '|') {
        if ((pdfFileName = doc->getFileName ())) {
            ...
            if (!strcmp (p, ".pdf") || !strcmp (p, ".PDF")) {
                psFileName2 = new GString (
                    pdfFileName->getCString (), pdfFileName->getLength () - 4);
            }
            else {
    ...
    if (psFileName) { delete psFileName; }
...
void XPDFViewer::printWithCmdBtnCbk (
    Widget widget, XtPointer ptr, XtPointer callData) {
    XPDFViewer* viewer = (XPDFViewer*)ptr;
    XmToggleButtonCallbackStruct* data =
        (XmToggleButtonCallbackStruct*)callData;
...
void XPDFViewer::printToFileBtnCbk (
    Widget widget, XtPointer ptr, XtPointer callData) {
    XPDFViewer* viewer = (XPDFViewer*)ptr;
    XmToggleButtonCallbackStruct* data =
        (XmToggleButtonCallbackStruct*)callData;
...
void XPDFViewer::printPrintCbk (
    ...
    psOut = new PSOutputDev (
        psFileName->getCString (), doc, firstPage, lastPage, psModePS);
    if (psOut->isOk ()) {
        doc->displayPages (
...
XmFontList XPDFViewer::createFontList (char* xlfd) {
    ...
    if (font) { fontList = XmFontListCreate (font, XmSTRING_DEFAULT_CHARSET); }
    else {
        params = (String)xlfd;
        nParams = 1;
        XtAppWarningMsg (
            app->getAppContext (), "noSuchFont", "CvtStringToXmFontList",
            "XtToolkitError", "No such font: %s", &params, &nParams);
        fontList = NULL;
    }
    ...
    entry = XmFontListEntryLoad (
        display, xlfd, XmFONT_IS_FONT, XmFONTLIST_DEFAULT_TAG);

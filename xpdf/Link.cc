...
LinkGoTo::LinkGoTo(Object *destObj)
{
    ...
    // named destination
    if (destObj->is_name()) {
        namedDest = new GString(destObj->as_name());
...
LinkNamed::LinkNamed(Object *nameObj)
{
    name = NULL;
    if (nameObj->is_name()) {
        name = new GString(nameObj->as_name());
    }
}
...
LinkHide::LinkHide(Object *fieldsObj, Object *hideFlagObj)
{
    ...
    if (hideFlagObj->is_bool()) {
        hideFlag = hideFlagObj->as_bool();
    } else {
            ...
            if ((obj1 = resolve(annots[i])).is_dict()) {
                ...
                if (obj2.is_name("Link") ||
                    ...
                    if (link->isOk()) {
                        if (numLinks >= size) {
                            size += 16;
                            links = (Link **)reallocarray(links, size,
                                                          sizeof(Link *));
                        }
                        links[numLinks++] = link;
                    } else {

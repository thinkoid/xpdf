...
GfxColorSpace* GfxColorSpace::parse (Object* csObj, int recursion) {
    ...
    if (csObj->isName ()) {
        if (csObj->isName ("DeviceGray") || csObj->isName ("G")) {
            ...
            error (
                errSyntaxError, -1, "Bad color space '{0:s}'",
                csObj->getName ());
        }
    }
...
GfxColorSpace* GfxColorSpace::create (GfxColorSpaceMode mode) {
    ...
    cs = NULL;
    if (mode == csDeviceGray) { cs = new GfxDeviceGrayColorSpace (); }
...
void GfxColorSpace::getDefaultRanges (
    double* decodeLow, double* decodeRange, int maxImgPixel) {
    int i;
...
void GfxLabColorSpace::getDefaultRanges (
    double* decodeLow, double* decodeRange, int maxImgPixel) {
    decodeLow[0] = 0;
    decodeRange[0] = 100;
    decodeLow[1] = aMin;
...
GfxICCBasedColorSpace::GfxICCBasedColorSpace (
    int nCompsA, GfxColorSpace* altA, Ref* iccProfileStreamA) {
    nComps = nCompsA;
    alt = altA;
...
GfxICCBasedColorSpace::~GfxICCBasedColorSpace () { delete alt; }
...
GfxColorSpace* GfxICCBasedColorSpace::parse (Array* arr, int recursion) {
    ...
    if (dict->lookup ("Alternate", &obj2)->isNull () ||
        ...
        case 3: altA = GfxColorSpace::create (csDeviceRGB); break;
        case 4: altA = GfxColorSpace::create (csDeviceCMYK); break;
        default:
...
void GfxICCBasedColorSpace::getDefaultRanges (
    double* decodeLow, double* decodeRange, int maxImgPixel) {
...
GfxColorSpace* GfxIndexedColorSpace::parse (Array* arr, int recursion) {
    ...
    if (obj1.isStream ()) {
        ...
        for (i = 0; i <= indexHighA; ++i) {
            for (j = 0; j < n; ++j) { cs->lookup[i * n + j] = (unsigned char)*s++; }
        }
    }
    else {
...
GfxColor*
GfxIndexedColorSpace::mapColorToBase (GfxColor* color, GfxColor* baseColor) {
    unsigned char* p;
...
void GfxIndexedColorSpace::getDefaultRanges (
    double* decodeLow, double* decodeRange, int maxImgPixel) {
    decodeLow[0] = 0;
...
GfxSeparationColorSpace::GfxSeparationColorSpace (
    ...
    if (!name->cmp ("Cyan")) { overprintMask = 0x01; }
...
GfxDeviceNColorSpace::GfxDeviceNColorSpace (
    ...
    for (i = 0; i < nComps; ++i) {
        names[i] = namesA[i];
        if (names[i]->cmp ("None")) { nonMarking = false; }
        if (!names[i]->cmp ("Cyan")) { overprintMask |= 0x01; }
...
GfxDeviceNColorSpace::GfxDeviceNColorSpace (
    ...
    nonMarking = nonMarkingA;
    overprintMask = overprintMaskA;
    for (i = 0; i < nComps; ++i) { names[i] = namesA[i]->copy (); }
}
...
err4:
    delete altA;
err3:
    for (i = 0; i < nCompsA; ++i) { delete namesA[i]; }
err2:
    obj1.free ();
err1:
    return NULL;
...
GfxColorSpace* GfxPatternColorSpace::copy () {
    ...
    cs = new GfxPatternColorSpace (
        under ? under->copy () : (GfxColorSpace*)NULL);
    return cs;
}
...
GfxPattern* GfxPattern::parse (Object* objRef, Object* obj) {
    ...
    if (obj->isDict ()) { obj->dictLookup ("PatternType", &typeObj); }
...
GfxTilingPattern* GfxTilingPattern::parse (Object* patObjRef, Object* patObj) {
    ...
    if (!patObj->isStream ()) { return NULL; }
    ...
    if (dict->lookup ("TilingType", &obj1)->isInt ()) {
        ...
        tilingTypeA = 1;
        error (
            errSyntaxWarning, -1, "Invalid or missing TilingType in pattern");
    }
    obj1.free ();
    bboxA[0] = bboxA[1] = 0;
    bboxA[2] = bboxA[3] = 1;
    ...
    obj1.free ();
    if (dict->lookup ("XStep", &obj1)->isNum ()) { xStepA = obj1.getNum (); }
    else {
        xStepA = 1;
        error (errSyntaxError, -1, "Invalid or missing XStep in pattern");
    }
    obj1.free ();
    if (dict->lookup ("YStep", &obj1)->isNum ()) { yStepA = obj1.getNum (); }
    else {
        yStepA = 1;
    ...
    pat = new GfxTilingPattern (
        paintTypeA, tilingTypeA, bboxA, xStepA, yStepA, &resDictA, matrixA,
        patObjRef);
    resDictA.free ();
    return pat;
}
...
GfxTilingPattern::GfxTilingPattern (
    int paintTypeA, int tilingTypeA, double* bboxA, double xStepA,
    double yStepA, Object* resDictA, double* matrixA, Object* contentStreamRefA)
    : GfxPattern (1) {
    int i;
...
GfxPattern* GfxTilingPattern::copy () {
    return new GfxTilingPattern (
        paintType, tilingType, bbox, xStep, yStep, &resDict, matrix,
        &contentStreamRef);
}
...
GfxShading* GfxShading::parse (Object* obj) {
    ...
    if (obj->isDict ()) { dict = obj->getDict (); }
    else if (obj->isStream ()) {
    ...
    switch (typeA) {
    case 1: shading = GfxFunctionShading::parse (dict); break;
    case 2: shading = GfxAxialShading::parse (dict); break;
    case 3: shading = GfxRadialShading::parse (dict); break;
    case 4:
        if (obj->isStream ()) {
            shading =
                GfxGouraudTriangleShading::parse (4, dict, obj->getStream ());
        }
        else {
    ...
    case 5:
        if (obj->isStream ()) {
            shading =
                GfxGouraudTriangleShading::parse (5, dict, obj->getStream ());
        }
        else {
...
GfxAxialShading* GfxAxialShading::parse (Dict* dict) {
    ...
    shading = new GfxAxialShading (
        x0A, y0A, x1A, y1A, t0A, t1A, funcsA, nFuncsA, extend0A, extend1A);
    if (!shading->init (dict)) {
        delete shading;
        return NULL;
    }
...
GfxRadialShading* GfxRadialShading::parse (Dict* dict) {
    ...
    shading = new GfxRadialShading (
        x0A, y0A, r0A, x1A, y1A, r1A, t0A, t1A, funcsA, nFuncsA, extend0A,
        extend1A);
    if (!shading->init (dict)) {
        delete shading;
        return NULL;
...
GfxGouraudTriangleShading::parse (int typeA, Dict* dict, Stream* str) {
    ...
    while (1) {
        if (typeA == 4) {
            if (!bitBuf->getBits (flagBits, &flag)) { break; }
        }
        ...
        for (i = 0; i < nCompsA; ++i) {
            if (!bitBuf->getBits (compBits, &c[i])) { break; }
        }
        if (i < nCompsA) { break; }
    ...
    shading = new GfxGouraudTriangleShading (
        typeA, verticesA, nVerticesA, trianglesA, nTrianglesA, nCompsA, funcsA,
        nFuncsA);
    if (!shading->init (dict)) {
        delete shading;
        return NULL;
...
void GfxGouraudTriangleShading::getTriangle (
    ...
    *x0 = vertices[v].x;
    *y0 = vertices[v].y;
    for (j = 0; j < nComps; ++j) { color0[j] = vertices[v].color[j]; }
    v = triangles[i][1];
    *x1 = vertices[v].x;
    *y1 = vertices[v].y;
    for (j = 0; j < nComps; ++j) { color1[j] = vertices[v].color[j]; }
    v = triangles[i][2];
    *x2 = vertices[v].x;
    *y2 = vertices[v].y;
...
GfxPatchMeshShading::parse (int typeA, Dict* dict, Stream* str) {
    ...
    while (1) {
        ...
        for (i = 0; i < nColors; ++i) {
            for (j = 0; j < nCompsA; ++j) {
                if (!bitBuf->getBits (compBits, &ci)) { break; }
    ...
    shading = new GfxPatchMeshShading (
        typeA, patchesA, nPatchesA, nCompsA, funcsA, nFuncsA);
    if (!shading->init (dict)) {
        delete shading;
        return NULL;
    }
...
GfxImageColorMap::GfxImageColorMap (
    int bitsA, Object* decode, GfxColorSpace* colorSpaceA) {
...
void GfxImageColorMap::getGray (unsigned char* x, GfxGray* gray) {
    ...
    if (colorSpace2) {
        for (i = 0; i < nComps2; ++i) { color.c[i] = lookup2[i][x[0]]; }
        colorSpace2->getGray (&color, gray);
    }
    else {
        for (i = 0; i < nComps; ++i) { color.c[i] = lookup[i][x[i]]; }
...
void GfxImageColorMap::getGrayByteLine (unsigned char* in, unsigned char* out, int n) {
    GfxColor color;
    GfxGray gray;
    int i, j;
...
void GfxImageColorMap::getRGBByteLine (unsigned char* in, unsigned char* out, int n) {
    GfxColor color;
    GfxRGB rgb;
    int i, j;
...
void GfxImageColorMap::getCMYKByteLine (unsigned char* in, unsigned char* out, int n) {
    GfxColor color;
    GfxCMYK cmyk;
    int i, j;
...
// Used for copy().
GfxPath::GfxPath (
    bool justMoved1, double firstX1, double firstY1, GfxSubpath** subpaths1,
    int n1, int size1) {
    int i;
...
void GfxPath::lineTo (double x, double y) {
    if (justMoved || (n > 0 && subpaths[n - 1]->isClosed ())) {
        ...
        if (justMoved) { subpaths[n] = new GfxSubpath (firstX, firstY); }
            ...
            subpaths[n] = new GfxSubpath (
                subpaths[n - 1]->getLastX (), subpaths[n - 1]->getLastY ());
        }
        ++n;
        justMoved = false;
    }
...
void GfxPath::offset (double dx, double dy) {
    ...
    for (i = 0; i < n; ++i) { subpaths[i]->offset (dx, dy); }
}
...
void GfxState::getUserClipBBox (
    ...
    if (tx < xMin1) { xMin1 = tx; }
    else if (tx > xMax1) {
        xMax1 = tx;
    }
    if (ty < yMin1) { yMin1 = ty; }
    else if (ty > yMax1) {
        yMax1 = ty;
    }
...
void GfxState::clip () {
    ...
    if (xMax < clipXMax) { clipXMax = xMax; }
    if (yMax < clipYMax) { clipYMax = yMax; }
}
...
void GfxState::clipToRect (double xMin, double yMin, double xMax, double yMax) {
    ...
    if (xMin1 > clipXMin) { clipXMin = xMin1; }
    if (yMin1 > clipYMin) { clipYMin = yMin1; }
    if (xMax1 < clipXMax) { clipXMax = xMax1; }
    if (yMax1 < clipYMax) { clipYMax = yMax1; }
}

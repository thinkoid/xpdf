...
{
    appShell = XtAppInitialize(&appContext, xpdfAppName, xOpts(), xOptsSize(),
                               argc, argv, fallbackResources(), NULL, 0);

    display = XtDisplay(appShell);
    screenNum = XScreenNumberOfScreen(XtScreen(appShell));
#if XmVERSION > 1
    XtVaSetValues(XmGetXmDisplay(XtDisplay(appShell)), XmNenableButtonTab, True,
                  NULL);
#endif
#if XmVERSION > 1
    ...
    XtVaSetValues(XmGetXmDisplay(XtDisplay(appShell)),
                  ...
                  XmNdragReceiverProtocolStyle, XmDRAG_NONE, NULL);
#endif
...
void XPDFApp::getResources()
{
    ...
    XtGetApplicationResources(appShell, &resources, xResources(),
                              xResourcesSize(), NULL, 0);
    ...
    if (resources.paperColor) {
        if (XAllocNamedColor(display, colormap, resources.paperColor, &xcol,
                             &xcol2)) {
    ...
    if (XAllocNamedColor(display, colormap, resources.matteColor, &xcol,
                         &xcol2)) {
        mattePixel = xcol.pixel;
    } else {
        mattePixel = paperPixel;
    }
    if (XAllocNamedColor(display, colormap, resources.fullScreenMatteColor, &xcol,
                         &xcol2)) {
...
XPDFUI *XPDFApp::open(GString *fileName, int page, GString *ownerPassword,
                      GString *userPassword)
{
    auto viewer = std::make_shared< XPDFUI >(
        this, fileName, page, nullptr, fullScreen, ownerPassword, userPassword);
    ...
    if (remoteAtom != None) {
        ...
        XtAddEventHandler(remoteWin, PropertyChangeMask, False, &remoteMsgCbk,
                          this);
...
XPDFUI *XPDFApp::openAtDest(GString *fileName, GString *dest,
                            GString *ownerPassword, GString *userPassword)
{
    auto viewer = std::make_shared< XPDFUI >(this, fileName, 1, dest, fullScreen,
...
XPDFUI *XPDFApp::reopen(XPDFUI *pviewer, PDFDoc *doc, int page, bool fullScreenA)
{
    actions::remove(viewers, pviewer, &std::shared_ptr< XPDFUI >::get);

    auto viewer =
        std::make_shared< XPDFUI >(this, doc, page, nullptr, fullScreenA);
...
void XPDFApp::close(XPDFUI *viewer, bool closeLast)
{
    if (viewers.size() == 1) {
        ...
        decltype(viewers) other =
            viewers | views::filter([&](auto &x) { return viewer != x.get(); }) |
            to< std::vector >;
...
void XPDFApp::remoteMsgCbk(Widget widget, XtPointer ptr, XEvent *event,
                           Boolean *cont)
{
    ...
    *cont = False;
    ...
    if (XGetWindowProperty(app->display, XtWindow(app->remoteWin),
                           app->remoteAtom, 0, remoteCmdSize / 4, True,
                           app->remoteAtom, &type, &format, &size, &remain,

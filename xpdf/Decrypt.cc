...
static void
aes256KeyExpansion (DecryptAES256State* s, unsigned char* objKey, int objKeyLen);
    ...
    if (encRevision == 5 || encRevision == 6) {
        ...
        if (ownerPassword) {
            ...
            if (!memcmp (test, ownerKey->c_str (), 32)) {
                ...
                aes256DecryptBlock (
                    &state, (unsigned char*)ownerEnc->c_str (), false);
        ...
        if (!memcmp (test, userKey->c_str (), 32)) {
            ...
            for (i = 0; i < 16; ++i) { state.cbc[i] = 0; }
            aes256DecryptBlock (
                &state, (unsigned char*)userEnc->c_str (), false);
            memcpy (fileKey, state.buf, 16);
            aes256DecryptBlock (
                &state, (unsigned char*)userEnc->c_str () + 16, false);
            memcpy (fileKey + 16, state.buf, 16);
        ...
        if (ownerPassword) {
            ...
            md5 (test, 32, test);
            if (encRevision == 3) {
                for (i = 0; i < 50; ++i) { md5 (test, keyLength, test); }
            }
            if (encRevision == 2) {
            ...
            if (makeFileKey2 (
                    encVersion, encRevision, keyLength, ownerKey, userKey,
        ...
        return makeFileKey2 (
            encVersion, encRevision, keyLength, ownerKey, userKey, permissions,
...
void Decrypt::r6Hash (
    ...
    while (1) {
        ...
        for (j = 1; j < 64; ++j) { memcpy (key1 + j * n, key1, n); }
        n *= 64;
        aesKeyExpansion (&state128, key, 16, false);
        for (j = 0; j < 16; ++j) { state128.cbc[j] = key[16 + j]; }
        for (j = 0; j < n; j += 16) {
            aesEncryptBlock (&state128, key1 + j);
            memcpy (key1 + j, state128.buf, 16);
        }
        k = 0;
        for (j = 0; j < 16; ++j) { k += key1[j] % 3; }
        k %= 3;
        switch (k) {
        case 0:
            sha256 (key1, n, key);
        ...
        ++i;
        if (i >= 64 && key1[n - 1] <= i - 32) { break; }
    }
}

bool Decrypt::makeFileKey2 (
    int encVersion, int encRevision, int keyLength, GString* ownerKey,
    GString* userKey, int permissions, GString* fileID, GString* userPassword,
    ...
    md5 (buf, len, fileKey);
    if (encRevision == 3) {
        for (i = 0; i < 50; ++i) { md5 (fileKey, keyLength, fileKey); }
    }
...
DecryptStream::DecryptStream (
    ...
    for (i = 0; i < keyLength; ++i) { objKey[i] = fileKey[i]; }
    switch (algo) {
    case cryptRC4:
        ...
        if ((objKeyLength = keyLength + 5) > 16) { objKeyLength = 16; }
        break;
    case cryptAES:
        objKey[keyLength] = objNum & 0xff;
        objKey[keyLength + 1] = (objNum >> 8) & 0xff;
        ...
        if ((objKeyLength = keyLength + 5) > 16) { objKeyLength = 16; }
        break;
    case cryptAES256: objKeyLength = keyLength; break;
    }
}

DecryptStream::~DecryptStream () { delete str; }
...
bool DecryptStream::isBinary (bool last) { return str->isBinary (last); }
...
unsigned char rc4DecryptByte (unsigned char* state, unsigned char* x, unsigned char* y, unsigned char c) {
...
void aesKeyExpansion (
    ...
    for (i = 4; i < 44; ++i) {
        temp = s->w[i - 1];
        if (!(i & 3)) { temp = subWord (rotWord (temp)) ^ rcon[i / 4]; }
        s->w[i] = s->w[i - 4] ^ temp;
    }
    if (decrypt) {
...
void aesDecryptBlock (DecryptAESState* s, unsigned char* in, bool last) {
    ...
    if (last) {
        ...
        for (i = 15; i >= n; --i) { s->buf[i] = s->buf[i - n]; }
        s->bufIdx = n;
    }
}
...
aes256KeyExpansion (DecryptAES256State* s, unsigned char* objKey, int objKeyLen) {
    ...
    for (i = 8; i < 60; ++i) {
        temp = s->w[i - 1];
        if ((i & 7) == 0) { temp = subWord (rotWord (temp)) ^ rcon[i / 8]; }
        else if ((i & 7) == 4) {
            temp = subWord (temp);
        }
        s->w[i] = s->w[i - 8] ^ temp;
    }
    for (round = 1; round <= 13; ++round) { invMixColumnsW (&s->w[round * 4]); }
}
...
static inline size_t md5Round1 (
    size_t a, size_t b, size_t c, size_t d, size_t Xk, size_t s, size_t Ti) {
    return b + rotateLeft ((a + ((b & c) | (~b & d)) + Xk + Ti), s);
}

static inline size_t md5Round2 (
    size_t a, size_t b, size_t c, size_t d, size_t Xk, size_t s, size_t Ti) {
    return b + rotateLeft ((a + ((b & d) | (c & ~d)) + Xk + Ti), s);
}

static inline size_t md5Round3 (
    size_t a, size_t b, size_t c, size_t d, size_t Xk, size_t s, size_t Ti) {
    return b + rotateLeft ((a + (b ^ c ^ d) + Xk + Ti), s);
}

static inline size_t md5Round4 (
    size_t a, size_t b, size_t c, size_t d, size_t Xk, size_t s, size_t Ti) {
...
void md5Finish (MD5State* state) {
    ...
    if (state->bufLen > 56) {
        while (state->bufLen < 64) { state->buf[state->bufLen++] = 0x00; }
        md5ProcessBlock (state);
    }
    while (state->bufLen < 56) { state->buf[state->bufLen++] = 0x00; }
...
void md5 (unsigned char* msg, int msgLen, unsigned char* digest) {
    ...
    md5Finish (&state);
    for (i = 0; i < 16; ++i) { digest[i] = state.digest[i]; }
}
...
static void sha256 (unsigned char* msg, int msgLen, unsigned char* hash) {
    ...
    blkLen = 0;
    for (i = 0; i + 64 <= msgLen; i += 64) { sha256HashBlock (msg + i, H); }
    blkLen = msgLen - i;
    if (blkLen > 0) { memcpy (blk, msg + i, blkLen); }

    // pad the message
    blk[blkLen++] = 0x80;
    if (blkLen > 56) {
        while (blkLen < 64) { blk[blkLen++] = 0; }
        sha256HashBlock (blk, H);
        blkLen = 0;
    }
    while (blkLen < 56) { blk[blkLen++] = 0; }
    blk[56] = 0;
    blk[57] = 0;
    blk[58] = 0;
    blk[59] = 0;
...
static inline SHA512Uint64
sha512Ch (SHA512Uint64 x, SHA512Uint64 y, SHA512Uint64 z) {
    return (x & y) ^ (~x & z);
}

static inline SHA512Uint64
sha512Maj (SHA512Uint64 x, SHA512Uint64 y, SHA512Uint64 z) {
    return (x & y) ^ (x & z) ^ (y & z);
}
...
static void sha512 (unsigned char* msg, int msgLen, unsigned char* hash) {
    ...
    blkLen = 0;
    for (i = 0; i + 128 <= msgLen; i += 128) { sha512HashBlock (msg + i, H); }
    blkLen = msgLen - i;
    if (blkLen > 0) { memcpy (blk, msg + i, blkLen); }

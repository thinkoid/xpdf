...
GfxFont *GfxFont::makeFont(XRef *xref, const char *tagA, Ref idA, Dict *fontDict)
{
    ...
    if (typeA < fontCIDType0) {
        font =
            new Gfx8BitFont(xref, tagA, idA, nameA, typeA, embFontIDA, fontDict);
    } else {
        font =
            new GfxCIDFont(xref, tagA, idA, nameA, typeA, embFontIDA, fontDict);
    }

    return font;
}

GfxFont::GfxFont(const char *tagA, Ref idA, GString *nameA, GfxFontType typeA,
                 Ref embFontIDA)
{
    ok = false;
...
void GfxFont::readFontDescriptor(XRef *xref, Dict *fontDict)
{
    ...
    if ((obj1 = resolve((*fontDict)["FontDescriptor"])).is_dict()) {
        ...
        if (obj2.is_name()) {
            embFontName = new GString(obj2.as_name());
        }

        // look for MissingWidth
        obj2 = resolve(obj1.as_dict()["MissingWidth"]);
        if (obj2.is_num()) {
            missingWidth = obj2.as_num();
        }
...
GfxFontLoc *GfxFont::locateFont(XRef *xref, bool ps)
{
    ...
    if (!ps && !isCIDFont() && ((Gfx8BitFont *)this)->base14) {
        ...
        delete base14Name;
        if (path && (fontLoc = getExternalFont(path, fontNum, oblique, false))) {
            return fontLoc;
        }
    }
    ...
    //----- system font
    if (name &&
        (path = globalParams->findSystemFontFile(name, &sysFontType, &fontNum))) {
        if (isCIDFont()) {
    ...
    if (!isCIDFont()) {
        ...
        if (ps) {
            ...
            path =
                globalParams->findBase14FontFile(substName, &fontNum, &oblique);
            delete substName;
            if (path) {
...
GfxFontLoc *GfxFont::getExternalFont(GString *path, int fontNum, double oblique,
                                     bool cid)
{
    ...
    if (missing && globalParams->getMapNumericCharNames()) {
        for (code = 0; code < 256; ++code) {
            if ((charName = enc[code]) && !toUnicode[code] &&
                ...
                } else if (hex && n == 2 && isxdigit(charName[0]) &&
                           isxdigit(charName[1])) {
                    sscanf(charName, "%x", &code2);
                } else if (!hex && n >= 2 && n <= 4 && isdigit(charName[0]) &&
                           isdigit(charName[1])) {
                    code2 = atoi(charName);
                } else if (n >= 3 && n <= 5 && isdigit(charName[1]) &&
                           isdigit(charName[2])) {
                    code2 = atoi(charName + 1);
                } else if (n >= 4 && n <= 6 && isdigit(charName[2]) &&
                           isdigit(charName[3])) {
...
int Gfx8BitFont::getNextChar(const char *s, int len, CharCode *code, Unicode *u,
        ...
        while (i + 1 < obj1.as_array().size()) {
            ...
            if (obj2.is_int() && obj3.is_int() &&
                ...
                if ((obj4 = resolve(obj1[i + 2])).is_num()) {
                    ...
                    widths.exceps[widths.nExceps].width = obj4.as_num() * 0.001;
                    ++widths.nExceps;
                } else {
                ...
                if (widths.nExceps + obj3.as_array().size() > excepsSize) {
                    excepsSize = (widths.nExceps + obj3.as_array().size() + 15) &
                                 ~15;
                    widths.exceps = (GfxFontCIDWidthExcep *)reallocarray(
                        widths.exceps, excepsSize, sizeof(GfxFontCIDWidthExcep));
                }
                j = obj2.as_int();
        ...
        while (i + 1 < obj1.as_array().size()) {
            ...
            if (obj2.is_int() && obj3.is_int() &&
                ...
                if (widths.nExcepsV + obj3.as_array().size() / 3 > excepsSize) {
                    excepsSize =
                        (widths.nExcepsV + obj3.as_array().size() / 3 + 15) & ~15;
                    widths.excepsV = (GfxFontCIDWidthExcepV *)reallocarray(
                        widths.excepsV, excepsSize,
                        sizeof(GfxFontCIDWidthExcepV));
                }
...
int GfxCIDFont::getNextChar(const char *s, int len, CharCode *code, Unicode *u,
                            int uSize, int *uLen, double *dx, double *dy,
                            double *ox, double *oy)
{
    CID      cid;
...
GfxFontDict::GfxFontDict(XRef *xref, Ref *fontDictRef, Dict *fontDict)
{
    ...
    for (i = 0; i < numFonts; ++i) {
        ...
        if (obj2.is_dict()) {
            ...
            fonts[i] = GfxFont::makeFont(xref, fontDict->key_at(i).c_str(), r,
                                         &obj2.as_dict());

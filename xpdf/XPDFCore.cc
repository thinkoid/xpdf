...
XPDFCore::XPDFCore (
    ...
    if (fullScreen) { zoom = zoomPage; }
    else {
        initialZoom = globalParams->getInitialZoom ();
        if (!initialZoom->cmp ("page")) { zoom = zoomPage; }
...
XPDFCore::~XPDFCore () {
    ...
    if (drawAreaGC) { XFreeGC (display, drawAreaGC); }
    if (scrolledWin) { XtDestroyWidget (scrolledWin); }
    if (busyCursor) { XFreeCursor (display, busyCursor); }
    if (linkCursor) { XFreeCursor (display, linkCursor); }
...
int XPDFCore::loadFile (
    ...
    if (err == errNone) {
        ...
        if (updateCbk) {
            (*updateCbk) (
                updateCbkData, doc->getFileName (), -1, doc->getNumPages (),
                NULL);
        }
    }
    return err;
...
void XPDFCore::loadDoc (PDFDoc* docA) {
    ...
    if (updateCbk) {
        (*updateCbk) (
            updateCbkData, doc->getFileName (), -1, doc->getNumPages (), NULL);
    }
}
...
void XPDFCore::resizeToPage (int pg) {
    ...
    if (fullScreen) {
        ...
        if (!doc || pg <= 0 || pg > doc->getNumPages ()) {
            ...
            doc->getPageRotate (pg) == 90 || doc->getPageRotate (pg) == 270) {
        ...
        if (continuousMode) { height += continuousModePageSpacing; }
        if (width > displayW - 100) { width = displayW - 100; }
        if (height > displayH - 100) { height = displayH - 100; }
    }

    if (XtIsRealized (shell)) {
        XtVaGetValues (
            shell, XmNwidth, &topW, XmNheight, &topH, XmNborderWidth,
            &topBorder, NULL);
        XtVaGetValues (drawArea, XmNwidth, &daW, XmNheight, &daH, NULL);
        XtVaSetValues (
            shell, XmNwidth, width + (topW - daW), XmNheight,
...
void XPDFCore::update (
    ...
    oldPage = topPage;
    PDFCore::update (
        topPageA, scrollXA, scrollYA, zoomA, rotateA, force, addToHist,
        adjustScrollX);
    linkAction = NULL;
    if (doc && topPage != oldPage) {
        if (updateCbk) { (*updateCbk) (updateCbkData, NULL, topPage, -1, ""); }
    }
}
...
void XPDFCore::copySelection () {
    ...
    if (!doc->okToCopy ()) { return; }
    if (getSelection (&pg, &ulx, &uly, &lrx, &lry)) {
        if (currentSelection) { delete currentSelection; }
        currentSelection = extractText (pg, ulx, uly, lrx, lry);
        currentSelectionOwner = this;
        XtOwnSelection (
            drawArea, XA_PRIMARY, XtLastTimestampProcessed (display),
            &convertSelectionCbk, NULL, NULL);
    }
}

Boolean XPDFCore::convertSelectionCbk (
    Widget widget, Atom* selection, Atom* target, Atom* type, XtPointer* value,
    unsigned long* length, int* format) {
    Atom* array;

    // send back a list of supported conversion targets
    if (*target == targetsAtom) {
        if (!(array = (Atom*)XtMalloc (sizeof (Atom)))) { return False; }
        array[0] = XA_STRING;
...
void XPDFCore::doAction (LinkAction* action) {
    ...
    case actionNamed:
        ...
        if (!actionName->cmp ("NextPage")) { gotoNextPage (1, true); }
            ...
            error (
                errSyntaxError, -1, "Unknown named action: '{0:t}'",
                actionName);
        }
        break;
    ...
    case actionUnknown:
        error (
            errSyntaxError, -1, "Unknown link action type: '{0:t}'",
...
void XPDFCore::setupX (bool installCmap, int rgbCubeSizeA) {
    ...
    visualList =
        XGetVisualInfo (display, VisualIDMask, &visualTempl, &nVisuals);
    if (nVisuals < 1) {
    ...
    if (!trueColor) {
        ...
        if (installCmap) {
            for (rgbCubeSize = xMaxRGBCube; rgbCubeSize >= 2; --rgbCubeSize) {
                ...
                if (XAllocColorCells (
                        display, colormap, False, NULL, 0, colors, m)) {
                    break;
                }
            }
            if (rgbCubeSize >= 2) {
...
void XPDFCore::initWindow () {
    ...
    if (!fullScreen) { XtManageChild (hScrollBar); }
    XtAddCallback (
        hScrollBar, XmNvalueChangedCallback, &hScrollChangeCbk,
        (XtPointer)this);
#ifndef DISABLE_SMOOTH_SCROLL
    XtAddCallback (
        hScrollBar, XmNdragCallback, &hScrollDragCbk, (XtPointer)this);
#endif
    n = 0;
    ...
    if (!fullScreen) { XtManageChild (vScrollBar); }
    XtAddCallback (
        vScrollBar, XmNvalueChangedCallback, &vScrollChangeCbk,
        (XtPointer)this);
#ifndef DISABLE_SMOOTH_SCROLL
    XtAddCallback (
        vScrollBar, XmNdragCallback, &vScrollDragCbk, (XtPointer)this);
#endif
    ...
    XtOverrideTranslations (
        drawArea, XtParseTranslationTable ("<Btn1Down>:DrawingAreaInput()\n"
...
void XPDFCore::hScrollChangeCbk (
...
void XPDFCore::vScrollChangeCbk (
    Widget widget, XtPointer ptr, XtPointer callData) {
    XPDFCore* core = (XPDFCore*)ptr;
    XmScrollBarCallbackStruct* data = (XmScrollBarCallbackStruct*)callData;
...
void XPDFCore::resizeCbk (Widget widget, XtPointer ptr, XtPointer callData) {
    ...
    if (XCheckTypedWindowEvent (
            core->display, XtWindow (top), ConfigureNotify, &event)) {
    ...
    if (core->zoom == zoomPage || core->zoom == zoomWidth) { sx = sy = -1; }
    else {
        sx = core->scrollX;
        sy = core->scrollY;
    }
...
void XPDFCore::inputCbk (Widget widget, XtPointer ptr, XtPointer callData) {
    ...
    case MotionNotify:
        if (core->doc && core->doc->getNumPages () > 0) {
            ok = core->cvtWindowToDev (
                data->event->xmotion.x, data->event->xmotion.y, &pg, &x, &y);
            if (core->dragging) {
                if (ok) { core->moveSelection (pg, x, y); }
            }
                ...
                if (ok && (action = core->findLink (pg, xu, yu))) {
                    ...
                    if (action != core->linkAction) {
                        ...
                        if (core->updateCbk) {
                            ...
                            case actionGoTo: s = "[internal link]"; break;
                            case actionGoToR:
                                s = ((LinkGoToR*)action)
                                        ->getFileName ()
                                        ->c_str ();
                            ...
                            case actionSubmitForm:
                            case actionHide:
                            case actionUnknown: s = "[unknown link]"; break;
                            }
                            (*core->updateCbk) (
                                core->updateCbkData, NULL, -1, -1, s);
                        }
                    }
                }
                else {
        ...
        if (core->panning) {
            core->scrollTo (
                core->scrollX - (data->event->xmotion.x - core->panMX),
    ...
    case KeyPress:
        if (core->keyPressCbk) {
            ...
            (*core->keyPressCbk) (
                core->keyPressCbkData, key, data->event->xkey.state,
                data->event);
        }
        break;
    }
...
void XPDFCore::redrawRect (
    ...
    if (tile) {
        XPutImage (
            display, drawAreaWin, drawAreaGC, tile->image, xSrc, ySrc, xDest,
            yDest, width, height);
        ...
        XFillRectangle (
            display, drawAreaWin, drawAreaGC, xDest, yDest, width, height);
    }
...
void XPDFCore::updateScrollbars () {
    ...
    if (pages->getLength () > 0) {
        if (continuousMode) { maxPos = maxPageW; }
        else {
    ...
    if (maxPos < drawAreaWidth) { maxPos = drawAreaWidth; }
    n = 0;
    ...
    if (pages->getLength () > 0) {
        if (continuousMode) { maxPos = totalDocH; }
        else {
    ...
    if (maxPos < drawAreaHeight) { maxPos = drawAreaHeight; }
    n = 0;
...
void XPDFCore::setCursor (Cursor cursor) {
    ...
    if (!(topWin = XtWindow (shell))) { return; }
    if (cursor == None) { XUndefineCursor (display, topWin); }
    else {
...
bool XPDFCore::doDialog (
    ...
    XmStringFree (s1);
    if (msg->getLength () <= 80) { XmStringFree (s2); }
    else {
        n = 0;
    ...
    if (hasCancel) {
        XtAddCallback (
            dialog, XmNcancelCallback, &dialogCancelCbk, (XtPointer)this);
    }
    else {
...
void XPDFCore::initPasswordDialog () {
    ...
    XtAddCallback (
        passwordText, XmNmodifyVerifyCallback, &passwordTextVerifyCbk, this);
    ...
    XtAddCallback (
        cancelBtn, XmNactivateCallback, &passwordCancelCbk, (XtPointer)this);
    n = 0;
...
void XPDFCore::passwordTextVerifyCbk (
    Widget widget, XtPointer ptr, XtPointer callData) {
    XPDFCore* core = (XPDFCore*)ptr;
    ...
    i = (int)data->startPos;
    n = (int)data->endPos - i;
    if (i > core->password->getLength ()) { i = core->password->getLength (); }
    ...
    for (i = 0; i < data->text->length; ++i) { data->text->ptr[i] = '*'; }
    data->doit = True;
}

void XPDFCore::passwordOkCbk (
    Widget widget, XtPointer ptr, XtPointer callData) {
    XPDFCore* core = (XPDFCore*)ptr;
...
void XPDFCore::passwordCancelCbk (
    Widget widget, XtPointer ptr, XtPointer callData) {
    XPDFCore* core = (XPDFCore*)ptr;

...
public:
    PSFontFileInfo (
        GString* psNameA, GfxFontType typeA, PSFontFileLocation locA);
    ~PSFontFileInfo ();
...
public:
    PSOutCustomColor (
        double cA, double mA, double yA, double kA, GString* nameA);
    ~PSOutCustomColor ();
...
PSOutCustomColor::~PSOutCustomColor () { delete name; }
...
public:
    DeviceNRecoder (
        Stream* strA, int widthA, int heightA, GfxImageColorMap* colorMapA);
...
DeviceNRecoder::~DeviceNRecoder () {
    if (imgStr) { delete imgStr; }
    if (str->isEncoder ()) { delete str; }
}
...
GBool DeviceNRecoder::fillBuf () {
    ...
    if (pixelIdx >= width * height) { return gFalse; }
...
PSOutputDev::PSOutputDev (
    char* fileName, PDFDoc* docA, int firstPage, int lastPage, PSOutMode modeA,
    int imgLLXA, int imgLLYA, int imgURXA, int imgURYA, GBool manualCtrlA,
    PSOutCustomCodeCbk customCodeCbkA, void* customCodeCbkDataA) {
    FILE* f;
    ...
    if (!strcmp (fileName, "-")) {
        ...
        if (!(f = fopen (fileName, "w"))) {
            error (
                errIO, -1, "Couldn't open PostScript file '{0:s}'", fileName);
            ok = gFalse;
            return;
        }
    }
    ...
    init (
        outputToFile, f, fileTypeA, docA, firstPage, lastPage, modeA, imgLLXA,
        imgLLYA, imgURXA, imgURYA, manualCtrlA);
}

PSOutputDev::PSOutputDev (
    PSOutputFunc outputFuncA, void* outputStreamA, PDFDoc* docA, int firstPage,
    int lastPage, PSOutMode modeA, int imgLLXA, int imgLLYA, int imgURXA,
    int imgURYA, GBool manualCtrlA, PSOutCustomCodeCbk customCodeCbkA,
    ...
    init (
        outputFuncA, outputStreamA, psGeneric, docA, firstPage, lastPage, modeA,
        imgLLXA, imgLLYA, imgURXA, imgURYA, manualCtrlA);
}

void PSOutputDev::init (
    ...
    for (i = 0; i < 14; ++i) {
        ff = new PSFontFileInfo (
            new GString (psBase14SubstFonts[i].psName), fontType1,
            psFontFileResident);
    ...
    for (i = 0; i < names->getLength (); ++i) {
        if (!fontFileInfo->lookup ((GString*)names->get (i))) {
            ff = new PSFontFileInfo (
                (GString*)names->get (i), fontType1, psFontFileResident);
            fontFileInfo->add (ff->psName, ff);
        }
    }
    delete names;
    ...
    if (!manualCtrl) {
        ...
        if (mode != psModeForm) { writePS ("%%BeginProlog\n"); }
        writeXpdfProcset ();
        if (mode != psModeForm) {
            writePS ("%%EndProlog\n");
            writePS ("%%BeginSetup\n");
        ...
        if (mode != psModeForm) { writePS ("%%EndSetup\n"); }
    }
...
PSOutputDev::~PSOutputDev () {
    ...
    if (ok) {
        if (!manualCtrl) {
            writePS ("%%Trailer\n");
            writeTrailer ();
            if (mode != psModeForm) { writePS ("%%EOF\n"); }
        }
        if (fileType == psFile) { fclose ((FILE*)outputStream); }
...
void PSOutputDev::writeHeader (
    ...
    case psModeEPS: writePS ("%!PS-Adobe-3.0 EPSF-3.0\n"); break;
    case psModeForm: writePS ("%!PS-Adobe-3.0 Resource-Form\n"); break;
    }
    ...
    case psModePS:
        if (paperMatch) {
            ...
            writePSFmt (
                "%%DocumentMedia: plain {0:d} {1:d} 0 () ()\n", paperWidth,
                paperHeight);
        }
        writePSFmt (
            "%%BoundingBox: 0 0 {0:d} {1:d}\n", paperWidth, paperHeight);
    ...
    case psModeForm:
        ...
        writePSFmt (
            "/BBox [{0:d} {1:d} {2:d} {3:d}] def\n", (int)floor (mediaBox->x1),
            (int)floor (mediaBox->y1), (int)ceil (mediaBox->x2),
            (int)ceil (mediaBox->y2));
...
void PSOutputDev::writeXpdfProcset () {
    ...
    if ((w = globalParams->getPSMinLineWidth ()) > 0) {
        ...
        for (p = minLineWidthProlog; *p; ++p) { writePSFmt ("{0:s}\n", *p); }
    }
    writePS ("%%EndResource\n");

    if (level >= psLevel3) {
        for (p = cmapProlog; *p; ++p) { writePSFmt ("{0:s}\n", *p); }
    }
}

void PSOutputDev::writeDocSetup (
    ...
    if (customCodeCbk) {
        if ((s = (*customCodeCbk) (
                 this, psOutCustomDocSetup, 0, customCodeCbkData))) {
            writePS (s->getCString ());
            delete s;
        }
    }
    if (mode != psModeForm) { writePS ("end\n"); }
}

void PSOutputDev::writePageTrailer () {
    if (mode != psModeForm) { writePS ("pdfEndPage\n"); }
}
...
void PSOutputDev::writeTrailer () {
    ...
    if (mode == psModeForm) {
        ...
        if (level == psLevel1Sep || level == psLevel2Sep ||
            ...
            if (processColors & psProcessCyan) { writePS (" Cyan"); }
            if (processColors & psProcessMagenta) { writePS (" Magenta"); }
            if (processColors & psProcessYellow) { writePS (" Yellow"); }
            if (processColors & psProcessBlack) { writePS (" Black"); }
            writePS ("\n");
            ...
            for (cc = customColors; cc; cc = cc->next) {
                writePSFmt (
                    "%%+ {0:.4g} {1:.4g} {2:.4g} {3:.4g} ", cc->c, cc->m, cc->y,
                    cc->k);
                writePSString (cc->name);
                writePS ("\n");
            }
...
void PSOutputDev::setupFont (GfxFont* font, Dict* parentResDict) {
    ...
    if (font->getType () == fontType3) {
            ...
            case gfxFontLocExternal:
                ...
                case fontCIDType2OT:
                    //~ should check to see if font actually uses vertical mode
                        ...
                        font, fontLoc->path, fontLoc->fontNum, gTrue);
                    break;
                default: break;
                }
                break;
        ...
        if (fontLoc->locType == gfxFontLocResident && fontLoc->substIdx >= 0) {
            ...
            if (code < 256) { w1 = ((Gfx8BitFont*)font)->getWidth (code); }
            else {
                w1 = 0;
            }
    ...
    if (font->isCIDFont ()) {
        ...
        for (i = 0; i < 256; i += 8) {
            ...
            for (j = 0; j < 8; ++j) {
                ...
                if (charName && !charName[0]) { writePS (" "); }
            }
...
PSFontFileInfo* PSOutputDev::setupEmbeddedType1Font (GfxFont* font, Ref* id) {
    ...
    if (start[0] == 0x80 && start[1] == 0x01) {
        ...
        while (1) {
            for (i = 2; i < 6; ++i) { start[i] = strObj.streamGetChar (); }
            if (start[2] == EOF || start[3] == EOF || start[4] == EOF ||
                start[5] == EOF) {
                break;
            }
            n = start[2] + (start[3] << 8) + (start[4] << 16) +
                (start[5] << 24);
            if (start[1] == 0x01) {
                for (i = 0; i < n; ++i) {
                    if ((c = strObj.streamGetChar ()) == EOF) { break; }
                    writePSChar (c);
                }
            }
            else {
      ...
      length2 += length3 >= 8 ? 8 : length3;
            ...
            for (i = 0; i < 4; ++i) { writePSChar (start[i]); }
            for (i = 4; i < length2; ++i) {
                if ((c = strObj.streamGetChar ()) == EOF) { break; }
                writePSChar (c);
            }
...
PSFontFileInfo*
PSOutputDev::setupExternalType1Font (GfxFont* font, GString* fileName) {
    static char hexChar[17] = "0123456789abcdef";
    GString* psName;
    PSFontFileInfo* ff;
    FILE* fontFile;
    ...
    if (buf[0] == 0x80 && buf[1] == 0x01) {
        while (1) {
            for (i = 2; i < 6; ++i) { buf[i] = fgetc (fontFile); }
        ...
        writePSChar (buf[0]);
        writePSChar (buf[1]);
        while ((c = fgetc (fontFile)) != EOF) { writePSChar (c); }
    }
...
PSFontFileInfo* PSOutputDev::setupEmbeddedType1CFont (GfxFont* font, Ref* id) {
    ...
    if ((fontBuf = font->readEmbFontFile (xref, &fontLen))) {
        if ((ffT1C = FoFiType1C::make (fontBuf, fontLen))) {
            ffT1C->convertToType1 (
                psName->getCString (), NULL, gTrue, outputFunc, outputStream);
            delete ffT1C;
        }
        gfree (fontBuf);
    }
...
PSFontFileInfo*
PSOutputDev::setupEmbeddedOpenTypeT1CFont (GfxFont* font, Ref* id) {
    GString* psName;
    PSFontFileInfo* ff;
    char* fontBuf;
    int fontLen;
    ...
    if ((fontBuf = font->readEmbFontFile (xref, &fontLen))) {
        if ((ffTT = FoFiTrueType::make (fontBuf, fontLen, 0))) {
            if (ffTT->isOpenTypeCFF ()) {
                ffTT->convertToType1 (
                    psName->getCString (), NULL, gTrue, outputFunc,
                    outputStream);
            }
            delete ffTT;
        }
...
PSFontFileInfo*
PSOutputDev::setupEmbeddedTrueTypeFont (GfxFont* font, Ref* id) {
    GString* psName;
    PSFontFileInfo* ff;
    char* fontBuf;
    int fontLen;
...
PSFontFileInfo* PSOutputDev::setupExternalTrueTypeFont (
...
PSFontFileInfo*
PSOutputDev::setupEmbeddedCIDType0Font (GfxFont* font, Ref* id) {
    GString* psName;
    PSFontFileInfo* ff;
    char* fontBuf;
    int fontLen;
    ...
    if ((fontBuf = font->readEmbFontFile (xref, &fontLen))) {
        if ((ffT1C = FoFiType1C::make (fontBuf, fontLen))) {
            if (globalParams->getPSLevel () >= psLevel3) {
                ...
                ffT1C->convertToCIDType0 (
                    psName->getCString (), NULL, 0, outputFunc, outputStream);
            }
            else {
                // otherwise: use a non-CID composite font
                ffT1C->convertToType0 (
                    psName->getCString (), NULL, 0, outputFunc, outputStream);
            }
            delete ffT1C;
        }
...
PSFontFileInfo* PSOutputDev::setupEmbeddedCIDTrueTypeFont (
    ...
    while (fontFileInfo->getNext (&iter, &psName, (void**)&ff)) {
        if (ff->loc == psFontFileEmbedded && ff->embFontID.num == id->num &&
            ...
            ((!ff->codeToGID && !codeToGID) ||
             (ff->codeToGID && codeToGID &&
              !memcmp (
                  ff->codeToGID, codeToGID, codeToGIDLen * sizeof (int))))) {
            fontFileInfo->killIter (&iter);
            return ff;
        }
    }
    ...
    if ((fontBuf = font->readEmbFontFile (xref, &fontLen))) {
        if ((ffTT = FoFiTrueType::make (fontBuf, fontLen, 0))) {
            if (globalParams->getPSLevel () >= psLevel3) {
                // Level 3: use a CID font
                ffTT->convertToCIDType2 (
                    psName->getCString (), codeToGID, codeToGIDLen,
...
PSFontFileInfo* PSOutputDev::setupExternalCIDTrueTypeFont (
    GfxFont* font, GString* fileName, int fontNum, GBool needVerticalMetrics) {
    GString* psName;
    PSFontFileInfo* ff;
    FoFiTrueType* ffTT;
    int* codeToGID;
    ...
    // map CID -> Unicode -> GID
    if (ctu->isIdentity ()) { codeToGIDLen = 65536; }
    else {
    ...
    if (globalParams->getPSLevel () >= psLevel3) {
        // Level 3: use a CID font
        ffTT->convertToCIDType2 (
            psName->getCString (), codeToGID, codeToGIDLen, needVerticalMetrics,
...
PSFontFileInfo*
PSOutputDev::setupEmbeddedOpenTypeCFFFont (GfxFont* font, Ref* id) {
    GString* psName;
    PSFontFileInfo* ff;
    char* fontBuf;
    int fontLen;
...
PSFontFileInfo*
PSOutputDev::setupType3Font (GfxFont* font, Dict* parentResDict) {
    PSFontFileInfo* ff;
    GString* psName;
    Dict* resDict;
    Dict* charProcs;
    ...
    // generate name
    psName = GString::format (
        "T3_{0:d}_{1:d}", font->getID ()->num, font->getID ()->gen);
    ...
    m = font->getFontBBox ();
    writePSFmt (
        "/FontBBox [{0:.6g} {1:.6g} {2:.6g} {3:.6g}] def\n", m[0], m[1], m[2],
        m[3]);
        ...
        for (i = 0; i < charProcs->getLength (); ++i) {
            ...
            if (t3String) {
                if (t3Cacheable) {
                    ...
                    buf = GString::format (
                        "{0:.6g} {1:.6g} setcharwidth\n", t3WX, t3WY);
                }
                (*outputFunc) (
                    outputStream, buf->getCString (), buf->getLength ());
                delete buf;
                (*outputFunc) (
                    outputStream, t3String->getCString (),
                    t3String->getLength ());
                delete t3String;
                t3String = NULL;
            }
            if (t3NeedsRestore) { (*outputFunc) (outputStream, "Q\n", 2); }
            writePS ("} def\n");
        }
        inType3Char = gFalse;
        delete gfx;
...
void PSOutputDev::setupImage (Ref id, Stream* str, GBool mask) {
    ...
    if (useCompressed) { str = str->getUndecodedStream (); }
    if (useLZW) { str = new LZWEncoder (str); }
    else if (useRLE) {
        str = new RunLengthEncoder (str);
    }
    if (useASCIIHex) { str = new ASCIIHexEncoder (str); }
    else {
    ...
    ++size;
    if (useLZW || useRLE) { ++size; }
    writePSFmt (
        "{0:d} array dup /{1:s}Data_{2:d}_{3:d} exch def\n", size,
    ...
    do {
        ...
        if (col > 225) {
            ...
            ++line;
            writePSFmt (
                (char*)(useASCIIHex ? "dup {0:d} <" : "dup {0:d} <~"), line);
            col = 0;
        }
    } while (c != (useASCIIHex ? '>' : '~') && c != EOF);
    writePS ((char*)(useASCIIHex ? "> put\n" : "~> put\n"));
...
GBool PSOutputDev::checkPageSlice (
    Page* page, double hDPI, double vDPI, int rotateA, GBool useMediaBox,
    GBool crop, int sliceX, int sliceY, int sliceW, int sliceH, GBool printing,
    ...
    if (globalParams->getPSAlwaysRasterize ()) { rasterize = gTrue; }
    else {
    ...
    // start the PS page
    page->makeBox (
        dpi, dpi, rotateA, useMediaBox, gFalse, sliceX, sliceY, sliceW, sliceH,
        &box, &crop);
    rotateA += page->getRotate ();
    if (rotateA >= 360) { rotateA -= 360; }
    else if (rotateA < 0) {
        rotateA += 360;
    }
    ...
    if (mono || level == psLevel1) {
        paperColor[0] = 0xff;
        splashOut = new SplashOutputDev (
            splashModeMono8, 1, gFalse, paperColor, gFalse,
        ...
        splashOut = new SplashOutputDev (
            splashModeCMYK8, 1, gFalse, paperColor, gFalse,
            globalParams->getAntialiasPrinting ());
#endif
    }
    else {
        ...
        splashOut = new SplashOutputDev (
            splashModeRGB8, 1, gFalse, paperColor, gFalse,
            globalParams->getAntialiasPrinting ());
    }
    ...
    if (sliceW < 0 || sliceH < 0) {
        if (useMediaBox) { box = *page->getMediaBox (); }
        else {
            box = *page->getCropBox ();
        }
        sliceX = sliceY = 0;
    ...
    for (stripeY = sliceY; stripeY < sliceH; stripeY += stripeH) {
        // rasterize a stripe
        page->makeBox (
            hDPI2, vDPI2, 0, useMediaBox, gFalse, sliceX, stripeY, sliceW,
            stripeH, &box, &crop);
        m0 = box.x2 - box.x1;
        m1 = 0;
        m2 = 0;
        ...
        m5 = box.y1;
        page->displaySlice (
            splashOut, hDPI2, vDPI2, (360 - page->getRotate ()) % 360,
            useMediaBox, crop, sliceX, stripeY, sliceW, stripeH, printing,
        ...
        writePS ("gsave\n");
        writePSFmt (
            "[{0:.6g} {1:.6g} {2:.6g} {3:.6g} {4:.6g} {5:.6g}] concat\n", m0,
            m1, m2, m3, m4, m5);
        switch (level) {
        case psLevel1:
            writePSFmt (
                "{0:d} {1:d} 8 [{2:d} 0 0 {3:d} 0 {4:d}] pdfIm1\n", w, h, w, -h,
                h);
        ...
        case psLevel1Sep:
            writePSFmt (
                "{0:d} {1:d} 8 [{2:d} 0 0 {3:d} 0 {4:d}] pdfIm1Sep\n", w, h, w,
                -h, h);
            p = bitmap->getDataPtr () + (h - 1) * bitmap->getRowSize ();
            i = 0;
        ...
        case psLevel3:
        case psLevel3Sep:
            if (mono) { writePS ("/DeviceGray setcolorspace\n"); }
            else {
...
void PSOutputDev::startPage (int pageNum, GfxState* state) {
    ...
    // underlays
    if (underlayCbk) { (*underlayCbk) (this, underlayCbkData); }
    if (overlayCbk) { saveState (NULL); }
    ...
    case psModePS:
        ...
        if (clipLLX0 < clipURX0 && clipLLY0 < clipURY0) {
            writePSFmt (
                "{0:.6g} {1:.6g} {2:.6g} {3:.6g} re W\n", clipLLX0, clipLLY0,
                clipURX0 - clipLLX0, clipURY0 - clipLLY0);
        }
        else {
            writePSFmt (
                "{0:d} {1:d} {2:d} {3:d} re W\n", x1, y1, x2 - x1, y2 - y1);
        }
    ...
    if (customCodeCbk) {
        if ((s = (*customCodeCbk) (
                 this, psOutCustomPageSetup, pageNum, customCodeCbkData))) {
            writePS (s->getCString ());
            delete s;
        }
    }

    if (mode == psModePS) { writePS ("%%EndPageSetup\n"); }
}
...
void PSOutputDev::updateCTM (
    GfxState* state, double m11, double m12, double m21, double m22, double m31,
    double m32) {
...
void PSOutputDev::updateFillColorSpace (GfxState* state) {
    switch (level) {
    case psLevel1:
    case psLevel1Sep: break;
    case psLevel2:
    case psLevel3:
        if (state->getFillColorSpace ()->getMode () != csPattern) {
            dumpColorSpaceL2 (
                state->getFillColorSpace (), gTrue, gFalse, gFalse);
            writePS (" cs\n");
        }
        break;
    case psLevel2Sep:
    case psLevel3Sep: break;
    }
}
...
void PSOutputDev::updateStrokeColorSpace (GfxState* state) {
    ...
    case psLevel3:
        if (state->getStrokeColorSpace ()->getMode () != csPattern) {
            dumpColorSpaceL2 (
                state->getStrokeColorSpace (), gTrue, gFalse, gFalse);
            writePS (" CS\n");
        }
        break;
    case psLevel2Sep:
...
void PSOutputDev::addProcessColor (double c, double m, double y, double k) {
    if (c > 0) { processColors |= psProcessCyan; }
    if (m > 0) { processColors |= psProcessMagenta; }
    if (y > 0) { processColors |= psProcessYellow; }
    if (k > 0) { processColors |= psProcessBlack; }
}
...
void PSOutputDev::updateTextMat (GfxState* state) {
    ...
    if (fabs (mat[0] * mat[3] - mat[1] * mat[2]) < 0.00001) {
        // avoid a singular (or close-to-singular) matrix
        writePSFmt (
            "[0.00001 0 0 0.00001 {0:.6g} {1:.6g}] Tm\n", mat[4], mat[5]);
    }
    else {
        writePSFmt (
            "[{0:.6g} {1:.6g} {2:.6g} {3:.6g} {4:.6g} {5:.6g}] Tm\n", mat[0],
...
void PSOutputDev::updateRender (GfxState* state) {
    ...
    writePSFmt ("{0:d} Tr\n", rm);
    rm &= 3;
    if (rm != 0 && rm != 3) { t3Cacheable = gFalse; }
}
...
void PSOutputDev::tilingPatternFill (
    ...
    if (paintType == 2) {
        writePSFmt (
            "{0:.6g} 0 {1:.6g} {2:.6g} {3:.6g} {4:.6g} setcachedevice\n", xStep,
            bbox[0], bbox[1], bbox[2], bbox[3]);
        t3FillColorOnly = gTrue;
    }
    else {
        if (x1 - 1 <= x0) { writePS ("1 0 setcharwidth\n"); }
        else {
    ...
    writePS ("fCol\n");
    writePSFmt (
        "gsave [{0:.6g} {1:.6g} {2:.6g} {3:.6g} {4:.6g} {5:.6g}] concat\n",
...
GBool PSOutputDev::functionShadedFill (
    ...
    writePSFmt (
        "/mat [{0:.6g} {1:.6g} {2:.6g} {3:.6g} {4:.6g} {5:.6g}] def\n", mat[0],
        mat[1], mat[2], mat[3], mat[4], mat[5]);
    ...
    if (shading->getNFuncs () == 1) {
        ...
        for (i = 0; i < shading->getNFuncs (); ++i) {
            if (i < shading->getNFuncs () - 1) { writePS ("2 copy\n"); }
...
GBool PSOutputDev::axialShadedFill (GfxState* state, GfxAxialShading* shading) {
    ...
    dx = x1 - x0;
    dy = y1 - y0;
    if (fabs (dx) < 0.01 && fabs (dy) < 0.01) { return gTrue; }
    else {
        mul = 1 / (dx * dx + dy * dy);
    ...
    if (shading->getNFuncs () == 1) {
        ...
        writePS ("/func {\n");
        for (i = 0; i < shading->getNFuncs (); ++i) {
            if (i < shading->getNFuncs () - 1) { writePS ("dup\n"); }
            cvtFunction (shading->getFunc (i));
            writePS ("exec\n");
...
GBool PSOutputDev::radialShadedFill (
    ...
    for (i = 0; i < n; ++i) {
        ...
        while (j < m) {
            if (subpath->getCurve (j)) {
                writePSFmt (
                    "{0:.6g} {1:.6g} {2:.6g} {3:.6g} {4:.6g} {5:.6g} c\n",
                    subpath->getX (j), subpath->getY (j), subpath->getX (j + 1),
                    subpath->getY (j + 1), subpath->getX (j + 2),
                    subpath->getY (j + 2));
                j += 3;
            }
            else {
...
void PSOutputDev::drawString (GfxState* state, GString* s) {
    ...
    while (len > 0) {
        n = font->getNextChar (
            p, len, &code, u, (int)(sizeof (u) / sizeof (Unicode)), &uLen, &dx,
            &dy, &originX, &originY);
    ...
    if (state->getRender () & 4) { haveTextClip = gTrue; }
}
...
void PSOutputDev::drawImageMask (
    ...
    case psLevel2Sep:
        doImageL2 (
            ref, NULL, invert, inlineImg, str, width, height, len, NULL, NULL,
            0, 0, gFalse);
        break;
    case psLevel3:
    case psLevel3Sep:
...
void PSOutputDev::drawImage (
    ...
    case psLevel2Sep:
        doImageL2 (
            ref, colorMap, gFalse, inlineImg, str, width, height, len,
            maskColors, NULL, 0, 0, gFalse);
        break;
    case psLevel3:
    case psLevel3Sep:
        doImageL3 (
...
void PSOutputDev::drawMaskedImage (
    GfxState* state, Object* ref, Stream* str, int width, int height,
    ...
    case psLevel2Sep:
        doImageL2 (
            ref, colorMap, gFalse, gFalse, str, width, height, len, NULL,
...
void PSOutputDev::doImageL1 (
    ...
    if ((inType3Char || preload) && !colorMap) {
        writePSFmt (
            "{0:d} {1:d} {2:s} [{3:d} 0 0 {4:d} 0 {5:d}] pdfImM1a\n", width,
        ...
        writePSFmt (
            "{0:d} {1:d} {2:s} [{3:d} 0 0 {4:d} 0 {5:d}] pdfImM1\n", width,
            height, invert ? "true" : "false", width, -height, height);
    }
    ...
    if (!((inType3Char || preload) && !colorMap)) {
        if (colorMap) {
            ...
            imgStr = new ImageStream (
                str, width, colorMap->getNumPixelComps (),
                colorMap->getBits ());
            imgStr->reset ();
...
void PSOutputDev::doImageL2 (
    ...
    if (maskColors && colorMap && !inlineImg) {
        ...
        for (y = 0; y < height; ++y) {
            ...
            while (x0 < width || i < rects0Len) {
                ...
                if (addRect || extendRect) {
                    ...
                    if (addRect) { rects1[rects1Len].y0 = y; }
                    if (extendRect) {
        ...
        for (y = 0; y < maskHeight; ++y) {
            ...
            while (x0 < maskWidth || i < rects0Len) {
                emitRect = addRect = extendRect = gFalse;
                if (x0 >= maskWidth) { emitRect = gTrue; }
                else if (i >= rects0Len) {
                    addRect = gTrue;
                }
                else if (rects0[i].x0 < x0) {
    ...
    if (mode == psModeForm || inType3Char || preload) {
        if (inlineImg) {
            ...
            if (globalParams->getPSLZW ()) { str2 = new LZWEncoder (str2); }
            else {
                str2 = new RunLengthEncoder (str2);
            }
            if (useASCIIHex) { str2 = new ASCIIHexEncoder (str2); }
            else {
    ...
    writePSFmt ("  /Height {0:d}\n", height);
    writePSFmt (
        "  /ImageMatrix [{0:d} 0 0 {1:d} 0 {2:d}]\n", width, -height, height);
    if (colorMap && colorMap->getColorSpace ()->getMode () == csDeviceN) {
        writePS ("  /BitsPerComponent 8\n");
    }
    else {
        writePSFmt (
            "  /BitsPerComponent {0:d}\n", colorMap ? colorMap->getBits () : 1);
    }
    ...
    if (colorMap) {
        ...
        if ((level == psLevel2Sep || level == psLevel3Sep) &&
            ...
            writePSFmt (
                "{0:.4g} {1:.4g}", colorMap->getDecodeLow (0) * n,
            ...
            for (i = 0; i < numComps; ++i) {
                if (i > 0) { writePS (" "); }
                writePSFmt (
                    "{0:.4g} {1:.4g}", colorMap->getDecodeLow (i),
    ...
    if ((mode == psModeForm || inType3Char || preload) &&
        ...
        s = str->getPSFilter (
            level < psLevel2 ? 1 : level < psLevel3 ? 2 : 3, "    ");
        if ((colorMap && colorMap->getColorSpace ()->getMode () == csDeviceN) ||
            inlineImg || !s) {
            if (globalParams->getPSLZW ()) {
                useLZW = gTrue;
    ...
    if (useCompressed) { writePS (s->getCString ()); }
    if (s) { delete s; }
    ...
    if (mode == psModeForm || inType3Char || preload) {
        ...
        // delete encoders
        if (useLZW || useRLE || useASCII || inlineImg) { delete str; }
    }
...
void PSOutputDev::doImageL3 (
    ...
    if (maskStr) {
        ...
        if (maskUseLZW) { maskFilters->append ("  /LZWDecode filter\n"); }
        else if (maskUseRLE) {
        ...
        if (mode == psModeForm || inType3Char || preload) {
            writePSFmt (
                "MaskData_{0:d}_{1:d} pdfMaskInit\n", ref->getRefNum (),
                ref->getRefGen ());
        }
        else {
            writePS ("currentfile\n");
            ...
            // add LZWEncode/RunLengthEncode and ASCIIHex/85 encode filters
            if (maskUseCompressed) { maskStr = maskStr->getUndecodedStream (); }
            if (maskUseLZW) { maskStr = new LZWEncoder (maskStr); }
            else if (maskUseRLE) {
                maskStr = new RunLengthEncoder (maskStr);
            }
            if (maskUseASCII) {
                if (useASCIIHex) { maskStr = new ASCIIHexEncoder (maskStr); }
                else {
            ...
            // delete encoders
            if (maskUseLZW || maskUseRLE || maskUseASCII) { delete maskStr; }
        }
    }
...
void PSOutputDev::dumpColorSpaceL2 (
    ...
    case csDeviceGray:
        writePS ("/DeviceGray");
        if (genXform) { writePS (" {}"); }
        if (updateColors) { processColors |= psProcessBlack; }
        break;
    ...
    case csCalGray:
        ...
        writePSFmt (
            " /MatrixA [{0:.4g} {1:.4g} {2:.4g}]\n", calGrayCS->getWhiteX (),
            calGrayCS->getWhiteY (), calGrayCS->getWhiteZ ());
        writePSFmt (
            " /WhitePoint [{0:.4g} {1:.4g} {2:.4g}]\n", calGrayCS->getWhiteX (),
            calGrayCS->getWhiteY (), calGrayCS->getWhiteZ ());
        writePSFmt (
            " /BlackPoint [{0:.4g} {1:.4g} {2:.4g}]\n", calGrayCS->getBlackX (),
            calGrayCS->getBlackY (), calGrayCS->getBlackZ ());
        writePS (">>]");
        if (genXform) { writePS (" {}"); }
        if (updateColors) { processColors |= psProcessBlack; }
        break;
    ...
    case csDeviceRGB:
        writePS ("/DeviceRGB");
        if (genXform) { writePS (" {}"); }
        if (updateColors) { processColors |= psProcessCMYK; }
        break;
    ...
    case csCalRGB:
        ...
        writePSFmt (
            ...
            calRGBCS->getGammaB ());
        writePSFmt (
            " /MatrixABC [{0:.4g} {1:.4g} {2:.4g} {3:.4g} {4:.4g} {5:.4g} "
            "{6:.4g} {7:.4g} {8:.4g}]\n",
        ...
        writePSFmt (
            " /WhitePoint [{0:.4g} {1:.4g} {2:.4g}]\n", calRGBCS->getWhiteX (),
            calRGBCS->getWhiteY (), calRGBCS->getWhiteZ ());
        writePSFmt (
            " /BlackPoint [{0:.4g} {1:.4g} {2:.4g}]\n", calRGBCS->getBlackX (),
            calRGBCS->getBlackY (), calRGBCS->getBlackZ ());
        writePS (">>]");
        if (genXform) { writePS (" {}"); }
        if (updateColors) { processColors |= psProcessCMYK; }
        break;
    ...
    case csLab:
        ...
        if (map01) {
            ...
            writePSFmt (
                " /RangeABC [0 100 {0:.4g} {1:.4g} {2:.4g} {3:.4g}]\n",
                labCS->getAMin (), labCS->getAMax (), labCS->getBMin (),
                labCS->getBMax ());
            writePS (
                " /DecodeABC [{16 add 116 div} bind {500 div} bind {200 div} "
                "bind]\n");
        }
        ...
        writePSFmt (
            "     {{4 29 div sub 108 841 div mul }} ifelse {0:.4g} mul}} "
            "bind\n",
            labCS->getWhiteX ());
        writePS ("    {dup 6 29 div ge {dup dup mul mul}\n");
        writePSFmt (
    ...
    case csIndexed:
        ...
        if (baseCS->getMode () == csDeviceN) {
            ...
            if (((GfxDeviceNColorSpace*)baseCS)->getAlt ()->getMode () ==
                csLab) {
                labCS = (GfxLabColorSpace*)((GfxDeviceNColorSpace*)baseCS)
                            ->getAlt ();
            }
            else {
                labCS = NULL;
            ...
            numAltComps =
                ((GfxDeviceNColorSpace*)baseCS)->getAlt ()->getNComps ();
            p = lookup;
            for (i = 0; i <= n; i += 8) {
                writePS ("  ");
                for (j = i; j < i + 8 && j <= n; ++j) {
    ...
    case csSeparation:
        ...
        writePS ("]");
        if (genXform) { writePS (" {}"); }
        if (updateColors) { addCustomColor (separationCS); }
        break;
...
void PSOutputDev::opiBegin13 (GfxState* state, Dict* dict) {
    ...
    if (obj1.isArray () && obj1.arrayGetLength () == 8) {
        ...
        writePSFmt (
            "%ALDImagePosition: {0:.4g} {1:.4g} {2:.4g} {3:.4g} {4:.4g} "
            "{5:.4g} {6:.4g} {7:.4g}\n",
...
GBool PSOutputDev::getFileSpec (Object* fileSpec, Object* fileName) {
    ...
    if (fileSpec->isDict ()) {
        ...
        if (fileName->isString ()) { return gTrue; }
        fileName->free ();
...
void PSOutputDev::type3D1 (
    GfxState* state, double wx, double wy, double llx, double lly, double urx,
    double ury) {
    if (t3String) {
        error (
            errSyntaxError, -1, "Multiple 'd1' operators in Type 3 CharProc");
        return;
    }
    t3WX = wx;
    t3WY = wy;
...
void PSOutputDev::writePSFmt (const char* fmt, ...) {
    ...
    va_start (args, fmt);
    if (t3String) { t3String->appendfv ((char*)fmt, args); }
    else {

...
enum struct rotation_t { none, quarter_turn, half_turn, three_quarters_turn };
...
template< typename T > struct point_t
{
    ...
    template< typename U > bool in(const bbox_t< U > &) const;
};
...
//
template< typename T > struct bbox_t
{
    using value_type = T;
...
template< typename T >
inline bool operator==(const bbox_t< T > &lhs, const bbox_t< T > &rhs)
{
    return std::equal(lhs.arr, lhs.arr + sizeof lhs.arr / sizeof *lhs.arr,
                      rhs.arr);
}
...
template< typename T >
inline bbox_t< T > operator+(const bbox_t< T > &lhs, const bbox_t< T > &rhs)
{
    return { (std::min)(lhs.arr[0], rhs.arr[0]),
...
template< typename T >
inline bbox_t< T > &operator+=(bbox_t< T > &lhs, const bbox_t< T > &rhs)
{
    return (lhs = { (std::min)(lhs.arr[0], rhs.arr[0]),
                    (std::min)(lhs.arr[1], rhs.arr[1]),
                    (std::max)(lhs.arr[2], rhs.arr[2]),
                    (std::max)(lhs.arr[3], rhs.arr[3]) });
}
...
template< typename T >
inline std::ostream &operator<<(std::ostream &ss, const bbox_t< T > &box)
{
    return ss << box.xmin << "," << box.ymin << "," << box.xmax << ","
              << box.ymax;
}

////////////////////////////////////////////////////////////////////////

template< typename T > inline bbox_t< T > normalize(bbox_t< T > x)
{
    if (x.arr[0] > x.arr[2]) {
        std::swap(x.arr[0], x.arr[2]);
    }
    if (x.arr[1] > x.arr[3]) {
        std::swap(x.arr[1], x.arr[3]);
    }
    return x;
}

template< typename T > inline T width_of(const bbox_t< T > &x)
{
    return x.arr[2] - x.arr[0];
}

template< typename T > inline T height_of(const bbox_t< T > &x)
{
    return x.arr[3] - x.arr[1];
}

template< typename T > inline std::tuple< T, T > center_of(const bbox_t< T > &x)
{
    return { x.arr[0] + (x.arr[2] - x.arr[0]) / 2,
             x.arr[1] + (x.arr[3] - x.arr[1]) / 2 };
}

template< typename T >
inline T horizontal_overlap(const bbox_t< T > &lhs, const bbox_t< T > &rhs)
{
    const auto dist =
        (std::min)(lhs.arr[2], rhs.arr[2]) - (std::max)(lhs.arr[0], rhs.arr[0]);
    return dist > 0 ? dist : 0;
}

template< typename T >
inline double vertical_overlap(const bbox_t< T > &lhs, const bbox_t< T > &rhs)
{
    const auto dist =
        (std::min)(lhs.arr[3], rhs.arr[3]) - (std::max)(lhs.arr[1], rhs.arr[1]);
    return dist > 0 ? dist : 0;
}

template< typename T >
inline bool overlapping(const bbox_t< T > &lhs, const bbox_t< T > &rhs)
{
    return horizontal_overlap(lhs, rhs) && vertical_overlap(lhs, rhs);
}

template< typename T >
inline T horizontal_distance(const bbox_t< T > &lhs, const bbox_t< T > &rhs)
{
...
template< typename T >
inline T vertical_distance(const bbox_t< T > &lhs, const bbox_t< T > &rhs)
{
...
template< typename T >
inline T min_height_of(const bbox_t< T > &lhs, const bbox_t< T > &rhs)
{
    return (std::min)(height_of(lhs), height_of(rhs));
}

template< typename T >
inline bbox_t< T > coalesce(const bbox_t< T > &lhs, const bbox_t< T > &rhs)
{
    return bbox_t< T >{ (std::min)(lhs.arr[0], rhs.arr[0]),
                        (std::min)(lhs.arr[1], rhs.arr[1]),
                        (std::max)(lhs.arr[2], rhs.arr[2]),
                        (std::max)(lhs.arr[3], rhs.arr[3]) };
}
...
{
    return box.xmin <= x && x < box.xmax && box.ymin <= y && y < box.ymax;
}

template< xpdf::rotation_t, typename > struct rotate_t;

template< typename T > struct rotate_t< xpdf::rotation_t::none, T >
{
    using box_type = bbox_t< T >;
    box_type operator()(const box_type &x, const box_type &) const { return x; }
};
...
#define XPDF_ROTATE_DEF(type, a, b, c, d)                                        \
    template< typename T > struct rotate_t< xpdf::rotation_t::type, T >          \
    {                                                                            \
        using box_type = bbox_t< T >;                                            \
...
template< xpdf::rotation_t, typename > struct unrotate_t;

template< typename T > struct unrotate_t< xpdf::rotation_t::none, T >
{
    using box_type = bbox_t< T >;
    box_type operator()(const box_type &x, const box_type &) const { return x; }
};
...
#define XPDF_UNROTATE_DEF(type, a, b, c, d)                                      \
    template< typename T > struct unrotate_t< xpdf::rotation_t::type, T >        \
    {                                                                            \
        using box_type = bbox_t< T >;                                            \
        box_type operator()(const box_type &x, const box_type &X) const          \
        {                                                                        \
...
inline detail::bbox_t< T > to(const detail::bbox_t< U > &x)
{
    return detail::bbox_t< T >{ T(x.arr[0]), T(x.arr[1]), T(x.arr[2]),
                                T(x.arr[3]) };
}

template< typename T, enable_if_floating_point_t< T > * = nullptr >
inline detail::bbox_t< T > ceil(const detail::bbox_t< T > &x)
{
    const auto &[a, b, c, d] = x.arr;
    return detail::bbox_t< T >{ std::floor(a), std::floor(b), std::ceil(c),
                                std::ceil(d) };
}

template< typename T, enable_if_floating_point_t< T > * = nullptr >
inline detail::bbox_t< T > floor(const detail::bbox_t< T > &x)
{
    const auto &[a, b, c, d] = x.arr;
    return detail::bbox_t< T >{ std::ceil(a), std::ceil(b), std::floor(c),
                                std::floor(d) };
}
...
inline detail::bbox_t< T > rotate(const detail::bbox_t< T > &box,
...
template< rotation_t rotation, typename T >
inline detail::bbox_t< T > unrotate(const detail::bbox_t< T > &box,
                                    const detail::bbox_t< T > &superbox)
{
    //
    // Rotation around origin followed by a translation, brings the box

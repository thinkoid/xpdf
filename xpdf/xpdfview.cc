...
static ArgDesc argDesc[] = {
    ...
    { "-title", argStringDummy, NULL, 0, "window title" },
    { "-cmap", argFlagDummy, NULL, 0, "install a private colormap" },
    { "-rgb", argIntDummy, NULL, 0,
      "biggest RGB cube to allocate (default is 5)" },
    { "-rv", argFlagDummy, NULL, 0, "reverse video" },
    { "-papercolor", argStringDummy, NULL, 0, "color of paper background" },
    { "-z", argStringDummy, NULL, 0,
      "initial zoom level (percent, 'page', 'width')" },
    { "-cont", argFlag, &contView, 0, "start in continuous view mode" },
...
int main (int argc, char* argv[]) {
    ...
    if (contView) { globalParams->setContinuousView (contView); }
    if (psFileArg[0]) { globalParams->setPSFile (psFileArg); }
    if (paperSize[0]) {
    ...
    if (level1) { globalParams->setPSLevel (psLevel1); }
    ...
    if (doRemoteRaise) { ok = ok && remoteName[5] && !doRemoteQuit; }
    if (doRemoteQuit) { ok = ok && remoteName[5] && argc == 1; }
    if (!ok || printVersion || printHelp) {
        fprintf (stderr, "xpdf version %s\n", PACKAGE_VERSION);
        fprintf (stderr, "%s\n", XPDF_COPYRIGHT);
        if (!printVersion) {
    ...
    if (argc >= 2) { fileName = new GString (argv[1]); }
    else {
        fileName = NULL;
    }
    pg = 1;
    destName = NULL;
    if (argc == 3) {
        if (argv[2][0] == '+') { destName = new GString (&argv[2][1]); }
        else {
            pg = atoi (argv[2]);
            if (pg < 0) {
    ...
    ownerPasswordStr = ownerPassword[0] != '\001' ? new GString (ownerPassword)
                                                  : (GString*)NULL;
    userPasswordStr =
        userPassword[0] != '\001' ? new GString (userPassword) : (GString*)NULL;

    // open the file and run the main loop
    if (destName) {
        if (!app->openAtDest (
                fileName, destName, ownerPasswordStr, userPasswordStr)) {
            exitCode = 1;
            goto done2;
        }
    }
    ...
    // clean up
done2:
    if (userPasswordStr) { delete userPasswordStr; }
    if (ownerPasswordStr) { delete ownerPasswordStr; }
    if (destName) { delete destName; }

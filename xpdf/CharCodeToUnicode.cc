...
CharCodeToUnicode::parseUnicodeToUnicode (GString* fileName) {
    ...
    while (getLine (buf, sizeof (buf), f)) {
        ...
        if (n == 1) { mapA[u0] = uBuf[0]; }
            ...
            sMapA[sMapLenA].c = u0;
            for (i = 0; i < n; ++i) { sMapA[sMapLenA].u[i] = uBuf[i]; }
...
void CharCodeToUnicode::parseCMap1 (
    int (*getCharFunc) (void*), void* data, int nBits) {
    PSTokenizer* pst;
    ...
    while (pst->getToken (tok2, sizeof (tok2), &n2)) {
        if (!strcmp (tok2, "usecmap")) {
            ...
            while (pst->getToken (tok1, sizeof (tok1), &n1)) {
                if (!strcmp (tok1, "endbfchar")) { break; }
            ...
            while (pst->getToken (tok1, sizeof (tok1), &n1)) {
                if (!strcmp (tok1, "endbfrange")) { break; }
                if (!pst->getToken (tok2, sizeof (tok2), &n2) ||
                ...
                if (code1 > maxCode || code2 > maxCode) {
                    ...
                    if (code2 > maxCode) { code2 = maxCode; }
                }
                if (!strcmp (tok3, "[")) {
                    i = 0;
                    while (pst->getToken (tok1, sizeof (tok1), &n1)) {
                        if (!strcmp (tok1, "]")) { break; }
                        if (tok1[0] == '<' && tok1[n1 - 1] == '>') {
                            if (code1 + i <= code2) {
                                tok1[n1 - 1] = '\0';
...
void CharCodeToUnicode::addMapping (
    CharCode code, char* uStr, int n, int offset) {
    CharCode oldLen, i;
    Unicode u;
    int j;
...
void CharCodeToUnicodeCache::add (CharCodeToUnicode* ctu) {
    ...
    if (cache[size - 1]) { cache[size - 1]->decRefCnt (); }
    for (i = size - 1; i >= 1; --i) { cache[i] = cache[i - 1]; }
    cache[0] = ctu;
    ctu->incRefCnt ();
}

...
template< typename T > inline bool lessX(const T &lhs, const T &rhs)
{
    return lhs->box.xmin < rhs->box.xmin;
};

template< typename T > inline bool lessY(const T &lhs, const T &rhs)
{
    return lhs->box.ymin < rhs->box.ymin;
};

template< typename T > inline bool lessYX(const T &lhs, const T &rhs)
{
    return lhs->box.ymin < rhs->box.ymin ||
...
//
inline bool lessPosX(const TextColumnPtr &lhs, const TextColumnPtr &rhs)
{
    return lhs->px < rhs->px;
};

inline bool lessCharPos(const TextWordPtr &lhs, const TextWordPtr &rhs)
{
...
//
void TextPage::doAddChar(GfxState *state, double x, double y, double dx,
                         double dy, CharCode c, int nBytes, Unicode *u, int uLen)
{
    ...
    if (uLen > 0) {
        ...
        for (i = 0; i < uLen; ++i) {
            ...
            chars.push_back(std::make_shared< TextChar >(
                TextChar{ curFont, curFontSize, xMin, yMin, xMax, yMax, u[j],
                          charPos, uint8_t(nBytes), uint8_t(curRot), clipped,
                          state->getRender() == 3 }));
        }
    }
...
//
void TextPage::doAddActualTextChar(double x, double y, double dx, double dy,
                                   int n)
{
    //
...
void TextPage::endActualText(GfxState *state)
{
    ...
    if (actualTextNBytes) {
        ...
        addChar(state, actualTextX0, actualTextY0, actualTextX1 - actualTextX0,
                actualTextY1 - actualTextY0, 0, actualTextNBytes, text.data(),
                text.size());
    }
    ...
    if (uMap->isUnicode()) {
        ...
        if (primaryLR) {
            ...
            while (i < len) {
                ...
                for (j = i; j < len &&
                            !(unicodeTypeL(text[j]) || unicodeTypeNum(text[j]));
                     ++j)
                    ;
                if (j > i) {
                    s->append(rle, rleLen);
...
template< typename T >
inline void do_rotate(T &t, double x0, double y0, double x1, double y1)
{
    t.box = { x0, y0, x1, y1 };
}

template< typename T > inline void rotate90(T &t, int w, int)
{
    const auto &[x0, y0, x1, y1] = t.box.arr;
    do_rotate(t, y0, w - x1, y1, w - x0);
};

template< typename T > inline void rotate180(T &t, int w, int h)
{
    const auto &[x0, y0, x1, y1] = t.box.arr;
    do_rotate(t, w - x1, h - y1, w - x0, h - y0);
}

template< typename T > inline void rotate270(T &t, int, int h)
{
    const auto &[x0, y0, x1, y1] = t.box.arr;
    do_rotate(t, h - y1, x0, h - y0, x1);
}

inline int prevalent_rotation_amongst(TextChars &chars)
{
...
template< typename T >
inline void do_unrotate(T &t, double x0, double y0, double x1, double y1, int n)
{
    t.box = { x0, y0, x1, y1 };
    t.rot = (t.rot + n) & 3;
}
...
template<>
inline void do_unrotate< TextColumn >(TextColumn &t, double x0, double y0,
...
template<>
inline void do_unrotate< TextParagraph >(TextParagraph &t, double x0, double y0,
                                         double x1, double y1, int)
{
    t.box = { x0, y0, x1, y1 };
}

template< typename T > inline void unrotate90(T &t, int w, int)
{
    const auto &[x0, y0, x1, y1] = t.box.arr;
    do_unrotate(t, w - y1, x0, w - y0, x1, 1);
};

template< typename T > inline void unrotate180(T &t, int w, int h)
{
    const auto &[x0, y0, x1, y1] = t.box.arr;
    do_unrotate(t, w - x1, h - y1, w - x0, h - y0, 2);
}

template< typename T > inline void unrotate270(T &t, int, int h)
{
    const auto &[x0, y0, x1, y1] = t.box.arr;
...
//
void TextPage::unrotateChars(TextChars &chars, int rot)
{
    switch (rot) {
    case 1:
        std::swap(pageWidth, pageHeight);
        ...
        for (auto &col : columns) {
            ...
            for (auto &par : col->paragraphs) {
                ...
                for (auto &line : par->lines) {
                    ...
                    actions::transform(line->edge,
                                       [=](auto &x) { return w - x; });

                    for (auto &word : line->words) {
                        unrotate180(*word, w, h);

                        actions::transform(word->edge,
                                           [=](auto &x) { return w - x; });
                    }
                }
            }
...
void TextPage::unrotateWords(TextWords &words, int rot)
{
    switch (rot) {
    case 0:
    default:
    ...
    case 2:
        for (auto &w : words) {
            ...
            actions::transform(w->edge,
                               [this](auto &x) { return pageWidth - x; });
        }
        break;
    ...
    case 3:
        for (auto &w : words) {
            ...
            actions::transform(w->edge,
                               [this](auto &x) { return pageHeight - x; });
        }
        break;
    }
}
...
inline bool duplicated(const TextChar &a, const TextChar &b, double x, double y)
{
    return a.c == b.c && fabs(a.box.xmin - b.box.xmin) < x &&
...
//
TextBlockPtr TextPage::splitChars(TextChars &charsA)
{
    ...
    for (const auto rot : { 0, 1, 2, 3 }) {
        ...
        copy_if(charsA, back_insert_iterator(chars2),
                [&](auto &c) { return c->rot == rot; });
...
TextBlockPtr TextPage::split(TextChars &charsA, int rot)
{
    ...
    if (doVertSplit) {
        ...
        for (x = start; x < xMaxI; ++x) {
            if (vprofile[x - xMinI] && !vprofile[x + 1 - xMinI]) {
                ...
                if (x - start > vertGapSize2) {
                    auto chars2 = charsIn(
                        charsA,
                        xpdf::bbox_t{ (prev + 0.5) * splitPrecision, box.ymin - 1,
                                      (start + 1.5) * splitPrecision,
                                      box.ymax + 1 });
        ...
        auto chars2 = charsIn(charsA, xpdf::bbox_t{ (prev + 0.5) * splitPrecision,
                                                    box.ymin - 1, box.xmax + 1,
                                                    box.ymax + 1 });
        ...
        for (y = start; y < yMaxI; ++y) {
            if (hprofile[y - yMinI] && !hprofile[y + 1 - yMinI]) {
                ...
                if (y - start > horizGapSize2) {
                    auto chars2 = charsIn(
                        charsA,
                        xpdf::bbox_t{ box.xmin - 1, (prev + 0.5) * splitPrecision,
                                      box.xmax + 1,
                                      (start + 1.5) * splitPrecision });
        ...
        auto chars2 = charsIn(charsA, xpdf::bbox_t{ box.xmin - 1,
...
TextChars TextPage::charsIn(TextChars &charsA, const xpdf::bbox_t &box) const
{
    const auto &a = box;
...
void TextPage::tagBlock(TextBlockPtr blk)
{
    ...
    if (blk->type == blkLeaf) {
        ...
        if (blk->type == ((blk->rot & 1) ? blkVertSplit : blkHorizSplit)) {
            ...
            for (auto &block : blk->as_blocks()) {
                if (block->tag != blkTagColumn && block->tag != blkTagLine) {
                    blk->tag = blkTagMulticolumn;
                    break;
                }
            }
...
void TextPage::doInsertLargeChars(TextChars &chars, TextBlockPtr pblock)
{
...
//
void TextPage::insertLargeChars(TextChars &chars, TextBlockPtr pblock)
{
    switch (chars.size()) {
    case 0:
        break;
...
// list of large chars.
//
void TextPage::insertLargeCharsInFirstLeaf(TextChars &  largeChars,
                                           TextBlockPtr blk)
{
    if (blk->type == blkLeaf) {
...
// it.
void TextPage::insertLargeCharInLeaf(TextCharPtr ch, TextBlockPtr blk)
{
    //~ this currently works only for characters in the primary rotation
...
void TextPage::insertIntoTree(TextBlockPtr blk, TextBlockPtr primaryTree)
{
    //
...
void TextPage::insertColumnIntoTree(TextBlockPtr column, TextBlockPtr tree)
{
...
void TextPage::insertClippedChars(TextChars &clippedChars, TextBlockPtr tree)
{
    //~ this currently works only for characters in the primary rotation
    ...
    while (!clippedChars.empty()) {
        ...
        for (size_t i = 0; i < clippedChars.size();) {
            ...
            if (ch2->box.xmin >
                ch->box.xmax + clippedTextMaxWordSpace * ch->size) {
                break;
            }
...
TextBlockPtr TextPage::findClippedCharLeaf(TextCharPtr ch, TextBlockPtr tree)
{
    //~ this currently works only for characters in the primary rotation
    ...
    if (tree->type == blkLeaf) {
        if (tree->rot == 0) {
            if (y > tree->box.ymin && y < tree->box.ymax &&
                ch->box.xmin <=
                    tree->box.xmax + clippedTextMaxWordSpace * ch->size) {
                return tree;
            }
        }
    } else {
...
TextColumnPtr TextPage::buildColumn(TextBlockPtr blk)
{
    ...
    return std::make_shared< TextColumn >(std::move(paragraphs), blk->box.xmin,
                                          blk->box.ymin, blk->box.xmax,
                                          blk->box.ymax);
}
...
TextColumns TextPage::buildColumns(TextBlockPtr tree)
{
    ...
    case blkTagMulticolumn: {
        ...
        for (auto &block : tree->as_blocks()) {
            ...
            columns.insert(columns.end(), std::make_move_iterator(other.begin()),
...
double TextPage::getLineIndent(const TextLine &line, TextBlockPtr blk) const
{
    double indent;
    ...
    case 0:
    default:
        indent = line.box.xmin - blk->box.xmin;
        break;
    case 1:
        indent = line.box.ymin - blk->box.ymin;
        break;
    case 2:
        indent = blk->box.xmax - line.box.xmax;
        break;
    case 3:
        indent = blk->box.ymax - line.box.ymax;
        break;
    }
...
double TextPage::getAverageLineSpacing(const TextLines &lines) const
{
    double avg = 0, sp;
    size_t n = 0;
...
double TextPage::getLineSpacing(const TextLine &lhs, const TextLine &rhs) const
{
    ...
    case 0:
    default:
        sp = rhs.box.ymin - lhs.box.ymax;
        break;
    case 1:
        sp = lhs.box.xmin - rhs.box.xmax;
        break;
    case 2:
        sp = lhs.box.ymin - rhs.box.ymin;
        break;
    case 3:
        sp = rhs.box.xmin - rhs.box.xmax;
        break;
    }
...
TextLinePtr TextPage::makeLine(TextBlockPtr blk)
{
    ...
    for (size_t i = 0, j; i < charsA.size();) {
        ...
        auto word = std::make_shared< TextWord >(charsA, i, j - i, int(blk->rot),
                                                 (blk->rot >= 2) ? spaceAfter2 :
                                                                   spaceAfter);
    ...
    return std::make_shared< TextLine >(std::move(words), blk->box.xmin,
                                        blk->box.ymin, blk->box.xmax,
                                        blk->box.ymax, lineFontSize);
}

TextChars TextPage::getLineOfChars(TextBlockPtr blk)
{
    if (blk->type == blkLeaf) {
        ...
        for (auto &block : blk->as_blocks()) {
            ...
            chars.insert(chars.end(), std::make_move_iterator(line.begin()),
        ...
        for (auto &par : col->paragraphs) {
            for (auto &line : par->lines) {
                ...
                if (control.fixedPitch > 0) {
                    line->px =
                        (line->box.xmin - col->box.xmin) / control.fixedPitch;
                } else if (fabs(line->fontSize) < 0.001) {
                    line->px = 0;
                } else {
                    line->px = (line->box.xmin - col->box.xmin) /
        ...
        if (control.fixedPitch) {
            ...
            for (j = 0; j < i; ++j) {
                ...
                if (xOverlap < slack * (col2->box.xmax - col2->box.xmin)) {
                    ...
                    yOverlap = (col->box.ymax < col2->box.ymax ? col->box.ymax :
                                                                 col2->box.ymax) -
                               (col->box.ymin > col2->box.ymin ? col->box.ymin :
                                                                 col2->box.ymin);
    ...
    for (ph = 0, i = 0; i < columns.size(); ++i) {
        ...
        for (j = 0; j < i; ++j) {
            ...
            if (yOverlap < slack * height_of(col2->box)) {
                ...
                xOverlap = (col->box.xmax < col2->box.xmax ? col->box.xmax :
                                                             col2->box.xmax) -
                           (col->box.xmin > col2->box.xmin ? col->box.xmin :
                                                             col2->box.xmin);
        ...
        for (auto &par : col->paragraphs) {
            for (auto &line : par->lines) {
                for (auto &word : line->words) {
                        ...
                        if (0 == height_of(underline.box)) {
                            if (word->rot == 0 || word->rot == 2) {
                                if (fabs(underline.box.ymin - base) < ubSlack &&
                                    underline.box.xmin <
                                        word->box.xmin + uSlack &&
                                    word->box.xmax - uSlack <
                                        underline.box.xmax) {
                            ...
                            if (word->rot == 1 || word->rot == 3) {
                                if (fabs(underline.box.xmin - base) < ubSlack &&
                                    underline.box.ymin <
                                        word->box.ymin + uSlack &&
                                    word->box.ymax - uSlack <
                                        underline.box.ymax) {
                                    word->underlined = true;
                                }
                            }
                    ...
                    // handle links
                    for (auto &link : links) {
                        if (link.box.xmin < word->box.xmin + hSlack &&
                            word->box.xmax - hSlack < link.box.xmax &&
                            link.box.ymin < word->box.ymin + hSlack &&
                            word->box.ymax - hSlack < link.box.ymax) {
                            word->link = link;
                        }
                    }
                }
...
#define XPDF_READING_ORDER_DEF(rot, a, b, c, d)                                  \
    template<>                                                                   \
    inline bool do_reading_order< xpdf::rotation_t::rot >(                       \
...
inline auto reading_order_of(int rotation)
{
    ...
    default:
    case 0:
        return reading_order< rotation_t::none, char_t >;
    case 1:
        return reading_order< rotation_t::quarter_turn, char_t >;
    case 2:
        return reading_order< rotation_t::quarter_turn, char_t >;
    case 3:
        return reading_order< rotation_t::three_quarters_turn, char_t >;
    }
};

std::vector< xpdf::bbox_t > search_all(const std::vector< char_t > &cs,
                                       std::wregex &                regex)
{
    std::wstring wstr;
    ...
    for (; iter != last; ++iter) {
        ...
        xs.push_back(accumulate(cs.begin() + pos, cs.begin() + pos + len,
...
bool TextPage::findText(Unicode *p, int len, bool startAtTop, bool stopAtBottom,
                        bool startAtLast, bool stopAtLast, bool caseSensitive,
                        bool backward, bool wholeWord, xpdf::bbox_t &box)
{
    ...
    for (int rot : { 0, 1, 2, 3 }) {
        ...
        transform(chars | rotated_by(rot), back_inserter(cs),
                  [](auto &ch) { return make_char(*ch); });
    ...
    auto iter2 = find_if(boxes, [&](auto &box) {
        return box.point[0].y > search_area.point[0].y ||
...
GString *TextPage::getText(const xpdf::bbox_t &box)
{
    ...
    for (auto &ch : chars) {
        ...
        if (box.xmin < xx && xx < box.xmax && box.ymin < yy && yy < box.ymax) {
            chars2.push_back(ch);
        }
    }
    ...
    for (colIdx = 0; colIdx < columns.size(); ++colIdx) {
        ...
        for (parIdx = 0; parIdx < col->paragraphs.size() && y < ph; ++parIdx) {
            ...
            for (lineIdx = 0; lineIdx < par->lines.size() && y < ph; ++lineIdx) {
                ...
                encodeFragment(line->text.data(), line->len, uMap, primaryLR,
                               out[y]);
                outLen[y] += line->pw;
